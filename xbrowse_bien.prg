...
*/
   ...
   METHOD FirstRow()     INLINE ::HeaderHeight() + If( ::lGetBar, IfNil( ::nGetBarHeight, 0 ), 0 )
      ...
      if ::lFastDraw
         for nFor := 1 to nLast
            ...
            if ! Empty( oCol:bBmpData ) .and. ! Empty( aBitmap := oCol:aBitmap( IfNil( XEval( oCol:bBmpData, oCol:Value() ), 0 ) ) )
               ::SayPalBmp( aBitmap, { nRow + 3, aCols[ nFor ] + 3, nRow + nHeight - 3, aCols[ nFor ] + oCol:nWidth - 3 }, ;
...
METHOD PaintHeader( hDC, aCols, nLast, hWhitePen, hGrayPen, hColPen ) CLASS TxBrowse
   ...
   if ::lRecordSelector
      if ! Empty( ::nRecSelHeadBmpNo )
         ::SayPalBmp( ::aBitmap( ::nRecSelHeadBmpNo ), { nRow, 2, nRow + nHeight, ::nRecSelWidth - 4 } )
...
METHOD RButtonDown( nRow, nCol, nKeyFlags ) CLASS TXBrowse
   ...
   elseif nRowPos > 0
      ...
      if oPopup != nil
         oPopup:Activate( (::nRowSel * ::nRowHeight) + ::nHeaderHeight, oCol:nDisplayCol, Self )
         return 0
...
static FUNCTION DolphinSeek( c, oBrw, cQryWhere )
   ...
   local cSortOrder, lSw := .f., lNumeric  := .F.
...
//      If( lNumeric, .F., .T. ), .T. ) != 0 )
   if lNumeric
      lRet  := ( oQry:Seek( c, cSortOrder, Max( 1, nStart - 1 ), oQry:LastRec(), .F., .T. ) != 0 )
      if !lRet
...
/*
   if oQry:FieldType( cSortorder  ) == 'N'
      if !ISDIGIT( Right( c, 1 ) ) //
         lSw  := .F.
      endif
      if lSw
         lRet  := ( oQry:Seek( Val( c ), cSortOrder, Max( 1, nStart - 1 ), oQry:LastRec(), .F., .T. ) != 0 )
         if !lRet
            lRet  := ( oQry:Seek( Val( c ), cSortOrder, Max( 1, nStart - 1 ), oQry:LastRec(), .T., .T. ) != 0 )
         endif
      else
         lRet  := .F.
      endif
      ...
      if ::nDataType != DATATYPE_ARRAY
         BEGIN SEQUENCE
            ...
            CASE Empty( cType )
               ...
               oSheet:Columns( nCol ):WrapText := .f.
               if oCol:nWidth > 100
            ...
            CASE cType == 'N'
               // Mascara SIEMPRE en notacion US neutra ( "#,##0.00" ): via OLE
               // la propiedad NumberFormat espera notacion inglesa, y Excel la
               // muestra despues con los separadores regionales de la PC que
               // abre el archivo (coma o punto, da igual). NO usar
               // Dbf2ExcelNumFormat(): arma la mascara con los separadores
               // locales y eso solo es valido para NumberFormatLocal.
               cFormat := PictToUSNumFormat( oCol:cEditPicture )
   ...
   if Empty( ::aSelected ) .or. Len( ::aSelected ) == 1
      ...
      if ::oRs != nil .AND. Len( aCols ) == ::oRs:Fields:Count()
         BEGIN SEQUENCE
            ...
            nRow := nDataRows + 2
         RECOVER
            nRow := EscribirCellwise( Self, oSheet, aCols, nDataRows, bProgress )
         END SEQUENCE
      else
         nRow := EscribirCellwise( Self, oSheet, aCols, nDataRows, bProgress )
      endif
      ...
      nRow := EscribirSeleccionadas( Self, oSheet, aCols )
   endif
         ...
         if oCol:lTotal
            BEGIN SEQUENCE
               cFormula := "SUBTOTAL(" + ;
                  ...
                  "," + ;
                  oSheet:Range( oSheet:Cells( 2, nCol ), oSheet:Cells( nRow - 1, nCol ) ):Address( .f., .f. ) + ;
                  ")"
...
//----------------------------------------------------------------
// Arma la mascara numerica de Excel en notacion US neutra a partir
// del picture Clipper. Siempre con separador de miles, y decimales
// segun el picture (2 por defecto si no hay picture). El resultado
// se asigna a NumberFormat (que via OLE espera notacion inglesa) y
// cada PC lo muestra con su propia configuracion regional.
//----------------------------------------------------------------
STATIC FUNCTION PictToUSNumFormat( cPic )
   ...
   if Empty( cPic )
      cRet += ".00"   // sin picture: formato estandar con 2 decimales
   else
      if Left( cPic, 1 ) == '@'
...
STATIC FUNCTION CrearProgreso( nTotal )
   ...
   local oDlg, oMeter, nValue := 0, oText
   ...
   @ 30, 10 METER oMeter VAR nValue TOTAL nTotal ;
            SIZE 170, 15 PIXEL OF oDlg

   @ 55, 140 BUTTON "Cancelar" ;
            SIZE 30, 12 PIXEL OF oDlg ;
            ACTION ( lCancelarExport := .t. )
...
// CHARTS
   ...
   DATA nBtnWidth, cBtnCaption, bClrBtn
   DATA bCreateBtn
...
// ACCESS METHODS
   ...
   DATA oBarGet, uBarGetVal, bBarGetValid, cBarGetPic
...
//   METHOD ChangeBitmap( ) // ButtonGet
   ...
   METHOD PaintBmpAndText( aRect, cText, nStrAlign, bLeftText, oFont, nTxtColor, ;
                        nBmpNo, nBmpAlign, lBmpTransparent, nAlpha, lBmpStretch )
...
METHOD DataHeight() CLASS TXBrwColumn
   ...
   if ::cDataType $ "PF" .and. ! ::lFullHeight
      ...
      if ValType( ::Value ) == 'C' .and. ! Empty( ::Value )
         aPalBmp  := ::oBrw:ReadPalBmpEx( ::Value,, ::oBrw:lGDIP )
         if ! Empty( aPalBmp[ 1 ] )
...
METHOD DataTextWidth( lCurrent ) CLASS TXBrwColumn
   ...
   DEFAULT nDataLen := ::nDataLen, nDataDec := ::nDataDec
   ...
   if ( ! Empty( nDataLen ) .and. cType $ "CN" ) .or. lUser
      if cType == 'N'
         if Empty( nDataDec )
            cText    := Replicate( '9', nDataLen )
         else
            cText    := Replicate( '9', nDataLen - nDataDec - 1 ) + '.' + ;
                        Replicate( '9', nDataDec )
         endif
         DEFAULT ::cEditPicture := NUMPICT( nDataLen, nDataDec )
         cText       := cValToStr( Val( cText ), ::cEditPicture )
         nWidth      := ::oBrw:GetWidth( cText, oFont )
      else
         cText    := Replicate( 'B', nDataLen )
...
METHOD PaintBmpAndText( aRect, cText, nStrAlign, bLeftText, oFont, nTxtColor, ;
                        nBmpNo, nBmpAlign, lBmpTransparent, nAlpha, lBmpStretch, cOrder ) CLASS TXBrwColumn
   ...
   if ! Empty( cOrder ) // Need to paint Sort Bmp in Header
      ...
      ::oBrw:SayPalBmp( aBitmap, oCellRect:aRect, .t., 0, nil, .f., "R" )
      oCellRect:nRight  -= ( aBitmap[ BITMAP_WIDTH ] + BMP_EXTRAWIDTH )
   endif
/*
   oBmpRect          := If( ValType( aRect ) == 'O', aRect, TRect():New( aRect ) )
   oBmpRect:nRight   := Min( oBmpRect:nRight, ::oBrw:GridWidth() - 4 )
   WITH OBJECT oBmpRect
      :nLeft         += ( COL_EXTRAWIDTH / 2 )
      :nTop          += ( ROW_EXTRAHEIGHT / 2 )
...
*/
   ...
   if ! Empty( aBitmap )
      ...
      ::oBrw:SayPalBmp( aBitmap, oBmpRect:aRect, lBmpTransparent, If( lBmpStretch, .t., 0 ), nAlpha )
   endif

   if ! Empty( cText ) .and. ValType( cText ) == 'C'
      cLeftText   := LeftText( Self, bLeftText, @cText, lAnd( nStrAlign, AL_RIGHT ) .and. FontEsc( oFont ) == 0 )
      if ! Empty( cLeftText )
         ::oBrw:SayText( cLeftText, oTxtRect:aRect, 'L', oFont, nTxtColor, nOr( DT_MODIFYSTRING, DT_EDITCONTROL, DT_NOPREFIX ) )
      endif
      cAlign      := If( lAnd( nStrAlign, AL_RIGHT ), 'R', If( lAnd( nStrAlign, AL_CENTER ), "", 'L' ) )
      ::oBrw:SayText( cText, oTxtRect:aRect, cAlign, oFont, nTxtColor, nOr( DT_MODIFYSTRING, DT_EDITCONTROL, DT_NOPREFIX ) )
   endif
...
METHOD PaintCell( nRow, nCol, nHeight, lHighLite, lSelected, nOrder, nPaintRow ) CLASS TXBrwColumn
   ...
   if ::lProgBar .and. oRect:nWidth > 4
      ...
      nWidth1  := Min( ::Value() * oRect:nWidth / IfNil( XEval( ::nProgTot, Self ), 1 ), oRect:nWidth )
      FillRect( hDC, { oRect:nTop, oRect:nLeft, oRect:nBottom, oRect:nLeft + nWidth1 }, hBrush1 )
      FillRect( hDC, { oRect:nTop, oRect:nLeft + nWidth1 + 1, oRect:nBottom, oRect:nRight }, hBrush2 )
   ...
   if !Empty( aBitmap := ::aBitmap( nBmpNo ) )
      ...
      if oRect:nWidth >= nBmpWidth //aBitmap[ BITMAP_WIDTH ]
         ::oBrw:SayPalBmp( aBitmap, oRect:aRect, ::lBmpTransparent, ::lBmpStretch, ::nAlphaLevel(), .f., ::cAlign( ::nDataBmpAlign ) )
...
//      ::DrawChart( hDC, { nRow, nCol, nRow + nHeight, Min( nCol + nWidth, ::oBrw:GridWidth() - 4 ) } )
   ...
   if ::bPaintText == nil
      if ! Empty( cData )
         ::PaintCellText( hDC, cData, oRect:aRect, { nTextColor, aColors[ 2 ] }, lHighLite, nHeight, nFontHeight, oFont )
      endif
   else
      SetTextColor( hDC, nTextColor )
      SetBkMode( hDC, 1 )
      Eval( ::bPaintText, Self, hDC, cData, oRect:aRect, aColors, lHighLite, lSelected )
...
METHOD PaintCellImage( cImageData, oRect ) CLASS TXBrwColumn
   ...
   local aBmpPal, aRect, ResizeMode, ImageDim

   aRect    := oRect:aRect
   if ::ImageDim == nil
      ResizeMode  := ::lBmpStretch
   else
      ImageDim    := XEval( ::ImageDim, Self )
      if ValType( ImageDim ) $ 'CN'
         ResizeMode  := ImageDim
      elseif ValType( ImageDim ) == 'A'
         if Len( ImageDim ) > 4
            ResizeMode := ImageDim[ 5 ]
         else
            ASize( ImageDim, 4 )
         endif
         aRect := oRect:Modify( ImageDim ):aRect
      endif
   endif

   aBmpPal     := ::oBrw:ReadPalBmpEx( cImageData,, ::oBrw:lGDIP )
   ::oBrw:SayPalBmp( aBmpPal, aRect, ;
...
METHOD PaintCellText( hDC, cData, aRect, aColors, lHighLite, nHeight, nFontHeight, oFont ) CLASS TXBrwColumn
   ...
   ::oBrw:SayText( cData, aRect, cAlign, oFont, aColors[ 1 ], nOr( DT_MODIFYSTRING, DT_EDITCONTROL, DT_NOPREFIX ) )
...
METHOD CreateBarGet() CLASS  TXBrwColumn
   ...
   if ::uBarGetVal != nil .and. ::oBarGet == nil
      ...
      ::oBarGet:bKeyChar := { |k| If( k == VK_TAB .or. k == VK_RETURN, ( ::oBrw:SetFocus(), 0 ), nil ) }
   endif
      ...
      WITH OBJECT ::oBarGet
         ...
         if :lVisible
            ...
            if lCheckShow
               ...
               nCol  := ::nDisplayCol + 4
               if :nTop != nRow .or. :nLeft != nCol .or. :nWidth != ( ::nWidth - 8 )
...
static function fnAddBitmap( o, uBmp, aResize )
   ...
   aBmpPal  := oBrw:ReadPalBmpEx( uBmp, aResize, oBrw:lGDIP .and. Empty( aResize ) )
...
//      if ::bFooter != nil
      if ::bFooter != nil .and. Empty( ::nFooterType ) // 2016-04-25
...
function CalcLineWH( oWnd, cLine )
   ...
   h  := DrawTextEx( oWnd:GetDC(), cLine, aRect, DT_SINGLELINE + DT_CALCRECT, @r )
   oWnd:ReleaseDC()
