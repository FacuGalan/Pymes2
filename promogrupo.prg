...
*******************************************************************************
** MOTOR DE CALCULO DE LAS PROMOS POR GRUPO
**
** Se llama DESPUES de CalcularPromos(), una vez que ya estan puestas en la tabla
** de detalle las lineas ESPROMO de las promos por articulo.
**
** Es UNA sola implementacion para las 4 pantallas: lo unico que cambia entre
** ellas es el nombre de la tabla de detalle, que entra por parametro.
**   fact.prg / fact_multi.prg -> "VENTAS_DET_H1"
**   ventas.prg                -> "ventas_det_H"
**   facrem.prg                -> "VENTAS_DET_H3"
** La columna de costo (PCOSTO en unas, COSTO en otra) se omite del INSERT a
** proposito: tiene DEFAULT en las tres tablas, asi que no hace falta nombrarla
** y el motor queda identico para todas.
**
** Parametros:
**   cTabla  = nombre de la tabla de detalle
**   nTipoFP = codigo de forma de pago, el mismo que usa CalcularPromos en el
**             FIND_IN_SET: IF(EMPTY(aFormaTip[nFormaPago]),0,aFormaTip[nFormaPago])
**
** Si las tablas del modulo todavia no existen (nadie entro nunca al ABM),
** sale sin hacer nada.
*******************************************************************************
FUNCTION CalcularPrGrupo(cTabla, nTipoFP)
LOCAL oQryGrp, oQryCar, oQryDet, oQryPri
LOCAL aCarro := {}, aPrioProd := {}, aA := {}, aZ := {}, aBenef := {}, aBajas := {}
LOCAL i, j, n, nCod, nSumA, nK, nCantZ, nAplicar, nResta, nToma, nBenefUnit
LOCAL nNeto, nIva, nBruto, nFactor, cNom, cFp

IF EMPTY(cTabla)
   RETURN nil
ENDIF
//Todavia no se creo el modulo: no hay nada que calcular
IF !oApp:oServer:TableExist('ge_'+oApp:cId+"promo_grupo") .or. ;
   !oApp:oServer:TableExist('ge_'+oApp:cId+"promo_grupo_det")
   RETURN nil
ENDIF
IF nTipoFP == nil
   nTipoFP := 0
ENDIF
cFp := ALLTRIM(STR(nTipoFP))

//--- Grupos vigentes, de mayor a menor prioridad -----------------------------
oQryGrp := oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"promo_grupo "+;
   "WHERE CURRENT_DATE BETWEEN fecha_inicio AND fecha_fin "+;
   "  AND CURTIME() BETWEEN hora_inicio AND hora_fin "+;
   "  AND FIND_IN_SET('"+cFp+"',formapago) > 0 "+;
   "ORDER BY prioridad DESC, id")
IF oQryGrp:nRecCount == 0
   oQryGrp:End()
   RETURN nil
ENDIF

//--- Carrito: solo las lineas reales, ordenadas por precio DESC ---------------
//    Se benefician las mas caras primero, asi que basta con recorrer en orden.
//    [1]=codart [2]=punit [3]=codiva [4]=cantidad [5]=unidades ya beneficiadas
oQryCar := oApp:oServer:Query("SELECT CODART, PUNIT, CODIVA, SUM(CANTIDAD) AS CANT "+;
   "FROM "+cTabla+" WHERE IFNULL(ESPROMO,0) = 0 AND CODART > 0 "+;
   "GROUP BY CODART, PUNIT, CODIVA ORDER BY PUNIT DESC")
oQryCar:GoTop()
DO WHILE !oQryCar:Eof()
   AADD(aCarro,{oQryCar:CODART, oQryCar:PUNIT, oQryCar:CODIVA, oQryCar:CANT, 0})
   oQryCar:Skip()
ENDDO
oQryCar:End()
IF LEN(aCarro) == 0
...
//--- Prioridad de la promo por articulo que YA se aplico, para la jerarquia ---
//    Solo interesan los articulos que efectivamente tienen linea ESPROMO.
IF oApp:oServer:TableExist('ge_'+oApp:cId+"promociones")
   oQryPri := oApp:oServer:Query("SELECT d.CODART AS codart, MAX(p.prioridad) AS prio "+;
      "FROM "+cTabla+" d JOIN ge_"+oApp:cId+"promociones p ON p.codart = d.CODART "+;
      "WHERE IFNULL(d.ESPROMO,0) = 1 AND d.CODART > 0 "+;
      "  AND CURRENT_DATE BETWEEN p.fecha_inicio AND p.fecha_fin "+;
      "  AND CURTIME() BETWEEN p.hora_inicio AND p.hora_fin "+;
      "  AND FIND_IN_SET('"+cFp+"',p.formapago) > 0 "+;
      "GROUP BY d.CODART")
   oQryPri:GoTop()
   DO WHILE !oQryPri:Eof()
      AADD(aPrioProd,{oQryPri:codart, oQryPri:prio})
      oQryPri:Skip()
   ENDDO
   oQryPri:End()
ENDIF
...
//--- Un pase por grupo -------------------------------------------------------
oQryGrp:GoTop()
DO WHILE !oQryGrp:Eof()
   aA := {}
   aZ := {}
   oQryDet := oApp:oServer:Query("SELECT codart, rol FROM ge_"+oApp:cId+"promo_grupo_det "+;
              "WHERE grupo_id = "+ClipValue2Sql(oQryGrp:id))
   oQryDet:GoTop()
   DO WHILE !oQryDet:Eof()
      IF ALLTRIM(cValToChar(oQryDet:rol)) == "Z"
         AADD(aZ,oQryDet:codart)
      ELSE
         AADD(aA,oQryDet:codart)
      ENDIF
      oQryDet:Skip()
   ENDDO
   oQryDet:End()
   //En el Mix no se guardan filas 'Z': Z es el mismo conjunto que A y M = N
   //(mismo_grupo es TINYINT -> Dolphin lo devuelve LOGICO, no se compara contra 1)
   IF oQryGrp:mismo_grupo
      aZ     := aA
      nCantZ := oQryGrp:cant_a
   ELSE
      nCantZ := oQryGrp:cant_z
   ENDIF
   IF LEN(aA) == 0 .or. LEN(aZ) == 0 .or. oQryGrp:cant_a <= 0 .or. nCantZ <= 0
      oQryGrp:Skip()
      LOOP
   ENDIF

   //Cuantas veces entra la promo
   nSumA := 0
   FOR i := 1 TO LEN(aCarro)
      IF ASCAN(aA,aCarro[i][1]) > 0
         nSumA += aCarro[i][4]
      ENDIF
   NEXT
   IF nSumA < oQryGrp:cant_a
      oQryGrp:Skip()
      LOOP
   ENDIF
   nK := INT(nSumA / oQryGrp:cant_a)
   IF oQryGrp:max_aplica > 0
      nK := MIN(nK, oQryGrp:max_aplica)
   ENDIF
   nAplicar := nK * nCantZ
   IF nAplicar <= 0
      oQryGrp:Skip()
   ...
   //Reparto del beneficio: aCarro ya viene por precio DESC
   aBenef := {}
   aBajas := {}
   nResta := nAplicar
   FOR i := 1 TO LEN(aCarro)
      ...
      ENDIF
      nCod := aCarro[i][1]
      IF ASCAN(aZ,nCod) == 0 .or. aCarro[i][4] - aCarro[i][5] <= 0
         LOOP
      ENDIF
      //Beneficio por unidad. Si el articulo ya esta mas barato que el precio de
      //promo no se toca ni se consume: seria regalarle unidades al tope.
      IF oQryGrp:tipo_benef == 1
         nBenefUnit := aCarro[i][2] - oQryGrp:precio_unitario
      ELSE
         nBenefUnit := aCarro[i][2] * oQryGrp:porcentaje / 100
      ENDIF
      IF nBenefUnit <= 0
         LOOP
      ENDIF
      //Jerarquia contra la promo por articulo (gana el numero de prioridad mas alto)
      n := ASCAN(aPrioProd,{|x| x[1] == nCod})
      IF n > 0
         IF oQryGrp:prioridad <= aPrioProd[n][2]
            LOOP   //gana la promo por articulo: el grupo no toca este codigo
         ENDIF
         IF ASCAN(aBajas,nCod) == 0
            AADD(aBajas,nCod)
         ENDIF
      ENDIF
      nToma := MIN(aCarro[i][4] - aCarro[i][5], nResta)
      n := ASCAN(aBenef,{|x| x[1] == aCarro[i][3]})
      IF n == 0
         AADD(aBenef,{aCarro[i][3], 0})
         n := LEN(aBenef)
      ENDIF
      aBenef[n][2]  += nToma * nBenefUnit
      aCarro[i][5]  += nToma
      nResta        -= nToma
   NEXT

   IF LEN(aBenef) > 0
      //El grupo le gano a la promo por articulo: se da de baja esa promo
      FOR j := 1 TO LEN(aBajas)
         oApp:oServer:Execute("DELETE FROM "+cTabla+" WHERE ESPROMO = TRUE AND CODART = "+;
                              ClipValue2Sql(aBajas[j]))
         //Ya no existe, asi que no puede ganarle a un grupo posterior
         n := ASCAN(aPrioProd,{|x| x[1] == aBajas[j]})
         IF n > 0
            ADEL(aPrioProd,n)
            ASIZE(aPrioProd,LEN(aPrioProd)-1)
         ENDIF
      NEXT
      //Una linea negativa por tasa de IVA, igual que el descuento por forma de pago
      cNom := LEFT(ALLTRIM(cValToChar(oQryGrp:nompromo)),60)
      FOR j := 1 TO LEN(aBenef)
         nBruto  := aBenef[j][2]
         nFactor := IF(aBenef[j][1] == 4, 1.105, IF(aBenef[j][1] == 5, 1.21, 1))
         nNeto   := nBruto / nFactor
         nIva    := nBruto - nNeto
         oApp:oServer:Execute("INSERT INTO "+cTabla+;
            " (CODART,DETART,CANTIDAD,PUNIT,NETO,DESCUENTO,STOTAL,IVA,CODIVA,PTOTAL,ESPROMO) VALUES (0,"+;
            ClipValue2Sql(cNom)+",1,"+ClipValue2Sql(-nBruto)+","+;
            ClipValue2Sql(-nNeto)+",0,"+ClipValue2Sql(-nNeto)+","+ClipValue2Sql(-nIva)+","+;
            ClipValue2Sql(aBenef[j][1])+","+ClipValue2Sql(-nBruto)+",1)")
      NEXT
   ENDIF
   oQryGrp:Skip()
ENDDO
