...
// Progreso embebido en el dialogo principal (se usa al generar las facturas)
STATIC oSayAyu, oSayProg, oMeterProg, nProgVal := 0, oBotGen, oBotCer
...
DEFINE DIALOG oForm TITLE "Facturacion por Lote (Cuenta Corriente)" FROM 05,15 TO 34,90 OF oApp:oWnd FONT oFont
   acor := AcepCanc(oForm)
   ...
   @ 25,010 SAY oSayAyu PROMPT "Click en Fiscal / NO facturar para tildar.  DELETE saca de la lista.  Doble clic = ver detalle." ;
            OF oForm PIXEL SIZE 430,12 COLOR CLR_GRAY
   //Progreso de la generacion: ocupa la misma fila del texto de ayuda (se oculta/muestra)
   @ 165,010 SAY oSayProg PROMPT "" OF oForm PIXEL SIZE 215,12 CENTER
   @ 180,010 METER oMeterProg VAR nProgVal TOTAL 100 OF oForm SIZE 215,12 PIXEL
   ...
   oBotGen := oBot[1]
   oBotCer := oBot[2]
ACTIVATE DIALOG oForm CENTER ON INIT (oSayProg:Hide(),oMeterProg:Hide())
oSayAyu := oSayProg := oMeterProg := oBotGen := oBotCer := nil
IF oQryLote != nil
   oQryLote:End()
   oQryLote := nil
ENDIF
...
*************************************************
** Progreso embebido: tapa el texto de ayuda con el SAY de estado + la barra
STATIC FUNCTION IniProgresoLote( nTot )
IF oSayAyu != nil
   oSayAyu:Hide()
ENDIF
nProgVal := 0
IF oMeterProg != nil
   oMeterProg:SetTotal( MAX(nTot,1) )
   oMeterProg:Set( 0 )
   oMeterProg:Show()
ENDIF
IF oSayProg != nil
   oSayProg:SetText( "Preparando..." )
   oSayProg:Show()
ENDIF
IF oBotGen != nil ; oBotGen:Disable() ; ENDIF
IF oBotCer != nil ; oBotCer:Disable() ; ENDIF
CursorWait()
SysRefresh()
RETURN nil
...
*************************************************
** Avance del proceso de generacion (cliente en curso)
STATIC FUNCTION VerProgresoLote( nAct, nTot, cNom )
IF oMeterProg != nil
   oMeterProg:Set( nAct )
ENDIF
IF oSayProg != nil
   oSayProg:SetText( "Facturando "+ALLTRIM(STR(nAct))+" / "+ALLTRIM(STR(nTot))+;
                     "   "+ALLTRIM(cNom) )
ENDIF
SysRefresh()
RETURN nil
...
*************************************************
** Restaura el dialogo al estado normal (texto de ayuda y botones)
STATIC FUNCTION FinProgresoLote()
CursorArrow()
IF oMeterProg != nil
   oMeterProg:Set( 0 )
   oMeterProg:Hide()
...
IF oSayAyu != nil
   oSayAyu:Show()
ENDIF
IF oBotGen != nil ; oBotGen:Enable() ; ENDIF
IF oBotCer != nil ; oBotCer:Enable() ; ENDIF
SysRefresh()
RETURN nil
...
LOCAL oQryG, nOk := 0, cMsg, nTotG, nActG := 0
IF oQryLote == nil
   MsgInfo("No hay clientes para facturar (todos estan marcados como NO facturar).","Ventas por lote")
   RETURN nil
ENDIF
nTotG := oApp:oServer:Query("SELECT COUNT(*) AS n FROM lote_clientes WHERE NOT nofacturar"):n
IF nTotG == 0
   MsgInfo("No hay clientes para facturar (todos estan marcados como NO facturar).","Ventas por lote")
   RETURN nil
ENDIF
