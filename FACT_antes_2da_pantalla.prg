...
LOCAL oQ, cSql := "SELECT id, nompromo, "+;
    "CASE tipo  "+;
    "    WHEN 2 THEN CONCAT( "+;
...
IF oQ:nRecCount = 0
   oSay:SetText(" ")
   ELSE 
   oSay:SetText(ALLTRIM(oQ:nompromo)+" "+alltrim(oQ:descripcion_promo))
ENDIF
RETURN nil   
...
function MostrarDlgEnSegundoMonitor( lForzarPrimario )
   ...
   // Hacer el proceso DPI-AWARE antes de consultar monitores. Sin esto,
   // Windows devuelve dimensiones virtualizadas (escaladas) para apps
   // DPI-unaware, lo que hace que en monitores con escalado >100% (4K,
   // alta densidad) las coordenadas vengan mucho m? chicas que la
   // resoluci? real. Llam?dola ahora, ObtenerMonitores() devuelve los
   // tama?s f?icos verdaderos del monitor secundario.
   SetProcessDPIAware()
   ...
   // (gracias a SetProcessDPIAware ya hecho arriba).
   ...
   if File( cLogo )
      @ nLogoY, nLogoX IMAGE oImg OF oDlgCust ;
           FILE cLogo ;
           SIZE nLogoW, nLogoH PIXEL ADJUST NOBORDER
   endif
