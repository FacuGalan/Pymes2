...
FOR nFp := 1 TO 6
   IF aForma[nFp]
      cFpForma += IF(EMPTY(cFpForma),""," OR ")+"FIND_IN_SET('"+LTRIM(STR(nFp))+"',formapago)"
   ENDIF
NEXT
cFpForma := IF(EMPTY(cFpForma),""," AND ("+cFpForma+") ")
...
IF lAlta
   IF base:tipo = 2 .or. base:tipo = 3
      IF oApp:oServer:Query("SELECT id FROM ge_"+oApp:cId+"promociones "+;
                            ...
                            " AND codart = "+ClipValue2Sql(base:codart)+cFpForma):nRecCount > 0
...
LOCAL oGet := ARRAY(25), oBot := ARRAY(6), oQryNue, oQryPro, oForm, lRta := .f., aCor, base, oError,;
      ...
      nNuevas, nReempla, nSolapa, nPrecioFijo := 0
TEXT INTO cSql 
...
DEFINE DIALOG oForm TITLE "Creacion de Promociones Multiples" FROM 05,15 TO 43,130 OF oWnd
   ...
   @155,426 BUTTON oBot[4] PROMPT "+ +" SIZE 20,12 OF oForm PIXEL ACTION(AgregaArtGen(oForm,oQryNue,oBrwNue,base,aForma,nPrecioFijo),oQryNue:Refresh(),;
                                                                       oBrwNue:Refresh())
   @170,426 BUTTON oBot[3] PROMPT " + " SIZE 20,12 OF oForm PIXEL ACTION(AgregaArt(oForm,oQryNue,oBrwNue,base,aForma,nPrecioFijo),oQryNue:Refresh(),;
                                                                       oBrwNue:Refresh())                                                                       
...
TRY
  ...
  oApp:oServer:Execute("INSERT INTO ge_"+oApp:cId+"promociones (codart,nompromo,tipo,fecha_inicio,fecha_fin,"+;
                       ...
                       "base_porcentual,prioridad) "+;
                       " (SELECT codart,nompromo,tipo,fecha_inicio,fecha_fin,"+;
                       "precio_especial,cantidad_requerida,cantidad_a_pagar,descuento_a_unidad,descuento_porcentual,"+;
                       "cantidad_minima,cantidad_maxima,precio_unitario,formapago,relacion_porcentual,base_calculo,"+;
                       "base_porcentual,prioridad FROM promos_temp WHERE estado <> 'Solapamiento')")
  oApp:oServer:CommitTransaction() 
  oQry:Refresh()
  oBrw:Refresh() 
CATCH oError
...
LOCAL oDlgA, oError, oGet:=ARRAY(12), ;
      oBot:=ARRAY(2), acor:=ARRAY(4), lRta:=.f., lBorrar:=.f., cWhere, cSql, cInter:="",;
...
INSERT INTO promos_temp (codart,nomart,nompromo,tipo,fecha_inicio,fecha_fin,precio_especial,cantidad_requerida,
                         ...
                         precio_unitario,formapago,relacion_porcentual,base_calculo,base_porcentual,prioridad)
ENDTEXT
...
oApp:oServer:Execute("UPDATE promos_temp AS pt JOIN ge_"+oApp:cId+"promociones AS gp ON pt.codart = gp.codart "+;
                     ...
                     " AND gp.tipo IN (1, 2, 3)"+cInter)
oApp:oServer:Execute("UPDATE promos_temp AS pt JOIN ge_"+oApp:cId+"promociones AS gp ON pt.codart = gp.codart "+;
                     "SET pt.estado = 'Existe, reemplaza', pt.idoriginal = gp.id WHERE pt.tipo = 4 "+;
                     " AND gp.tipo = 4 AND pt.cantidad_minima = gp.cantidad_minima AND pt.cantidad_maxima = gp.cantidad_maxima"+cInter)
oApp:oServer:Execute("UPDATE promos_temp AS pt JOIN ge_"+oApp:cId+"promociones AS gp ON pt.codart = gp.codart "+;
                     ...
                     " AND pt.cantidad_maxima = gp.cantidad_maxima )"+cInter)
oQryNue:Refresh()
oBrwNue:Refresh()
RETURN nil
...
LOCAL oDlgA,oError,oQryArt,nArticu:=0,cNomArt:=SPACE(50),oGet1,oGet2,oGet3,oGet4,oGet5,lBorrar,;
      oBot:=ARRAY(2),acor:=ARRAY(4),lRta:=.f.,nPrecio:=0,nPrecioBase:=0, cSql, cInter:=""
