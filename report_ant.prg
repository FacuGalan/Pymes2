...
CLASS TReport
   ...
   DATA nMemoLines INIT 0  // used internally
   DATA nStretchCol INIT 0
   ...
   METHOD SayBitmap( nRow, nCol, cBitmap, nWidth, nHeight, nScale, nAlphaLevel )
   ...
   METHOD ToExcel( bProgress )
   METHOD xlSetFont( oRange, oFont )
   METHOD xlHeader( oPageSetup )
   METHOD xlFooter( oPageSetup )
...
METHOD New( aTitle, aHead, aFoot, aFont, aPen, lSummary, cRptFile, ;
   ...
   ::aGroups    := {}
   ::bStdFont   := {|| 1 }
   ::bPreview   := {|oDevice| RPreview( oDevice, Self ) }
   ::cRptFile   := cRptFile
   ::cResName   := cResName
   ::cPageTotal   := ""
   ::cGrandTotal  := ""
   ...
   */
   ...
   elseif ::lScreen
      ...
      ::oDevice := TPrinter():New( cName, .f., .t., nil, .t. )
   elseif ! Empty( ::cFile )
...
METHOD SayBitmap( nRow, nCol, cBitmap, nWidth, nHeight, nScale, nAlphaLevel ) CLASS TReport
...
*/
   ...
   ::oDevice:SayImage( aOffset[ 1 ], aOffset[ 2 ],;
                        cBitmap, nPixWidth, nPixHeight,, .f.,  nAlphaLevel )
   ...
   while ! ::lBreak .and. Eval( ::bWhile )
      ...
      */
      ...
      for nFor2 := 1 to nColumns
         if ( nFor1 := ::aColumns[ nFor2 ]:LineCount() ) > ::nMemoLines
            ::nMemoLines   := nFor1       // nFor1 is used as temp variable
         endif
      next nFor2

      for nFor1 := 1 to Max( ::nMaxData, ::nMemoLines )
          ...
          if ! ::lSummary
             ...
             ! ::lFirstRow .and. ;
             nFor1 == 1 )
             ::StartLine( ::aDataHeight[ Min( nFor1, ::nMaxData ) ], lSeparator )
          endif
          ...
          if ! ::lSummary
             ::EndLine( ::aDataHeight[ Min( nFor1, ::nMaxData ) ] )
          endif
...
METHOD Stabilize() CLASS TReport
   ...
   */
   ...
   if !Empty( ::nStretchCol ) .and. ( ::lScreen .or. ::lPrinter )

      nFor1          := ::aColumns[ ::nStretchCol ]:nWidth  // nFor1 is a  temporary holder

      if ::nRptWidth < ( ::nWidth - ::nLeftMargin - ::nRightMargin )

         ::aColumns[ ::nStretchCol ]:nWidth   += ;
            ( ::nWidth - ::nLeftMargin - ::nRightMargin - ::nRptWidth )
         if ::aColumns[ ::nStretchCol ]:lMemo
             ::aColumns[ ::nStretchCol ]:nSize := Int( ;
               ::aColumns[ ::nStretchCol ]:nWidth * ;
               ::aColumns[ ::nStretchCol ]:nSize / nFor1 )
         ...
         nFor2 := ::nRptWidth - ( ::nWidth - ::nLeftMargin - ::nRightMargin )
         ::aColumns[ ::nStretchCol ]:nWidth -= Min( nFor2, 0.75 * nFor1 )
         if ::aColumns[ ::nStretchCol ]:lMemo
             ::aColumns[ ::nStretchCol ]:nSize := Int( ;
               ::aColumns[ ::nStretchCol ]:nWidth * ;
               ::aColumns[ ::nStretchCol ]:nSize / nFor1 )
                  ...
                  if aGrpVal[ nGrp ] != nil
                     ...
                     for nCol := 1 to nCols
                        ...
                        if oCol:lTotal
                           ...
                           oSheet:Cells( nRow, nCol ):FormulaLocal := "=SUBTOTALES(9;" + cAdr + ")"
      ...
      for nCol := 1 to nCols
         ...
         if oCol:lTotal
            WITH OBJECT oSheet:Cells( nRow, nCol )
               oSheet:Cells( nRow, nCol ):FormulaLocal := "=SUBTOTALES( 9; " + ;
