...
LOCAL oBar, hHand, oGetSeek, cSeek := SPACE(50), oPop2, oPop3, oPop4, oPopInfo
cVentana := PROCNAME()
...
ENDMENU
MENU oPop4 POPUP
     MENUITEM "Listar recetas"  ACTION (RepRecetas(),oGetSeek:SetFocus())
     MENUITEM "Duplicar Receta" ACTION DuplicarReceta()
ENDMENU

MENU oPopInfo POPUP
     MENUITEM "Precios"  ACTION (IF(oQryBrw:nRecCount>0,PreciosArt(oQryBrw:codigo),nil),oGetSeek:SetFocus())     
     MENUITEM "Info"     ACTION (Info(),oGetSeek:SetFocus())
     MENUITEM "Precios Posdatados" ACTION PrePos(cPermisos)          
ENDMENU
  ...
  DEFINE WINDOW oWnd1 MDICHILD TITLE "A/B/M de Articulos: Solo articulos vendibles";
         ...
         DEFINE BUTTON RESOURCE "CONFIRMA" OF oBar ;
                ...
                ACTION IF(oQryBrw:nRecCount>0,PreciosArt(oQryBrw:codigo),nil);
                PROMPT "Precios" TOP WHEN("M"$cPermisos) MENU oPopInfo                        
         ...
         DEFINE BUTTON RESOURCE "PROD" OF oBar ;
            ...
            PROMPT "Lista Rec." TOP WHEN("R"$cPermisos)    MENU oPop4
...
DEFINE DIALOG oForm TITLE IF(lAlta,"Alta","Modificacion") + " de Articulo" RESOURCE "ABMART1" OF oWnd1
 ...
 REDEFINE GET oGet[01] VAR base:codigo PICTURE "99999999999999" ID 100 OF oForm WHEN(lAlta)
  ...
  REDEFINE GET oGet[04] VAR base:prov ID 104 OF oForm PICTURE "99999";
                VALID(BuscarArt(oQry2,oForm,oGet[4],oGet[5]));
                ACTION (oGet[04]:cText:= 0, BuscarArt(oQry2,oForm,oGet[4],oGet[5])) BITMAP "BUSC1"
  REDEFINE GET oGet[05] VAR cProve          ID 105 OF oForm WHEN (.F.)
  REDEFINE GET oGet[21] VAR base:marca      ID 122 OF oForm PICTURE "9999";
               VALID(BuscarArt(oQry4,oForm,oGet[21],oGet[22]));
               ACTION (oGet[21]:cText:= 0, BuscarArt(oQry4,oForm,oGet[21],oGet[22])) BITMAP "BUSC1"
  REDEFINE GET oGet[22] VAR cMarc           ID 123 OF oForm WHEN (.F.)
  REDEFINE GET oGet[23] VAR base:rubro      ID 124 OF oForm PICTURE "9999";
                VALID(BuscarArt(oQry5,oForm,oGet[23],oGet[24]) .and. (oGet[25]:cText:= oQry5:depto) <> 'XXX' .and. BuscarArt(oQry6,oForm,oGet[25],oGet[26]));
               ACTION (oGet[23]:cText:= 0, BuscarArt(oQry5,oForm,oGet[23],oGet[24]),oGet[25]:cText:= oQry5:depto,BuscarArt(oQry6,oForm,oGet[25],oGet[26])) BITMAP "BUSC1"
  REDEFINE GET oGet[24] VAR cRub            ID 125 OF oForm WHEN (.F.)
  REDEFINE GET oGet[27] VAR base:empresa    ID 126 OF oForm PICTURE "9999";
               VALID(BuscarArt(oQry7,oForm,oGet[27],oGet[28]));
               ACTION (oGet[27]:cText:= 0, BuscarArt(oQry7,oForm,oGet[27],oGet[28])) BITMAP "BUSC1"
  REDEFINE GET oGet[28] VAR cEmp            ID 127 OF oForm WHEN (.F.)
  REDEFINE GET oGet[25] VAR base:depto      ID 128 OF oForm PICTURE "9999";
               VALID(BuscarArt(oQry6,oForm,oGet[25],oGet[26]));
               ACTION (oGet[25]:cText:= 0, BuscarArt(oQry6,oForm,oGet[25],oGet[26])) BITMAP "BUSC1"
  ...
  REDEFINE GET oGet[14] VAR base:iva        ID 115 OF oForm PICTURE "99";
               VALID(BuscarArt(oQry3,oForm,oGet[14]) .and. ;             
               (oGet[15]:cText := STR(oQry3:tasa,6,2) + "%") <> "xxx");
               ACTION  (oGet[14]:cText:= 0, (BuscarArt(oQry3,oForm,oGet[14]) .and. ;             
  ...
  REDEFINE GET oGet[17] VAR base:porcentaje ID 118 OF oForm PICTURE "9999.99";
               VALID(CalcularPrecio(oGet,base,.t.))
  REDEFINE GET oGet[18] VAR base:porcentajerev  ID 119 OF oForm PICTURE "9999.99";
  ...
  REDEFINE GET oGet[32] VAR base:stockact   ID 133 OF oForm PICTURE "99999999.999"
...
FUNCTION Reseta(base,oForm,lReceta, mcodigo_duplica)
...
DEFAULT lReceta := .t., mcodigo_duplica := 0      
...
oApp:oServer:Execute("INSERT INTO reseta_temp (codart,detart,cantidad,punit,ptotal) "+;
                   "(SELECT r.codusa,a.nombre,r.cantidad,a.preciocos*if(a.endolares,d.dolar,1),(r.cantidad * a.preciocos*if(a.endolares,d.dolar,1)) "+;
                    ...
                    "WHERE r.codart = "+ClipValue2SQL(IF(mcodigo_duplica>0,mcodigo_duplica, base:codigo))+")")
oApp:oServer:NextResult()
...
oApp:oServer:Execute("INSERT INTO reseta_temp (codart,detart,cantidad,punit,ptotal) "+;
                   ...
                   ",a.preciocos*IF(a.endolares,d.dolar,1),"+ClipValue2Sql(oGet[4]:cText)+"*a.preciocos*IF(a.endolares,d.dolar,1) "+;                   
                    "FROM ge_"+oApp:cId+"articu a "+;
                    " LEFT JOIN ge_"+oApp:cId+"parametros d ON true  "+;
                    " WHERE a.codigo = "+ClipValue2Sql(oGet[2]:cText)+")")
CATCH oError
    ValidaError(oError)
END TRY
oQryRes:Refresh()
...
REDEFINE GET oGet[01] VAR base:codigo PICTURE "99999999999999" ID 100 OF oForm 
  ...
  REDEFINE BUTTON oBot[3] ID 4009 OF oForm ACTION (Reseta(base,oForm,.t.,oQryBrw:codigo),oGet[06]:Refresh()) WHEN (base:produccion)
  ...
  REDEFINE GET oGet[22] VAR cMarc           ID 123 OF oForm WHEN (.F.)
  REDEFINE GET oGet[23] VAR base:rubro      ID 124 OF oForm PICTURE "9999";
               VALID(BuscarArt(oQry5,oForm,oGet[23],oGet[24]));
               ACTION (oGet[23]:cText:= 0, BuscarArt(oQry5,oForm,oGet[23],oGet[24])) BITMAP "BUSC1"
  REDEFINE GET oGet[24] VAR cRub            ID 125 OF oForm WHEN (.F.)
  REDEFINE GET oGet[27] VAR base:empresa    ID 126 OF oForm PICTURE "9999";
               VALID(BuscarArt(oQry7,oForm,oGet[27],oGet[28]));
               ACTION (oGet[27]:cText:= 0, BuscarArt(oQry7,oForm,oGet[27],oGet[28])) BITMAP "BUSC1"
  ...
  REDEFINE BUTTON oBot[8] ID 4015 OF oForm ACTION (Reseta(base,oForm,.f.,oQryBrw:codigo)) WHEN (.f.)
...
*************************************************************************************************************
********** Validacion de la tabla de promociones (crea la tabla o agrega la columna prioridad segun el caso)
STATIC FUNCTION ValidarPromosArt()
...
LOCAL oDlgH, oBot := ARRAY(2), oBrwH, oQryHist, oGet1, oGet2, aCor, lRta := .f.
...
DEFINE DIALOG oDlgH TITLE "Historial de precios" FROM 05,15 TO 23,82;
   ...
   PintaBrw(oBrwH,0) 
   @ acor[1],acor[2] BUTTON oBot[1] PROMPT "&Exportar" OF oDlgH SIZE 30,10 ;
           ACTION (oBrwH:ToExcel()) PIXEL WHEN(oQryHist:nRecCount()>0)
   @ acor[3],acor[4] BUTTON oBot[2] PROMPT "&Salir" OF oDlgH SIZE 30,10 ;
           ACTION ((lRta := .f.), oDlgH:End() ) PIXEL CANCEL
...
DEFINE DIALOG oFilt TITLE "Filtrado de Articulo" RESOURCE "ABMART1" OF oWnd1
 oFilt:lHelpIcon := .f.
  ...
  REDEFINE GET oGet[04] VAR vprov ID 104 OF oFilt ;
               VALID(EMPTY(oGet[04]:cText) .or. BuscarArt(oQry2,oFilt,oGet[04],oGet[05]));
               ACTION (oGet[04]:cText:= 0, BuscarArt(oQry2,oFilt,oGet[04],oGet[05])) BITMAP "BUSC1"
  REDEFINE GET oGet[05] VAR cProve          ID 105 OF oFilt WHEN (.F.)
  REDEFINE GET oGet[21] VAR vmarca ID 122 OF oFilt ;
               VALID(EMPTY(oGet[21]:cText) .or. BuscarArt(oQry4,oFilt,oGet[21],oGet[22]));
               ACTION (oGet[21]:cText:= 0, BuscarArt(oQry4,oFilt,oGet[21],oGet[22])) BITMAP "BUSC1"
  REDEFINE GET oGet[22] VAR cMarc      ID 123 OF oFilt WHEN (.F.)
  REDEFINE GET oGet[23] VAR vrubro ID 124 OF oFilt ;
               VALID(EMPTY(oGet[23]:cText) .or. BuscarArt(oQry5,oFilt,oGet[23],oGet[24]));
               ACTION (oGet[23]:cText:= 0, BuscarArt(oQry5,oFilt,oGet[23],oGet[24])) BITMAP "BUSC1"
  REDEFINE GET oGet[24] VAR cRub ID 125 OF oFilt WHEN (.F.)
  REDEFINE GET oGet[27] VAR vempresa ID 126 OF oFilt ;
               VALID(EMPTY(oGet[27]:cText) .or. BuscarArt(oQry7,oFilt,oGet[27],oGet[28]));
               ACTION (oGet[27]:cText:= 0, BuscarArt(oQry7,oFilt,oGet[27],oGet[28])) BITMAP "BUSC1"
  REDEFINE GET oGet[28] VAR cEmp ID 127 OF oFilt WHEN (.F.)
  REDEFINE GET oGet[25] VAR vdepto ID 128 OF oFilt ;
               VALID(EMPTY(oGet[25]:cText) .or. BuscarArt(oQry6,oFilt,oGet[25],oGet[26]));
               ACTION (oGet[25]:cText:= 0, BuscarArt(oQry6,oFilt,oGet[25],oGet[26])) BITMAP "BUSC1"
  ...
  REDEFINE GET oGet[14] VAR viva ID 115 OF oFilt ;
              VALID(EMPTY(oGet[14]:cText) .or. (BuscarArt(oQry3,oFilt,oGet[14],oGet[15]) .and. ;             
               ((oGet[15]:cText := STR(oQry3:tasa,6,2) + "%") <> "xxx"))  );
               ACTION (oGet[14]:cText:= 0, BuscarArt(oQry3,oFilt,oGet[14],oGet[15]),;             
               (oGet[15]:cText := STR(oQry3:tasa,6,2) + "%") <> "xxx" ) BITMAP "BUSC1"
...
DEFINE DIALOG oForm TITLE "Alta de Rubros";
   ...
   @ 35, 60 GET oGet[3] VAR base:depto  PICTURE "9999" RIGHT OF oForm SIZE 25,12 PIXEL;
                VALID(BuscarArt(oQry6,oForm,oGet[03],oGet[04]));
                ACTION (oGet[03]:cText:= 0, BuscarArt(oQry6,oForm,oGet[03],oGet[04])) BITMAP "BUSC1" 
...
GROUP oGru ON oQry:codart HEADER "Para "+STR(oQry:cantprod,14)+" "+ ALLTRIM(oQry:nombre) + " ("+STR(oQry:codart)+")" FOOTER "Totales"  FONT 2   
  ...
  COLUMN TITLE "Cantidad"       DATA oQry:cantidad   PICTURE "999999.999" SIZE 08 FONT 1 
...
******************************************
** Duplicar Recetas
STATIC FUNCTION DuplicarReceta()
LOCAL mcodigo := 0, base, oQry, mcant
LOCAL oDlgH, acor:= ARRAY(4), mrta:=.F., oBot1, oBot2,;
      i, oError, cSql, oGet1, oGet2       
oQry:= oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"articu WHERE codigo = " + ClipValue2Sql(oQryBrw:codigo))
IF !oQry:produccion .and. !oQry:stockotro
   MsgInfo("Ese producto no es produccion ni stock de otro","Atencion")
   RETURN nil 
ENDIF   
DO WHILE .T.
    DEFINE DIALOG oDlgH TITLE "Duplicar Receta de Articulos" FROM 03,15 TO 10,65
       acor := AcepCanc(oDlgH)    
       @ 07,05 SAY "Codigo a Copiar:" OF oDlgH SIZE 60,12 PIXEL RIGHT  
       @ 05,70 GET oGet2 VAR mcodigo        OF oDlgH PIXEL PICTURE "99999999999999" RIGHT
       @ acor[1],acor[2] BUTTON oBot1 PROMPT "&Buscar" OF oDlgH SIZE 30,10 ;
               ACTION ((mrta := .t.), oDlgH:End() ) PIXEL
       @ acor[3],acor[4] BUTTON oBot2 PROMPT "&Cancelar" OF oDlgH SIZE 30,10 ;
               ACTION ((mrta := .f.), oDlgH:End() ) PIXEL CANCEL
    ...
    IF !mrta
       RETURN nil 
    ENDIF   
    IF oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"reseta WHERE codart = "+str(mcodigo)):nRecCount = 0
       MsgInfo("Ese producto no tiene receta","Atencion")
       LOOP
    ENDIF
    mcant := oApp:oServer:Query("SELECT cantprod FROM ge_"+oApp:cId+"articu WHERE codigo = "+str(mcodigo)):cantprod
    EXIT 
ENDDO
oQry:= oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"articu WHERE codigo = " + ClipValue2Sql(oQryBrw:codigo))
base := oQry:GetRowObj()
base:cantprod := mcant
IF Reseta(base,oWnd1,base:produccion,mcodigo)   
   oQry:cantprod := mcant
   oQry:Save()
ENDIF   
RETURN NIL
...
*************************************************************************************************************
********** PRECIOS DEL ARTICULO: edicion de precios + promos + listas de precios especiales
STATIC FUNCTION PreciosArt(nCodigo)
LOCAL oForm, oGet := ARRAY(54), oBot := ARRAY(4), base, oError, lRta := .f.,;
      nNeto := 0, cIvaNom := SPACE(30), nPrecioven, nPreciocos, lActualizaGrupo := .f.,;
      oQryPro, oBrwPro, oQryLis, oBrwLis, oFont, aCor
IF EMPTY(nCodigo) .or. nCodigo = 0
   RETURN nil
ENDIF
oQry := oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"articu WHERE codigo = "+ClipValue2Sql(nCodigo))
IF oQry:nRecCount = 0
   RETURN nil
ENDIF
base := oQry:GetRowObj()
...
oQry3 := oApp:oServer:Query("SELECT codigo,nombre,tasa FROM ge_"+oApp:cId+"ivas WHERE codigo = "+str(base:iva))
cIvaNom := STR(oQry3:tasa,6,2) + "%"
oQryPro := oApp:oServer:Query("SELECT id,nompromo,tipo,precio_especial,precio_unitario,fecha_inicio,fecha_fin,formapago "+;
           "FROM ge_"+oApp:cId+"promociones WHERE codart = "+ClipValue2Sql(nCodigo)+" ORDER BY nompromo")
oQryLis := oApp:oServer:Query("SELECT l.codigo AS codlis,l.nombre,d.numpre,d.porc,d.precio "+;
           "FROM ge_"+oApp:cId+"lispredet d JOIN ge_"+oApp:cId+"lispre l ON l.codigo = d.codlis "+;
           "WHERE d.codart = "+ClipValue2Sql(nCodigo)+" ORDER BY l.nombre")
DEFINE FONT oFont NAME "TAHOMA" SIZE 0,-11.5
DO WHILE .T.
DEFINE DIALOG oForm TITLE "Precios del articulo: "+ALLTRIM(STR(base:codigo))+" - "+ALLTRIM(base:nombre);
       FROM 05,15 TO 40,110 OF oWnd1 FONT oFont
   acor := AcepCanc(oForm)
   oForm:lHelpIcon := .f.
   @ 07,005 SAY "Costo s/IVA:"   OF oForm PIXEL SIZE 65,12 RIGHT
   @ 05,080 GET oGet[06] VAR base:precossiva PICTURE "999999999.99" OF oForm PIXEL SIZE 35,12 RIGHT
   @ 05,200 CHECKBOX oGet[46] VAR base:siniva PROMPT "Sin IVA" OF oForm PIXEL SIZE 75,12

   @ 22,005 SAY "Descuentos %:"  OF oForm PIXEL SIZE 65,12 RIGHT
   @ 20,080 GET oGet[07] VAR base:desc1 PICTURE "999.99" OF oForm PIXEL SIZE 18,12 RIGHT
   @ 20,100 GET oGet[08] VAR base:desc2 PICTURE "999.99" OF oForm PIXEL SIZE 18,12 RIGHT
   @ 20,120 GET oGet[09] VAR base:desc3 PICTURE "999.99" OF oForm PIXEL SIZE 18,12 RIGHT
   @ 20,140 GET oGet[10] VAR base:desc4 PICTURE "999.99" OF oForm PIXEL SIZE 18,12 RIGHT
   @ 20,160 GET oGet[11] VAR base:desc5 PICTURE "999.99" OF oForm PIXEL SIZE 18,12 RIGHT

   @ 37,005 SAY "Neto:"          OF oForm PIXEL SIZE 65,12 RIGHT
   @ 35,080 GET oGet[39] VAR nNeto PICTURE "999999999.99" OF oForm PIXEL SIZE 35,12 RIGHT;
            WHEN(CalcularNeto(base,oGet))
   @ 37,170 SAY "Imp.Int:"       OF oForm PIXEL SIZE 40,12 RIGHT
   @ 35,215 GET oGet[12] VAR base:impint PICTURE "99999.99" OF oForm PIXEL SIZE 22,12 RIGHT
   @ 37,260 SAY "Flete %:"       OF oForm PIXEL SIZE 40,12 RIGHT
   @ 35,305 GET oGet[13] VAR base:flete PICTURE "99999.9" OF oForm PIXEL SIZE 20,12 RIGHT

   @ 52,005 SAY "IVA:"           OF oForm PIXEL SIZE 65,12 RIGHT
   @ 50,080 GET oGet[14] VAR base:iva PICTURE "99" OF oForm PIXEL SIZE 25,12 RIGHT;
            VALID(BuscarArt(oQry3,oForm,oGet[14]) .and. (oGet[15]:cText := STR(oQry3:tasa,6,2)+"%") <> "xxx");
            ACTION (oGet[14]:cText:= 0, (BuscarArt(oQry3,oForm,oGet[14]) .and. (oGet[15]:cText := STR(oQry3:tasa,6,2)+"%") <> "xxx")) BITMAP "BUSC1"
   @ 50,130 GET oGet[15] VAR cIvaNom OF oForm PIXEL SIZE 30,12 WHEN(.f.)
   @ 52,195 SAY "P.Costo:"       OF oForm PIXEL SIZE 45,12 RIGHT
   @ 50,245 GET oGet[16] VAR base:preciocos PICTURE "999999999.99" OF oForm PIXEL SIZE 35,12 RIGHT;
            WHEN((oGet[16]:cText:= nNeto + base:impint + nNeto * (base:flete/100) + ((nNeto * oQry3:tasa )/100)) = "-999999")

   @ 67,005 SAY "% Venta:"       OF oForm PIXEL SIZE 65,12 RIGHT
   @ 65,080 GET oGet[17] VAR base:porcentaje PICTURE "9999.99" OF oForm PIXEL SIZE 22,12 RIGHT;
            VALID(CalcularPrecio(oGet,base,.t.))
   @ 67,150 SAY "% Reventa:"     OF oForm PIXEL SIZE 50,12 RIGHT
   @ 65,205 GET oGet[18] VAR base:porcentajerev PICTURE "9999.99" OF oForm PIXEL SIZE 22,12 RIGHT;
            VALID(CalcularPrecio(oGet,base,.f.))

   @ 82,005 SAY "P.Venta 1:"     OF oForm PIXEL SIZE 65,12 RIGHT
   @ 80,080 GET oGet[20] VAR base:precioven PICTURE "999999999.999" OF oForm PIXEL SIZE 35,12 RIGHT
   @ 82,170 SAY "P.Venta 2:"     OF oForm PIXEL SIZE 50,12 RIGHT
   @ 80,225 GET oGet[19] VAR base:reventa   PICTURE "999999999.999" OF oForm PIXEL SIZE 35,12 RIGHT

   @ 100,005 SAY "Promociones del articulo (doble clic o boton Modif):" OF oForm PIXEL SIZE 300,12
   @ 112,005 XBROWSE oBrwPro DATASOURCE oQryPro ;
           COLUMNS "id","nompromo","tipo","precio_especial","precio_unitario","fecha_inicio","fecha_fin","formapago";
           HEADERS "#","Promo","Tipo","P.Especial","P.Unitario","Desde","Hasta","Formas pago";
           SIZES 22,140,28,70,70,60,60,80 OF oForm SIZE 310,55 PIXEL
   oBrwPro:CreateFromCode()
   PintaBrw(oBrwPro,0)
   oBrwPro:bLDblClick := { || IF(oQryPro:nRecCount > 0,(ModifPromoArt(oQryPro:id),oQryPro:Refresh(),oBrwPro:Refresh()),) }
   @ 112,320 BUTTON oBot[3] PROMPT "Modif." OF oForm SIZE 30,14 PIXEL;
             ACTION (IF(oQryPro:nRecCount > 0,(ModifPromoArt(oQryPro:id),oQryPro:Refresh(),oBrwPro:Refresh()),))

   @ 175,005 SAY "Listas de precios especiales (doble clic o boton Modif):" OF oForm PIXEL SIZE 300,12
   @ 187,005 XBROWSE oBrwLis DATASOURCE oQryLis ;
           COLUMNS "codlis","nombre","numpre","porc","precio";
           HEADERS "#Lista","Lista","N.Precio","%","Precio";
           SIZES 40,200,55,50,75 OF oForm SIZE 310,50 PIXEL
   oBrwLis:CreateFromCode()
   PintaBrw(oBrwLis,0)
   oBrwLis:bLDblClick := { || IF(oQryLis:nRecCount > 0,(ModifListaArt(oQryLis:codlis,nCodigo),oQryLis:Refresh(),oBrwLis:Refresh()),) }
   @ 187,320 BUTTON oBot[4] PROMPT "Modif." OF oForm SIZE 30,14 PIXEL;
             ACTION (IF(oQryLis:nRecCount > 0,(ModifListaArt(oQryLis:codlis,nCodigo),oQryLis:Refresh(),oBrwLis:Refresh()),))
...
*************************************************************************************************************
********** EDITOR COMPLETO DE UNA PROMO EXISTENTE (desde Precios del articulo)
STATIC FUNCTION ModifPromoArt(nId)
LOCAL oGet := ARRAY(25), oBot := ARRAY(2), oQryP, oQryA, oForm, lRta := .f., aCor, base, oError,;
      oFont, aTipo:={"Precio Especial","Lleva n paga m","Descuento unidad N","Precio x cantidad"},;
      cNombre := SPACE(50), aForma := {.T.,.T.,.T.,.T.,.T.,.T.}, aRelacion := {"Lista 1","Lista 2","P.Costo"}
oQryP := oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"promociones WHERE id = "+ClipValue2Sql(nId))
IF oQryP:nRecCount = 0
   RETURN nil
ENDIF
base := oQryP:GetRowObj()
oQryA := oApp:oServer:Query("SELECT nombre,precioven,reventa,preciocos FROM ge_"+oApp:cId+"articu WHERE codigo = "+ClipValue2Sql(base:codart))
cNombre := oQryA:nombre
...
DEFINE FONT oFont NAME "TAHOMA" SIZE 0,-11.5
DO WHILE .T.
DEFINE DIALOG oForm TITLE "Modificacion de Promocion" FROM 05,15 TO 33,130 OF oWnd1
   oForm:lHelpIcon := .f.
   ...
   @ 67,260 SAY "Prioridad:"             OF oForm PIXEL SIZE 50,12 RIGHT 
   @ 82, 05 SAY "Promo Precio Especial"  OF oForm PIXEL SIZE 125,12 CENTER
   @ 82,105 SAY "Promo lleva n paga m"   OF oForm PIXEL SIZE 125,12 CENTER
   @ 82,205 SAY "Promo Desc. n unidad"   OF oForm PIXEL SIZE 125,12 CENTER
   @ 82,305 SAY "Promo por cantidad"     OF oForm PIXEL SIZE 125,12 CENTER
   ...
   @142,005 SAY "Aplicar a Forma de pago:" OF oForm PIXEL SIZE 90,12 RIGHT

   @ 05, 80 GET oGet[1] VAR base:codart PICTURE "99999999999999" RIGHT OF oForm SIZE 60,12 PIXEL WHEN(.f.)
   ...
   @ 67,315 GET oGet[24] VAR base:prioridad PICTURE "9999" RIGHT OF oForm PIXEL
   ...
   IF MsgYesNo("Desea recalcular el precio en base al calculo porcentual?","Atencion")
      IF base:tipo = 1
         IF base:base_calculo = 1 
            base:precio_especial := oQryA:precioven * (1+base:base_porcentual/100)
            ELSE
            IF base:base_calculo = 2
               base:precio_especial := oQryA:reventa * (1+base:base_porcentual/100)  
               ELSE 
               base:precio_especial := oQryA:preciocos * (1+base:base_porcentual/100)
            ENDIF
         ENDIF                          
         ELSE
         IF base:base_calculo = 1 
            base:precio_unitario := oQryA:precioven * (1+base:base_porcentual/100)
            ELSE
            IF base:base_calculo = 2
               base:precio_unitario := oQryA:reventa * (1+base:base_porcentual/100)  
               ELSE 
               base:precio_unitario := oQryA:preciocos * (1+base:base_porcentual/100)
            ENDIF
         ENDIF
      ENDIF
   ENDIF
...
*************************************************************************************************************
********** MODIFICAR EL PRECIO ESPECIAL DEL ARTICULO EN UNA LISTA DE PRECIOS
STATIC FUNCTION ModifListaArt(nCodlis, nCodart)
LOCAL oQryD, oForm, oGet := ARRAY(3), oBot := ARRAY(2), base, lRta := .f., oError, oFont, cNomLista,;
      aListas:={"Lista 1","Lista 2","Precio de costo","Costo Sin IVA"}, aCor, oQryA
oQryD := oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"lispredet WHERE codlis = "+ClipValue2Sql(nCodlis)+;
                            " AND codart = "+ClipValue2Sql(nCodart))
oQryA := oApp:oServer:Query("SELECT nombre,precioven,reventa,preciocos,precossiva FROM ge_"+oApp:cId+"articu WHERE codigo = "+ClipValue2Sql(nCodart))
IF oQryD:nRecCount = 0
   RETURN nil
ENDIF
base := oQryD:GetRowObj()
cNomLista := oApp:oServer:Query("SELECT nombre FROM ge_"+oApp:cId+"lispre WHERE codigo = "+ClipValue2Sql(nCodlis)):nombre
DEFINE FONT oFont NAME "TAHOMA" SIZE 0,-11.5
DO WHILE .T.
DEFINE DIALOG oForm TITLE "Precio en lista: "+ALLTRIM(cNomLista) FROM 08,18 TO 18,60 OF oWnd1 FONT oFont
   acor := AcepCanc(oForm)
   oForm:lHelpIcon := .f.
   @ 07,005 SAY "Lista base:" OF oForm PIXEL SIZE 70,12 RIGHT
   @ 05,080 COMBOBOX oGet[1] VAR base:numpre ITEMS aListas OF oForm PIXEL SIZE 70,12
   @ 22,005 SAY "% Sobre lista:"   OF oForm PIXEL SIZE 70,12 RIGHT
   @ 20,080 GET oGet[2] VAR base:porc PICTURE "9999.99" OF oForm PIXEL SIZE 22,12 RIGHT
   @ 37,005 SAY "Precio:"         OF oForm PIXEL SIZE 70,12 RIGHT
   @ 35,080 GET oGet[3] VAR base:precio PICTURE "999999999.999" OF oForm PIXEL SIZE 35,12 RIGHT
   @ acor[1],acor[2] BUTTON oBot[1] PROMPT "&Grabar" OF oForm SIZE 30,10 ;
           ACTION ((lRta := .t.), oForm:End() ) PIXEL
   @ acor[3],acor[4] BUTTON oBot[2] PROMPT "&Cancelar" OF oForm SIZE 30,10 ;
           ACTION ((lRta := .f.), oForm:End() ) PIXEL CANCEL   
...
IF MsgYesNo("Desea recalcular el precio en base al calculo porcentual?","Atencion")
  DO CASE 
     CASE base:numpre = 1
          base:precio := oQryA:precioven * (1+base:porc/100)
     CASE base:numpre = 2
          base:precio := oQryA:reventa * (1+base:porc/100)
     CASE base:numpre = 3
          base:precio := oQryA:preciocos * (1+base:porc/100)
     CASE base:numpre = 4
          base:precio := oQryA:precossiva * (1+base:porc/100) 
  ENDCASE         
ENDIF
oQryD:oRow := base
TRY
