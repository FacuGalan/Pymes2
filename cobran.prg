...
STATIC oGet, oDlg, oBot, mfecha, manticipo, mcodcli, mentrega, ;
       ...
       oQryDeu, oBrwD, nCot,; // BROWSE DE FACTURAS
...
oApp:oServer:Execute("CREATE TEMPORARY TABLE IF NOT EXISTS `tranci_pagos` ("+;
                           ...
                           "`saldodolar` DECIMAL(12,4) DEFAULT 0 NOT NULL "+;
...
DO WHILE .T.
   ...
   DEFINE DIALOG oDlg RESOURCE cRecurso OF oApp:oWnd TITLE "Cobranza de cuentas corrientes"
    ...
    REDEFINE GET oGet[4] VAR mentrega  ID 116 OF oDlg PICTURE "9999999999.99" VALID(IF(lEdita,.t.,(Mostrar()=nil .and. oBrw:SetFocus()=nil)))
    REDEFINE GET oGet[5] VAR manticipo ID 117 OF oDlg PICTURE "9999999999.99" WHEN(.f.)    
    ...
    IF oApp:usar_cuotas
        ...
        REDEFINE XBROWSE oBrw DATASOURCE oQry;
                  COLUMNS "cuota","cantcuo","cuota","fecha","saldo","pagado","saldonue","Importe","aplicado","interes","saldodolar";
                  HEADERS "Cuota","De"," ","Fecha","Saldo","Pagado","Nuevo Saldo","Origen","Aplicado","Interes","Saldo u$s" FOOTERS;
                  SIZES 35,30,95,75,75,75,75,75,75,75,75 ID 20 OF oDlg 
        oBrw:aCols[3]:Hide()
        oBrw:aCols[10]:Hide()
        ...
        ELSE 
        REDEFINE XBROWSE oBrw DATASOURCE oQry;
                  COLUMNS "tipo","numcomp","cuota","fecha","saldo","pagado","saldonue","Importe","aplicado","interes","saldodolar";
                  HEADERS "Tip","Nro comprobante"," ","Fecha","Saldo","Pagado","Nuevo Saldo","Origen","Aplicado","Interes","Saldo u$s" FOOTERS;
    ...
    *oBrwCheT:aCols[1]:nEditType := 1
    ...
    REDEFINE GET oGet[09] VAR nTarjeta       ID 4004 OF oDlg PICTURE "999999999.99" 
    REDEFINE GET oGet[08] VAR nCheqT         ID 202  OF oDlg PICTURE "999999999.99" WHEN(.F.)
    REDEFINE GET oGet[12] VAR nAnticipo      ID 203  OF oDlg PICTURE "999999999.99";
                 VALID(nAnticipo <= manticipo );
                 WHEN(manticipo>0)
    REDEFINE GET oGet[20] VAR nMPago         ID 4005 OF oDlg PICTURE "999999999.99" 
    ...
    REDEFINE GET oGet[14] VAR nFaltante ID 121 OF oDlg COLOR CLR_RED , CLR_YELLOW FONT oFont PICTURE "9999999999.99";
                 WHEN(CalcTot())
    REDEFINE GET oGet[15] VAR nTotal ID 122 OF oDlg COLOR CLR_RED , CLR_YELLOW FONT oFont PICTURE "9999999999.99";
                 WHEN(CalcTot())
    REDEFINE GET oGet[17] VAR nTotal1 ID 123 OF oDlg COLOR CLR_RED , CLR_YELLOW FONT oFont PICTURE "9999999999.99";
                 WHEN(CalcTot())
     ...
     IF oApp:usar_cuotas
         DO WHILE !oQry:Eof()
            IF oQry:pagado <> 0
               ...
               IF oQry:pagado <> 0 .and. oQry:saldodolar <> 0
                  oApp:oServer:Execute("UPDATE ge_"+oApp:cId+"ventas_cuota SET saldodolar = " + str(ABS(oQry:saldonue/nCot),12,2) +" "+ ;
                                    " WHERE tipo = " + ClipValue2Sql(oQryDeu:ticomp) +;
                                    " AND   letra    = " + ClipValue2Sql(oQryDeu:letra) +;
                                    " AND   numero   = " + ClipValue2Sql(oQryDeu:numcomp) +;
                                    " AND   cuota   = " + ClipValue2Sql(oQry:cuota) +;
         ...
         DO WHILE !oQry:Eof()
            oApp:oServer:Execute("UPDATE ge_"+oApp:cId+"ventas_cuota SET saldo = " + ClipValue2Sql(ABS(oQry:saldonue)) +" "+ ;
                                    ...
                                    " AND   cliente   = " + ClipValue2Sql(mcodcli) )  
            IF oQry:pagado <> 0 .and. oQry:saldodolar <> 0
                  oApp:oServer:Execute("UPDATE ge_"+oApp:cId+"ventas_cuota SET saldodolar = " + str(ABS(oQry:saldonue)/nCot,12,4) +" "+ ;
                                    " WHERE tipo = " + ClipValue2Sql(oQry:tipo) +;
                                    " AND   letra    = " + ClipValue2Sql(LEFT(oQry:numcomp,1)) +;
                                    " AND   numero   = " + ClipValue2Sql(RIGHT(oQry:numcomp,13)) +;
                                    " AND   cuota   = " + ClipValue2Sql(oQry:cuota) +;
...
LOCAL mentre := mentrega + nAnticipo, nCotiza, lRta := .f., oDlg2, aCor, oGet1 := ARRAY(1), oBot := ARRAY(2)
oQry:Zap()
oApp:oServer:Execute("INSERT INTO tranci_pagos (tipo,numcomp,cuota,cantcuo,fecha,saldo,pagado,saldonue,neto,importe,aplicado,interes,saldodolar) "+;
                     ...
                     " saldo * IF(tipo = 'NC', -1,1) * "+;
                     " ( SELECT (i.indice-1) "+;
                     " FROM ge_"+oApp:cId+"indexa i WHERE i.dias <   DATEDIFF(CURDATE() , v.fecvto)  ORDER BY dias DESC LIMIT 1), v.saldodolar "+;
...
//Me fijo si hay saldos en dolar
IF oApp:oServer:Query("SELECT tipo FROM tranci_pagos WHERE saldodolar > 0"):nRecCount > 0
    nCotiza := oApp:oServer:Query("SELECT dolar FROM ge_"+oApp:cId+"parametros"):dolar

    lRta := .T.
    DEFINE DIALOG oDlg2 TITLE "Cotizacion Dolar" FROM 05,15 TO 14,60 OF oDlg 
     acor := AcepCanc(oDlg2)
     @ 07, 05 SAY "Existen facturas con saldo en dolares. Por favor indique la cotización que quiere aplicar!" OF oDlg2 PIXEL SIZE 160,24 CENTER
     @ 37, 05 SAY "Precio por dolar:" OF oDlg2 PIXEL SIZE 60,12 RIGHT
    
     @ 35, 70 GET oGet1[1] VAR nCotiza  PICTURE "99999999.99" OF oDlg2 PIXEL SIZE 25,12 RIGHT;
                  VALID(nCotiza > 0 )      
     @ acor[1],acor[2] BUTTON oBot[1] PROMPT "&Aplicar" OF oDlg2 SIZE 30,10 ;
             ACTION ((lrta := .t.), oDlg2:End() ) PIXEL    
    ACTIVATE DIALOG oDlg2 CENTER ON INIT oGet1[1]:SetFocus()
    nCot := nCotiza
    oApp:oServer:Execute("UPDATE tranci_pagos SET saldo = saldodolar * "+STR(nCotiza)+" * IF(saldo>0,1,-1), "+;
                         "interes = saldo - saldonue, saldonue = saldo WHERE saldodolar <> 0")
ENDIF
oApp:oServer:NextResult()
oQry:Refresh()
DO WHILE !oQry:Eof()
