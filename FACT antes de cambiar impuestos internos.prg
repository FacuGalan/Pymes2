...
oApp:oServer:Execute("";
    ...
    +"`PUNIT` DECIMAL(10,3) DEFAULT '0.00', ";
    +"`NETO`  DECIMAL(10,3) DEFAULT '0.00', ";
    +"`DESCUENTO`  DECIMAL(10,3) DEFAULT '0.00', ";
    +"`STOTAL`  DECIMAL(10,3) DEFAULT '0.00', ";
    +"`IVA` DECIMAL(10,3) DEFAULT '0.00', ";
    +"`CODIVA` INT(2) DEFAULT '0', ";
    +"`PTOTAL` DECIMAL(10,3) DEFAULT '0.00',";
    +"`PCOSTO`  DECIMAL(10,3) DEFAULT '0.00', ";
...
oQryArt:= oApp:oServer:Query("SELECT codigo,nombre,precioven,reventa,stockact,stockmin FROM ge_"+oApp:cId+"articu")
lReemplaza:=.t.
...
DEFINE DIALOG oDlg1 RESOURCE "POS" OF oApp:oWnd TITLE "Facturacion punto de venta"
   ...
   */
   ...
   REDEFINE BTNBMP oBot[37] ID 308 OF oDlg1 2007 CENTER; //CALCULADORA
                   PROMPT "Departamen&tos";
   				         ACTION(FactDept(oDlg1,oFont2))
   ...
   REDEFINE XBROWSE oBrwDet DATASOURCE oQryDet;
            ...
            FOOTERS ;
            SIZES 40,243,65,64,55,55,55,64,55 ID 1000 OF oDlg1 WHEN(oQryDet:RecCount()>0)
   PintaBrw(oBrwDet,0)
...
/*
   ...
   oQryDet:total    := nVal * oQryDet:punitario
   oQryDet:neto     := oQryDet:total / 1.21
   oQryDet:iva      := oQryDet:total - oQryDet:neto
   oQryDet:Save()
   oQryDet:Refresh()
   oBrwDet:Refresh()
   oBrwDet:MakeTotals()
   ...
   IF n = 2
      ...
      oQryDet:neto     := oQryDet:ptotal / IF(oQryDet:codiva = 5,1.21,1)
      oQryDet:iva      := oQryDet:ptotal - oQryDet:neto
...
LOCAL nNumero,cNumComp,cLetra:=IF(oApp:tipo_iva<>6,IF(nCondicion>2,"B","A"),"C"),nPuntoVta,nCtaCte,nEfecti,;
      ...
      aTablaIva := {},oTabIva, cCae := "", dFecVtoc := DATE(),nTipFor:=0,dFecha := DATE()
...
cNumComp := STRTRAN(STR(nPuntoVta,4)+"-"+STR(nNumero,8)," ","0")
 ...
 oApp:oServer:BeginTransaction()
      ...
      base:fecha:= DATE()
      base:importe := nEfecti + nTarjet + nCheque + nMPago
      ...
      oApp:oServer:Execute("INSERT INTO ge_"+oApp:cId+"ventas_encab (ticomp,letra,numcomp,codcli,fecha,neto,iva,importe,tipopag,observa,"+;
                                                      "nombre,cuit,dni,direccion,localidad,usuario,fecmod,ip,coniva,condven,formapag,cae,fecvto,tipfor) VALUES "+;
...
*/
      ...
      oApp:oServer:Execute("UPDATE ge_"+oApp:cId+"articu a "+; 
						   ...
						   "SET a.stockact = a.stockact - v.suma ")
...
IF nCodArt >= 0
  ...
  IF oQryPun:pidestock = 2 .and. oQryAux:stockact-nCantidad < 0
     ...
     RETURN .F.
  ENDIF
  IF oQryPun:pidestock < 3 .and. oQryArt:stockmin > 0 .and. oQryArt:stockact-nCantidad  < oQryArt:stockmin
  ...
  ENDIF
   ...
   oApp:oServer:Execute("INSERT INTO VENTAS_DET_H1 (codart,detart,cantidad,punit,ptotal,neto,iva,codiva,descuento,stotal,pcosto) "+;
                         "VALUE ( "+ClipValue2Sql(nCodArt)+","+;
                                 ...
                                 ClipValue2Sql(oQryAux:preciocos*nCantidad)+;
                         " )" )	
ELSE
...
DEFINE DIALOG oDlgA TITLE "DEPARTAMENTOS" FROM 05,10 TO 22,110 OF oWnd FONT oFont
   ...
   @ 40, 90 GET oGet3 VAR nImporte PICTURE "9999999.99"  OF oDlgA PIXEL RIGHT SIZE 95,22
   
   @ acor[1],acor[2] BUTTON oBot1 PROMPT "&Ok" OF oDlgA SIZE 50,15 ;
           ACTION ((mrta := .t.), oDlgA:End() ) PIXEL
   @ acor[3],acor[4] BUTTON oBot2 PROMPT "&Sale" OF oDlgA SIZE 50,15 ;
           ACTION ((mrta := .f.), oDlgA:End() ) PIXEL CANCEL
ACTIVATE DIALOG oDlgA CENTER ON INIT oGet1:SetFocus()
IF !mRta
   RETURN nil
