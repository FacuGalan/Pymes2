...
STATIC oWnd1, nTipoEmp, nTipoFac, nTipoDoc, cVentana, oGet, oQryArt, oQryCli, oQryPar, oQryDet, oQryRub,;
       ...
       nBorrarPend,lUsoPedido,oQryLisPre,aFormaNom,aFormaInc,aFormaTip,aFormaCod,nFormaPago, lAcopio, nBorrarPresu,lPedidoUsado,;
       lFacturaM , lFacturaD, cOrigenNC, lMantener
...
lPermiteNC := oApp:oServer:Query( "SELECT * FROM ge_"+oApp:cId+"menu_nuevo WHERE (usuario="+;
        ClipValue2SQL(oApp:usuario)+ " AND modulo = 'NotaCred' AND permisos = 'ABMRE') "):nRecCount > 0      
...
ENDIF   
  ...
  DEFINE WINDOW oWnd1 MDICHILD TITLE "Facturacion por ventas" OF oApp:oWnd NOZOOM ICON oApp:oIco
    ...
    *oWnd1:bGotFocus := { || oDlg:SetFocus}
    ...
    REDEFINE CHECKBOX oGet[49] VAR lMantener   OF oDlg ID 4031
    ...
    REDEFINE COMBOBOX oGet[02] VAR nTipoDoc ITEMS aTipoDoc ID 177 OF oDlg;
             ON CHANGE(IF(oGet[2]:nAt = 3 .and. !lPermiteNC,(MsgInfo('Sin permisos para hacer NC','Atencion'),oGet[2]:Set(1), oGet[08]:SetFocus()), oGet[08]:SetFocus()))
    ...
    REDEFINE RADIO oGet[07] VAR nTipoCli ID 183, 184 OF oDlg WHEN lPedidoUsado=.f.;
             ON CHANGE IF(nTipoCli = 1, (nCodCli := 0, oGet[08]:SetFocus()), ;
                          (oGet[08]:cText := 0, oGet[09]:cText := SPACE(50), oSay1:SetText('B'),;
    ...
    REDEFINE GET oGet[31] VAR nDescuGen ID 308 OF oDlg PICTURE "999.99" WHEN(nTipoDoc = 5 .and. oApp:modifica_descu)
    ...
    REDEFINE GET oGet[38] VAR nIngBru   ID 4014 OF oDlg PICTURE "99999999999.99" WHEN(.F.)
    //neto y IVA y total
    REDEFINE GET oGet[19] VAR nNeto  ID 198 OF oDlg PICTURE "9999999999.99" WHEN(.F.)
    REDEFINE GET oGet[20] VAR nIVA   ID 199 OF oDlg PICTURE "9999999999.99" WHEN(.F.)
    REDEFINE GET oGet[21] VAR nTotal ID 200 OF oDlg COLOR CLR_RED , CLR_YELLOW READONLY;
           PICTURE "99999999999.99" FONT oFontTotal WHEN(.F.)
    REDEFINE GET oGet[42] VAR nImpInt ID 4019 OF oDlg PICTURE "9999999999.99" WHEN(.F.)    
    //GRILLA ---------------------------------------------------------------      
    ...
    ID 197 OF oDlg AUTOSORT WHEN(oQryDet:RecCount()>0)
    oBrw:nFreeze := 3
    PintaBrw(oBrw,0)
    ...
    IF oApp:modifica_precios .or. lAdmin
    ...
    // Ver Historial
    REDEFINE BUTTON oBot[6] ID 4021 ACTION VerHistorial(nCodCli,oQryDet,oBrw,nTipoDoc) WHEN(nCodCli > 0)
    //Vales 
...
ENDIF
    ...
    IF !EMPTY(oQryRem:vendedor)
        oGet[36]:Set(ALLTRIM(oQryRem:vendedor))
    ENDIF    
...
LOCAL base := oQryDet:GetRowObj(),nPrecio2,nDescuento,nPrecio1,nIvaPes,nSubTotal,nTotalcIva,;
      oQryIva,nCodIva1,nIvaCalc,nII,oQryPrecio,oQryArt, ;
      nCantIni := n, nPunitIni := nPunit
DEFAULT n := base:cantidad,nPorc:= base:descup,nPUnit:= base:punit, cNombre:= base:detart,lPedido:=.f.
nII := IF(!empty(base:cantidad),base:impint/base:cantidad,0)
IF nCantIni <> nil
   IF !ValidarCantidad(oQryDet:codart, nCantIni-oQryDet:cantidad)
      RETURN nil 
   ENDIF 
ENDIF   
IF nPunitIni <> nil
   IF !ValidarCosto(oQryDet:pcosto, nPunitIni - nPunitIni * (nPorc/100))
      RETURN nil 
   ENDIF 
ENDIF   
...
LOCAL oGet1, oGet2, oGet3, oGet4, oBot1, oBot2, oDlg1, oFont, mnumfac, ;
      ...
      mnumero := 0,oQryFac, oQryPend,oBrwPend, oQryUlt, oBrwB
DEFINE FONT oFont NAME "TAHOMA" SIZE 0,-11.5   

IF nTipoDoc = 3   
    oQryUlt := oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"ventas_encab WHERE ticomp IN ('FC','FR') ORDER BY fecha DESC,hora DESC LIMIT 30")
...
LOCAL oQryIva, nTotalcIva, nIvaPes, nCodIva1, nSubTotal, nDescuento,;
      i,nCantidad,nPrecio1,nPrecio2,lRta:=.f.,nIvaCalc:=0,nDescMar, nPunit, oQryS

IF nTiArt = 1
  IF !ValidarCantidad(oQryArt:codigo,oGet[16]:value)
      RETURN .f.
  ENDIF    
  /* IF oQryPun:pidestock = 1 .and. oQryArt:stockact-oGet[16]:value < 0 .and. nTipoDoc <> 3 .and. !oQryArt:stockotro
  ...
  IF oQryPun:pidestock = 1 .and. oQryArt:stockotro 
      ...
      ENDIF    
  ENDIF
  IF oQryPun:pidestock = 2 .and. oQryArt:stockact-oGet[16]:value < 0 .and. nTipoDoc <> 3 .and. !oQryArt:stockotro
  ...
  ENDIF*/
   ...
   IF !ValidarCosto(oQryArt:preciocos,nTotalcIva/oGet[16]:value)
      RETURN nil 
   ENDIF   
   /*IF oQryPun:validacosto = 1
      IF oQryArt:preciocos > nTotalcIva / oGet[16]:value
          lRta:=MsgYesNo("El precio que está facturando está por debajo,"+CHR(10)+;
              "del costo del producto ¿Desea continuar?"+CHR(10)+;
              "Costo: "+ALLTRIM(STR(oQryArt:preciocos)),"Atencion")
          IF !lRta 
             RETURN .f.
          ENDIF      
      ENDIF 
...
oQryArt:= oApp:oServer:Query("SELECT codigo"+IF(oApp:usar_codpro,",codigopro","")+",nombre,stockact,precioven,iva,fecmod,reventa,preciocos,endolares,marca,stockmin,impint,stockotro "+;
...
STATIC FUNCTION ValidaArtP()
    oQryArt:= oApp:oServer:Query("SELECT codigo,nombre,stockact,precioven,iva,fecmod,reventa,preciocos,endolares,marca,stockmin,impint,stockotro "+;
...
LOCAL oGet1 := ARRAY(22), oBot := ARRAY(4), oForm, lRta := .f., oBrw2, oQryCuo, dFecVto,;
      ...
      cNotaCre := "", lPreciosFinales := .f., cSQL, cObserva
cLetra := IF(nTipoDoc=5,"R",IF(oApp:tipo_iva<>6,IF(oGet[13]:nAt = 1 .or. oGet[13]:nAt = 2 .or. oGet[13]:nAt = 6,"A","B"),"C"))
cLetra := IF(!lFacturaM,cLetra,IF(cLetra="A","M",cLetra))
...
IF nCliente = -1
   IF EMPTY(oGet[09]:cText)
      MsgStop("Debe poner el nombre del cliente ocacional","Error")
      RETURN nil 
   ENDIF   
   IF oGet[13]:nAt <> 5 .and. (EMPTY(oGet[12]:cText) .OR. oGet[12]:cText = "  -        - ")
      MsgStop("Debe poner un CUIT para esa condición de IVA","Error")
      RETURN nil
   ENDIF   
ENDIF   
DO CASE 
...
IF !lRta
   oApp:oServer:Execute("DELETE FROM ventas_det_H WHERE ESPROMO IS TRUE")
   oQryDet:Refresh()
   oBrw:Refresh()
   oBrw:Maketotals()
   ActualizarDet()
...
cTipoDoc := IF(nTipoDoc=1,"FC",IF(nTipoDoc=2,"ND","NC"))
  ...
  oApp:oServer:BeginTransaction()
      ...
      IF oApp:usar_cuotas  
         oApp:oServer:Execute("INSERT INTO ge_"+oApp:cId+"ventas_cuota (tipo,letra,numero,cuota,cantcuo,fecha,cliente,neto,importe,saldo,estado,fecvto,"+;
                              ...
                              "(SELECT "+ClipValue2Sql(cTipoDoc)+","+;
                                         ...
                                         ClipValue2Sql(oGet[1]:value)+","+ ClipValue2Sql(nCliente)+","+ClipValue2Sql(nNetoPar/oGet1[9]:value)+","+;
      ...
      IF !oApp:usar_cuotas
             oApp:oServer:Execute("INSERT INTO ge_"+oApp:cId+"ventas_cuota (tipo,letra,numero,cuota,cantcuo,fecha,cliente,neto,importe,saldo,estado,fecvto,"+;
                              ...
                              "VALUES ("+ClipValue2Sql(cTipoDoc)+","+;
                                         ...
                                         ClipValue2Sql(oGet[1]:value)+","+ ClipValue2Sql(nCliente)+",0,"+;
                                         ClipValue2Sql(nTotal)+","+;
     ...
     ENDDO   
      ...
      ENDIF
      cObserva := ALLTRIM(oGet[23]:cText)
      IF lPedidoUsado = .t.
         cObserva := cObserva + " Ped:"+oApp:oServer:Query("SELECT GROUP_CONCAT(id) AS id FROM pedidos_temp WHERE tilde"):id
      ENDIF   
      oApp:oServer:Execute("INSERT INTO ge_"+oApp:cId+"ventas_encab (ticomp,letra,numcomp,codcli,coniva,fecha,neto,iva,iibb,importe,tipopag,observa,"+;
                           ...
                           "("+ClipValue2Sql(cTipoDoc)+","+ClipValue2Sql(LEFT(cNumComp,1))+","+ClipValue2Sql(RIGHT(cNumComp,13))+","+;
                            ...
                            ClipValue2Sql(IF(EMPTY(cNotaCre),"",cNotaCre+" ")+ALLTRIM(cObserva)+IF(nDescuGen>0," Descuento: %"+ALLTRIM(STR(nDescuGen,5,2)),""))+","+;
                            ...
                            ClipValue2Sql(oApp:usuario)+","+ClipValue2SQL(oGet[1]:value)+","+ClipValue2SQL(oApp:cIp)+","+ClipValue2Sql(cVendedor)+","+;
      ...
      IF !lAcopio
          oApp:oServer:Execute("UPDATE ge_"+oApp:cId+"articu a "+; 
                   ...
                   "FROM ventas_det_H "+;
                   "WHERE ESPROMO IS FALSE AND codart > 0 "+;
                   "GROUP BY codart "+;
      ...
      ***oApp:oServer:Execute("DELETE FROM ge_"+oApp:cId+"presupuestos_det   WHERE numven = "+ClipValue2Sql(nBorrarPresu))
      ...
      IF lPedidoUsado = .t.
         ...
         oApp:oServer:Execute("TRUNCATE pedidos_temp")    
      ENDIF      
      EjecutarReplicaCompra(cTipoDoc, LEFT(cNumComp,1), RIGHT(cNumComp,13), nCliente, oGet[1]:value )
...
CASE nTipoDoc = 5
  ...
  oApp:oServer:Execute("INSERT INTO ge_"+oApp:cId+"remitos "+;
                        " (codart,detart,cantidad,fecha,codcli,nrorem,codiva,facturado,incrp,importe,iva,neto,neton,tipo,auto,vendedor,usuario) "+;
                        ...
                        "'I',bultos,"+ClipValue2Sql(cVendedor)+","+ClipValue2Sql(oApp:usuario)+;
...
LOCAL oGet := ARRAY(5), oBot := ARRAY(3), oForm, lRta := .F., aCor, base, oError, oQry,;
      cNomCue:=SPACE(50),oQryCue,oDlg1
DEFAULT lImpSinIva := .f.

DEFINE DIALOG oForm TITLE "Emision de presupuesto";
       FROM 05,15 TO 20,70 OF oDlg FONT oApp:oFont
   ...
   IF oApp:presudiscriva
      @ 50, 50 CHECKBOX oGet[4] VAR lImpSinIva OF oForm PIXEL SIZE 200,12 PROMPT "Imprimir presupuesto sin IVA" WHEN (!lPreciosFinales)
      @ 65, 50 CHECKBOX oGet[5] VAR lPreciosFinales OF oForm PIXEL SIZE 200,12 PROMPT "Imprimir Precios finales" WHEN (!lImpSinIva)
      ELSE 
...
FUNCTION PrintFactuElec(cTipoDoc,cNumComp,aMetodosPago)
LOCAL aiva := { "IVA Responsable Inscripto","IVA Responsable no Inscripto",;
  ...
  oQryIvaDet, aCor,cCondVen,cFormaPag, nDesFac, oQryP , nRow1, nHoja , oQryPar1, cDomicilioFiscal, nBultos,;
  lImprimio_promo, lAgrupa, nCopiaCC := 0, aOriginal := {"ORIGINAL","DUPLICADO","TRIPLICADO","CUADRUPLICADO","QUINTUPLICADO","SEXTUPLICADO"} 
DEFAULT aMetodosPago := {}  
  oQryVen1 := oApp:oServer:Query("SELECT v.*,c.codpos FROM ge_"+oApp:cId+"ventas_encab v "+;
                                 " LEFT JOIN ge_"+oApp:cId+"clientes c ON c.codigo = v.codcli "+;
                                 "WHERE v.ticomp = " + ClipValue2Sql(cTipoDoc)+;
                                 " AND v.letra = "+Clipvalue2sql(LEFT(cNumComp,1))+;
                                 " AND v.numcomp = " + ClipValue2Sql(RIGHT(cNumComp,13)))
  ...
  oApp:cDestinoMail := oApp:oServer:Query("SELECT mail FROM ge_"+oApp:cId+"clientes WHERE codigo = "+ClipValue2Sql(oQryVen1:codcli)):mail
   ...
   IF lAgrupa
       oQryDet1 := oApp:oServer:Query("SELECT vd.codart,vd.detart,SUM(vd.cantidad) AS cantidad,vd.punit,SUM(vd.importe) AS importe,SUM(vd.neto) AS neto,SUM(vd.iva) AS iva,"+;
                                  "vd.codiva,SUM(vd.neton) AS neton,SUM(vd.descu) AS descu,SUM(vd.impint) AS impint,SUM(vd.bultos) AS bultos,"+;
       "IF(vd.descup>0,vd.descup,0) AS descupor,"+;
       "IF(SUBSTR(nrofac,3,1) = 'A', SUM(neto),SUM(importe)) AS subtot,"+;
       "IF(codiva = 5,'21%',IF(codiva=3,'0%','10.5%')) AS alicu, (i.tasa / 100) as tasa,"+;
       "SUM(importe) AS subtotalciva,a.codigopro, a.condicioncompra FROM ge_"+oApp:cId+"ventas_det vd "+;
                                  ...
                                  IF(lAgrupa," GROUP BY vd.codart,vd.punit,vd.descup  ORDER BY vd.id",""))
       ELSE 
       ...
       "IF(codiva = 5,'21%',IF(codiva=3,'0%','10.5%')) AS alicu, (i.tasa / 100) as tasa,"+;
       "importe AS subtotalciva,a.codigopro, a.condicioncompra FROM ge_"+oApp:cId+"ventas_det vd "+;
                                  " LEFT JOIN ge_"+oApp:cId+"articu a ON a.codigo = vd.codart "+;
                                  " LEFT JOIN ge_"+oApp:cId+"ivas i ON i.codigo = vd.codiva "+;
                                  " WHERE nrofac = " + ClipValue2Sql(cTipoDoc+cNumComp))
   ENDIF    
   ...
   DO CASE
      CASE !oQryP:imprimeTic
           ** FACTURACION CON FORMATO PARA FACTURA ELECTRONICA
           ...
           ENDIF
           IF oQryPun:dosenuno
              P2x1FacturaElectronica(cTipoDoc, cNumComp, oQryVen1, oQryDet1, oQryIvaDet,;
                                     oQryP, oQryPar1, config, cCondVen, cFormaPag,;
                                     nIndiceDolar, cSigno, cDomicilioFiscal, aiva)
              oApp:cDestinoMail := " "
              RETURN nil
           ENDIF   
           ...
           oPrn:SetPortrait()
           oPrn:SetPage(9)
           FOR x := 1 TO IF(oQryP:copiafa=1,2,oQryP:copiafa)
             nHoja := 1
             nBultos := 0
             lImprimio_promo := .f.
             PAGE    
             ...
             *                   oPrn:nHorzRes(), oPrn:nVertRes() ,   nil, .t. )
             @ 25,01 PRINT TO oPrn IMAGE "https://app.bcnsoft.com.ar/img/arca.jpg" /*"afip.jpg"*/ SIZE 2.5, 2.2 CM 
             ...
             @ .8, 01.15 PRINT TO oPrn TEXT aOriginal[ x] ;
           ...
           oPrn:CmSay( 3.0, 11, "Fecha de emision:"+DTOC(oQryVen1:fecmod)+" "+ALLTRIM(oQryVen1:hora),oFont)
             ...
             @ 5.5, 12.5 PRINT TO oPrn TEXT ALLTRIM(oQryVen1:nombre)+" ("+ALLTRIM(str(oQryVen1:codcli))+")" ;
             ...
             @ nRow, 12.5 PRINT TO oPrn TEXT ALLTRIM(oQryVen1:direccion) + " " + ;
                                                   ALLTRIM(oQryVen1:localidad) + " "+ ALLTRIM(oQryVen1:codpos);
             ...
             FOR i = 1 TO oQryDet1:nRecCount  
                 IF oQryDet1:punit < 0 .and. !lImprimio_promo 
                    @ y+.4, 01 PRINT TO oPrn TEXT "Descuentos Promociones" SIZE 10,1 CM FONT oFont ALIGN "TC" LASTROW y
                    lImprimio_promo := .t.
                    y := y + .3
                 ENDIF         
                 IF oApp:impcodpro
                    ...
                    @ y-0.07, 01 PRINT TO oPrn TEXT STR(oQryDet1:codart,13) ;
                          SIZE 2.5,.5 CM FONT oFont5 ALIGN "TL"
                 ENDIF         
                 @ y-0.07, 03.5 PRINT TO oPrn TEXT ALLTRIM(oQryDet1:detart)+" "+ALLTRIM(oQryDet1:condicioncompra) ;
                      SIZE 6.5,2.5 CM FONT oFont5 LASTROW nRow ALIGN "TL"
                 @ y-0.07, 10 PRINT TO oPrn TEXT STR(oQryDet1:cantidad,10,2) ;
                 ...
                 ENDIF     
                 @ y-0.07, 12 PRINT TO oPrn TEXT TRANSFORM(IF(!LEFT(cNumComp,1)$"AM",oQryDet1:punit/nIndiceDolar ,oQryDet1:neton/oQryDet1:cantidad/nIndiceDolar),"@E 999,999,999.99") ;
                      SIZE 2,.5 CM FONT oFont5 ALIGN "TR"
                 @ y-0.07, 14.2 PRINT TO oPrn TEXT IF(oQryDet1:descupor > 0,;
                           ALLTRIM(IF(oQryDet1:descupor = INT(oQryDet1:descupor),;
                                      STR(INT(oQryDet1:descupor),3),;
                                      STR(oQryDet1:descupor,6,2)))+"% "+;
                           IF(oQryPar1:imprimedescuimp, ALLTRIM(TRANSFORM(oQryDet1:descu,"@E 999,999,999.99")),"")," ") ;
                      SIZE 1.8,.5 CM FONT oFont5 ALIGN "TR" 
                 @ y-0.07, 15.5 PRINT TO oPrn TEXT  TRANSFORM(oQryDet1:subtot/nIndiceDolar,"@E 999,999,999.99") ;
                      SIZE 2,.5 CM FONT oFont5 ALIGN "TR"
                 @ y-0.07, 17.7 PRINT TO oPrn TEXT oQryDet1:alicu ;
                      SIZE .7,.5 CM FONT oFont5 ALIGN "TC"
                 @ y-0.07, 18.3 PRINT TO oPrn TEXT TRANSFORM(oQryDet1:subtotalciva/nIndiceDolar,"@E 999,999,999.99");
                      SIZE 2,.5 CM FONT oFont5 ALIGN "TR"    
                 nDesFac := nDesFac + oQryDet1:descu   
                 oQryDet1:Skip()
                 y := nRow + .1 + config:salto/10
                 IF y > 21
              ...
              NEXT       
              y := y + .5
              @ y, 1 PRINT TO oPrn TEXT ALLTRIM(oQryVen1:observa) + IF(nDesFac>0," Desc."+cSigno+" "+TRANSFORM(nDesFac/nIndiceDolar,"@E 999,999,999.99")+ " (IVA Incl.) "," ") + ;
              ...
              @ 22.5,6.1 PRINT TO oPrn TEXT TRANSFORM(oQryVen1:IIBB/nIndiceDolar,"@E 999,999,999.99") ;
                      SIZE 3,.5 CM FONT oFont ALIGN "R"
              
            IF !LEFT(cNumComp,1)$"AM"    
                 @ 22.5, 12 PRINT TO oPrn TEXT "Subtotal "+cSigno+":" ;
                      SIZE 4,.5 CM FONT oFont3 ALIGN "R"
                 @ 22.5, 17 PRINT TO oPrn TEXT TRANSFORM((oQryVen1:importe-oQryVen1:sobretasa)/nIndiceDolar,"@E 999,999,999.99") ;
                      SIZE 3,.5 CM FONT oFont ALIGN "R"
                 @ 23, 12 PRINT TO oPrn TEXT "Otros Tributos "+cSigno+":" ;
                      SIZE 4,.5 CM FONT oFont3 ALIGN "R"
                 @ 23, 17 PRINT TO oPrn TEXT TRANSFORM(oQryVen1:sobretasa/nIndiceDolar,"@E 999,999,999.99") ;
                      SIZE 3,.5 CM FONT oFont ALIGN "R"
                 @ 23.5, 12 PRINT TO oPrn TEXT "Total "+cSigno+":" ;
                      SIZE 4,.5 CM FONT oFont3 ALIGN "R"
                 @ 23.5, 17 PRINT TO oPrn TEXT TRANSFORM(oQryVen1:importe/nIndiceDolar,"@E 999,999,999.99") ;
                      SIZE 3,.5 CM FONT oFont ALIGN "R"   
                 //Nuevo... Transparencia Fiscal
                 @ 23.7, 01 PRINT TO oPrn TEXT "Regimen de transparencia fiscal al Consumidor (Ley 27.743)" ;
                      SIZE 7,.5 CM FONT oFont5 ALIGN "R" LASTROW y 
                 @ y   , 01 PRINT TO oPrn TEXT "IVA Contenido: $"+TRANSFORM(oQryVen1:iva/nIndiceDolar,"@E 999,999,999.99") + " " ;
                      SIZE 7,.5 CM FONT oFont5 ALIGN "R" LASTROW y    
               ELSE 
                 @ 22.5, 12 PRINT TO oPrn TEXT "Subtotal "+cSigno+":" ;
                      SIZE 4,.5 CM FONT oFont3 ALIGN "R"
                 @ 22.5, 17 PRINT TO oPrn TEXT TRANSFORM(oQryVen1:neto/nIndiceDolar,"@E 999,999,999.99") ;
                      SIZE 3,.5 CM FONT oFont ALIGN "R"
                 @ 23, 12 PRINT TO oPrn TEXT "Otros Tributos "+cSigno+":" ;
                      SIZE 4,.5 CM FONT oFont3 ALIGN "R"
                 @ 23, 17 PRINT TO oPrn TEXT TRANSFORM(oQryVen1:sobretasa/nIndiceDolar,"@E 999,999,999.99") ;
               ...
               FOR i = 1 TO oQryIvaDet:nRecCount           
                     @ y, 12 PRINT TO oPrn TEXT "I.V.A. "+ALLTRIM(oQryIvaDet:nomiva) + " "+cSigno+":" ;
                      SIZE 4,.5 CM FONT oFont3 ALIGN "R"
                     @ y, 17 PRINT TO oPrn TEXT TRANSFORM(oQryIvaDet:iva/nIndiceDolar,"@E 999,999,999.99") ;
                      SIZE 3,.5 CM FONT oFont ALIGN "R" 
                      oQryIvaDet:Skip()        
                     y := y + .5
               NEXT
               @ y, 12 PRINT TO oPrn TEXT "Total "+cSigno+":" ;
                      SIZE 4,.5 CM FONT oFont3 ALIGN "R"
               @ y, 17 PRINT TO oPrn TEXT TRANSFORM(oQryVen1:importe/nIndiceDolar,"@E 999,999,999.99") ;
                      SIZE 3,.5 CM FONT oFont ALIGN "R"                  
             ENDIF    
           ...
           oApp:cDestinoMail := " "
       OTHERWISE
           IF oQryPar1:imprimetickecc 
              IF oApp:oServer:Query("SELECT saldo FROM ge_"+oApp:cId+"ventas_cuota  "+;                                 
                                 "WHERE tipo = " + ClipValue2Sql(cTipoDoc)+;
                                 " AND letra = "+Clipvalue2sql(LEFT(cNumComp,1))+;
                                 " AND numero = " + ClipValue2Sql(RIGHT(cNumComp,13))):saldo > 0
                 nCopiaCC := 1
              ENDIF                 
           ENDIF
           IF !oApp:tick80
               *********** Impresion de Ticket 48 mm 
               ...
               ENDIF   
               DEFINE FONT oFont   NAME "ARIAL"       SIZE config:fon*.8,config:fon*3
               DEFINE FONT oFont1  NAME "ARIAL"       SIZE config:fon*1.5,config:fon*3.5 BOLD
               FOR x := 1 TO oQryP:copiafa + nCopiaCC
               PRINT oPrn TO ALLTRIM(oQryP:impresoraT)
                  ...
                  PAGE                                  
                     ...
                     @ nRow, .1 PRINT TO oPrn TEXT "Cod:" + oQryVen1:tipfor+" Nro:"+RIGHT(cNumcomp,13) ;
                                  SIZE 4.8,.5 CM FONT oFont LASTROW nRow ALIGN "C"                 
                     @ nRow, .1 PRINT TO oPrn TEXT "Fecha Emision:" + +DTOC(oQryVen1:fecmod)+" "+oQryVen1:hora;
                                  SIZE 4.8,.5 CM FONT oFont LASTROW nRow ALIGN "C"                                                  
                     IF oQryVen1:coniva = 5 .and. oQryVen1:codcli <= 1 .and. "CONSUMIDOR"$oQryVen1:nombre 
                        @ nRow, .1 PRINT TO oPrn TEXT "A Consumidor Final";
                                  SIZE 4.8,.5 CM FONT oFont LASTROW nRow ALIGN "C"
                        ELSE 
                        @ nRow, .1 PRINT TO oPrn TEXT ALLTRIM(oQryVen1:nombre)+" ("+ALLTRIM(str(oQryVen1:codcli))+")";
                     ...
                     FOR i = 1 TO oQryDet1:nRecCount           
                         nRow1 := nRow
                         IF oQryDet1:punit < 0 .and. !lImprimio_promo 
                            @ nRow1+.2, 00 PRINT TO oPrn TEXT "Descuentos Promos" SIZE 5,1 CM FONT oFont1 ALIGN "C" LASTROW nRow1
                            lImprimio_promo := .t.
                            nRow1 := nRow1 + .1
                         ENDIF
                      ...
                      IF LEFT(cNumComp,1)<>"A"       
                         @ nRow, .1 PRINT TO oPrn TEXT "Total $ " + STR(oQryVen1:importe,12,2);
                                  SIZE 4.8,.5 CM FONT oFont1 LASTROW nRow ALIGN "L"      
                         //Nuevo... Transparencia Fiscal
                         @ nRow, .1 PRINT TO oPrn TEXT "Regimen Transparencia Fiscal";
                                  SIZE 4.8,.5 CM FONT oFont1 LASTROW nRow ALIGN "L"
                         @ nRow, .1 PRINT TO oPrn TEXT    "IVA Contenido: $"+STR(oQryVen1:iva,12,2);
                                  SIZE 4.8,.5 CM FONT oFont1 LASTROW nRow ALIGN "L" 
                         IF oQryVen1:iibb > 0
                            @ nRow, .1 PRINT TO oPrn TEXT "Percepcion IIBB "+STR(oQryVen1:percep,6,2)+"%: $"+STR(oQryVen1:iibb,12,2) ;
                              SIZE 4.8,.5 CM FONT oFont LASTROW nRow ALIGN "L"                     
                         ENDIF                                           
                     ...
                     ENDIF
                     IF oQryPar1:emite_pie                        
                        IF !EMPTY(aMetodosPago)
                           @ nRow+.3, .1 PRINT TO oPrn TEXT "FORMAS DE PAGO" ;
                                    SIZE 4.8,1 CM FONT oFont1 LASTROW nRow ALIGN "L"   
                           FOR i := 1 TO LEN(aMetodosPago)
                               @ nRow+.3, .1 PRINT TO oPrn TEXT aMetodosPago[i,1] ;
                                    SIZE 2.8,.5 CM FONT oFont ALIGN "TL"
                               @ nRow+.3, 3 PRINT TO oPrn TEXT STR(aMetodosPago[i,2],12,2) ;
                                    SIZE 1.8,.5 CM FONT oFont ALIGN "TR" LASTROW nRow
                           NEXT i
                        ENDIF           
                        @ nRow, .1 PRINT TO oPrn TEXT oQryPar1:texto_pie ;
                                    SIZE 4.8,1.5 CM FONT oFont2 LASTROW nRow ALIGN "L"  
                     ENDIF            
                     @ nRow,.1 PRINT TO oPrn TEXT "...";
                              SIZE 4.8,.5 CM FONT oFont1 LASTROW nRow ALIGN "L"
                     @ nRow,.1 PRINT TO oPrn TEXT "...";
               ...
               PRINT oPrn TO ALLTRIM(oQryP:impresoraT)
                  ...
                  PAGE                                  
                     ...
                     FOR i = 1 TO oQryDet1:nRecCount           
                         nRow1 := nRow
                         IF oQryDet1:punit < 0 .and. !lImprimio_promo 
                            @ nRow1+.2, 00 PRINT TO oPrn TEXT "Descuentos Promos" SIZE 7.1,1 CM FONT oFont1 ALIGN "C" LASTROW nRow1
                            lImprimio_promo := .t.
                            nRow1 := nRow1 + .1
                         ENDIF
                         @ nRow1, 00.00 PRINT TO oPrn TEXT ALLTRIM(oQryDet1:detart);
                              SIZE 3.5,.5 CM FONT oFont LASTROW nRow ALIGN "L"
                         @ nRow1, 03.60 PRINT TO oPrn TEXT STR(oQryDet1:cantidad,07,2);
...
LOCAL aiva := { "IVA Responsable Inscripto","IVA Responsable no Inscripto",;
      ...
      oQryP :=  oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"punto WHERE ip = "+ ClipValue2Sql(oApp:cip)),;
      lComan :=.f., cDomicilioFiscal, nRow1, lAgrupa := oQryP:agrupaimpresion, oQryDet1    
DEFAULT dFecVen:= nil, lImpSinIva := .f., lPreciosFinales := .f.
   ...
   IF lAgrupa 
      oQryDet1 := oQryDet 
      oQryDet := oApp:oServer:Query("SELECT codart,detart,SUM(cantidad) AS cantidad,punit,"+;
                                    "SUM(ptotal) AS ptotal,SUM(stotal) AS stotal "+;                                         
       " FROM ventas_det_H  GROUP BY codart,punit,DESCUENTO ORDER BY id")
   ENDIF
   cDomicilioFiscal := oApp:dire_emp
   ...
   IF !oQryP:imprimeTic
       ** PRESUPUESTOS
       ...
       ENDIF
       IF oQryPun:dosenuno
          P2x1Presupuesto(oQryDet, oGet, oBrw, lDolar, dFecVen, cNumcomp,;
                          lImpSinIva, lPreciosFinales, oQryP, config, aiva)
          oApp:cDestinoMail := " "
          RETURN nil 
       ENDIF   
       ...
       FOR x := 1 TO oQryP:copiafb
         ...
         *                   oPrn:nHorzRes(), oPrn:nVertRes() ,   nil, .t. )
         ...
         oPrn:CmSay( 2.0, 16, "Nº: "+SUBSTR(cNumcomp,2,16),oFont)         
         ...
         @ 5.5, 9.5  PRINT TO oPrn TEXT "Razon Social:" ;
                  SIZE 2.5,.5 CM FONT oFont3 ALIGN "R"
         @ 5.5, 12.5 PRINT TO oPrn TEXT ALLTRIM(oGet[9]:cText) + " ("+ALLTRIM(oGet[8]:cText)+")";
                  SIZE 8,1 CM FONT oFont LASTROW nRow 
         @ 6,1  PRINT TO oPrn TEXT "Condicion IVA:" ;
                  SIZE 3,.5 CM FONT oFont3 ALIGN "R"
         ...
         @ 8.2, 11 PRINT TO oPrn TEXT "Cantidad" ;
                  SIZE 2,.5 CM FONT oFont ALIGN "R"                  
         @ 8.2, 14 PRINT TO oPrn TEXT "Pr.Unit."+IF(oApp:presudiscriva .and. !lImpSinIva .and. !lPreciosFinales," S/IVA","") ;
                  SIZE 2.1,.5 CM FONT oFont ALIGN "R"
         @ 8.2, 17 PRINT TO oPrn TEXT "Subtotal"+IF(oApp:presudiscriva .and. !lImpSinIva .and. !lPreciosFinales," S/IVA","") ;
                  SIZE 3,.5 CM FONT oFont ALIGN "R"
         y := 9.2
         oQryDet:GoTop()
         ...
         FOR i = 1 TO oQryDet:nRecCount                      
             ...
             IF !oApp:presudiscriva     
                 @ y-0.07, 14 PRINT TO oPrn TEXT TRANSFORM(oQryDet:punit/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                      SIZE 2,.5 CM FONT oFont ALIGN "TR"
                 @ y-0.07, 17 PRINT TO oPrn TEXT TRANSFORM(oQryDet:ptotal/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99");
                      SIZE 3,.5 CM FONT oFont ALIGN "TR" 
                 ELSE
                 IF !lPreciosFinales 
                     @ y-0.07, 14 PRINT TO oPrn TEXT TRANSFORM(oQryDet:stotal/oQryDet:cantidad/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                          SIZE 2,.5 CM FONT oFont ALIGN "TR"
                     @ y-0.07, 17 PRINT TO oPrn TEXT TRANSFORM(oQryDet:stotal/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99");
                          SIZE 3,.5 CM FONT oFont ALIGN "TR" 
                     nIva := nIva + oQryDet:ptotal - oQryDet:stotal     
                    ELSE 
                    @ y-0.07, 14 PRINT TO oPrn TEXT TRANSFORM(oQryDet:ptotal/oQryDet:cantidad/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                          SIZE 2,.5 CM FONT oFont ALIGN "TR"
                     @ y-0.07, 17 PRINT TO oPrn TEXT TRANSFORM(oQryDet:ptotal/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99");
                          SIZE 3,.5 CM FONT oFont ALIGN "TR" 
                     nIva := nIva + 0
          ...
          IF oBrw:aCols[6]:nTotal > 0        
              @ 23.5,01  PRINT TO oPrn TEXT "Descuento General"+IF(lDolar,"u$s","$")+":"  ;
                      SIZE 3.9,.5 CM FONT oFont3 ALIGN "R"
              @ 23.5,5 PRINT TO oPrn TEXT TRANSFORM(oBrw:aCols[6]:nTotal/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                  SIZE 4,.5 CM FONT oFont1 ALIGN "L"
          ENDIF    
          IF !oApp:presudiscriva      
              @ 23.5,11  PRINT TO oPrn TEXT "Total presupuesto "+IF(lDolar,"u$s","$")+":"  ;
                      SIZE 3.9,.5 CM FONT oFont3 ALIGN "R"
              @ 23.5,15 PRINT TO oPrn TEXT TRANSFORM(oGet[21]:value/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                      SIZE 4,.5 CM FONT oFont1 ALIGN "L"
              ELSE 
              IF !lImpSinIva 
                  IF !lPreciosFinales
                  @ 23.5,11  PRINT TO oPrn TEXT "Tasa gral (IVA) "+IF(lDolar,"u$s","$")+":"  ;
                          SIZE 3.9,.5 CM FONT oFont3 ALIGN "R"
                  @ 23.5,15 PRINT TO oPrn TEXT TRANSFORM(nIva/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                          SIZE 4,.5 CM FONT oFont1 ALIGN "L"
                  ENDIF        
                  @ 24.0,11  PRINT TO oPrn TEXT "Total presupuesto "+IF(lDolar,"u$s","$")+":"  ;
                          SIZE 3.9,.5 CM FONT oFont3 ALIGN "R"
                  @ 24.0,15 PRINT TO oPrn TEXT TRANSFORM(oGet[21]:value/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                          SIZE 4,.5 CM FONT oFont1 ALIGN "L"
                 ELSE             
                  @ 24.0,11  PRINT TO oPrn TEXT "Total presupuesto "+IF(lDolar,"u$s","$")+":"  ;
                          SIZE 3.9,.5 CM FONT oFont3 ALIGN "R"
                  @ 24.0,15 PRINT TO oPrn TEXT TRANSFORM(oGet[21]:value/IF(lDolar,oApp:dolar,1)-nIva/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                          SIZE 4,.5 CM FONT oFont1 ALIGN "L"
              ENDIF            
          ENDIF 
           ...
           PRINT oPrn TO ALLTRIM(oQryP:impresoraT)
              ...
              PAGE                                  
                 ...
                 @ nRow, .1 PRINT TO oPrn TEXT "Fecha Emision:" + +DTOC(DATE());
                              SIZE 4.8,.5 CM FONT oFont LASTROW nRow ALIGN "C"                                                  
                 @ nRow, .1 PRINT TO oPrn TEXT ALLTRIM(oGet[9]:cText) + " ("+ALLTRIM(oGet[8]:cText)+")";
                              SIZE 4.8,.5 CM FONT oFont LASTROW nRow ALIGN "L"
...
*******************************************
** Impresion de Remito
FUNCTION PrintRemito(oQryDet,oGet,oBrw, cNumComp, nCodCli,cFactRemitada)
LOCAL aiva := { "IVA Responsable Inscripto","IVA Responsable no Inscripto",;
                "IVA no Responsable","IVA Sujeto Exento",;
                "Consumidor Final","Responsable Monotributo",;
                "Sujeto no Categorizado","Proveedor del Exterior",;
      ...
      oQryP :=  oApp:oServer:Query("SELECT * FROM ge_"+oApp:cId+"punto WHERE ip = "+ ClipValue2Sql(oApp:cip)),;
      lComan :=.f., cDomicilioFiscal, nRow1, dFecha
DEFAULT cFactRemitada:=''
    IF nCodcli = nil
    ...
    IF oQryP:preguntaimpr 
      ...
      oQryP:imprimeTic := lComan
    ENDIF 
    IF cNumComp = "00000000000000000"
       dFecha := DATE()
       ELSE 
       dFecha := oApp:oServer:Query("SELECT fecha FROM ge_"+oApp:cId+"remitos WHERE nrorem = "+;
                                    Clipvalue2sql('RE'+cNumcomp) + " LIMIT 1"):fecha
    ENDIF   
    ...
    IF !oQryP:imprimeTic
         ...
         ** REMITO A4
         ...
         ENDIF
         IF oQryPun:dosenuno
            P2x1Remito(oQryDet, oGet, oBrw, cNumComp, nCodCli, cFactRemitada,;
                       oQryP, oQryCli, lRta, config, aiva,dFecha)
            oApp:cDestinoMail := " "
            RETURN NIL
         ENDIF   
         ...
         FOR x := 1 TO oQryP:copiare
           ...
           IF lRta
               ...
               *                   oPrn:nHorzRes(), oPrn:nVertRes() ,   nil, .t. )
               ...
               oPrn:CmSay( 3.0, 11, "Fecha de emision:"+DTOC(dFecha),oFont)
           ...
           FOR i = 1 TO oQryDet:nRecCount          
                 ...
                 ENDIF
                 IF oQryDet:codart > 0  
                    cCodigoPro := oApp:oServer:Query("SELECT condicioncompra FROM ge_"+oApp:cId+"articu WHERE codigo = "+STR(oQryDet:codart)):condicioncompra           
                    ELSE 
                    cCodigopro := ""
                 ENDIF                   
                 @ y-0.07, 04 PRINT TO oPrn TEXT ALLTRIM(oQryDet:detart)+" "+ALLTRIM(cCodigopro) ;
                    SIZE 7,2.5 CM FONT oFont LASTROW nRow ALIGN "TL"
           ...
           IF oGet <> nil
              @ y, 1 PRINT TO oPrn TEXT oGet[23]:cText ;
                    SIZE 18,3 CM FONT oFont ALIGN "L" LASTROW y
           ENDIF
           IF LEN(cFactRemitada) > 0
              @ y, 1 PRINT TO oPrn TEXT "Factura remitada: "+ALLTRIM(cFactRemitada);
                    SIZE 18,3 CM FONT oFont ALIGN "L" LASTROW y
           ENDIF
           IF oApp:usabultosventa
              @ y, 1 PRINT TO oPrn TEXT "Total Bultos:"+STR(nBultos,4) ;
               ...
               *********** Impresion de Ticket 48 mm 
               ...
               PRINT oPrn TO ALLTRIM(oQryP:impresoraT)
                  ...
                  PAGE                                  
                     ...
                     @ nRow, .1 PRINT TO oPrn TEXT cNumcomp ;
                                  SIZE 4.8,.5 CM FONT oFont LASTROW nRow ALIGN "C"                 
                     @ nRow, .1 PRINT TO oPrn TEXT "Fecha Emision:" + +DTOC(dFecha);
                                  SIZE 4.8,.5 CM FONT oFont LASTROW nRow ALIGN "C"                                                  
                     @ nRow, .1 PRINT TO oPrn TEXT ALLTRIM(oQryCli:nombre) + " ("+ALlTRIM(STR(oQryCli:codigo))+")"; 
                                  SIZE 4.8,.5 CM FONT oFont LASTROW nRow ALIGN "L"
...
FUNCTION FacturaNoFiscal(cTipoDoc,cNumComp,aMetodosPago)
LOCAL aiva := { "IVA Responsable Inscripto","IVA Responsable no Inscripto",;
                "IVA no Responsable","IVA Sujeto Exento",;
                "Consumidor Final","Responsable Monotributo",;
                "Sujeto no Categorizado","Proveedor del Exterior",;
      ...
      lAgrupa, oQryIvaDet, aCor,cCondVen,cFormaPag, oQryP, nRow1, nHoja, nDesFac, nBultos, lComan,;
      lImprimio_promo := .f., nCopiaCC := 0, aOriginal := {"ORIGINAL","DUPLICADO","TRIPLICADO","CUADRUPLICADO","QUINTUPLICADO","SEXTUPLICADO"}
DEFAULT aMetodosPago := {}      
   oQryVen1 := oApp:oServer:Query("SELECT v.*,c.codpos  FROM ge_"+oApp:cId+"ventas_encab v "+;
                                 " LEFT JOIN ge_"+ oApp:cId+"clientes c ON c.codigo = v.codcli "+;
                                 " WHERE v.ticomp = " + ClipValue2Sql(cTipoDoc)+;
   ...
   IF lAgrupa
       ...
       "SUM(importe) AS subtotalciva,a.codigopro,a.condicioncompra FROM ge_"+oApp:cId+"ventas_det vd "+;
                                  ...
                                  " LEFT JOIN ge_"+oApp:cId+"ivas i ON i.codigo = vd.codiva "+;
                                  " WHERE nrofac = " + ClipValue2Sql(cTipoDoc+cNumComp)+;
                                  IF(lAgrupa,"GROUP BY vd.codart,vd.punit,vd.descu ORDER BY vd.id",""))
       ELSE 
       oQryDet1 := oApp:oServer:Query("SELECT vd.codart,vd.detart,vd.cantidad,vd.punit,vd.importe,vd.neto,vd.iva,"+;
                                  "vd.codiva,vd.neton,vd.descu,vd.impint,vd.bultos,"+;
       "IF(vd.descup>0,vd.descup,0) AS descupor,"+;
   ...
   DO CASE
      CASE !oQryP:imprimeTic   
           ** FACTURACION CON FORMATO PARA FACTURA ELECTRONICA
           ...
           DEFINE FONT oFont4 NAME "Interleaved 2of5 Text"    SIZE config:fon*1.5, config:fon*5 
           IF oQryP:dosenuno
              P2x1FacturaNoFiscal(cTipoDoc, cNumComp, oQryVen1, oQryDet1, oQryIvaDet,;
                                  oQryP, oQryPar, config, cCondVen, cFormaPag, aiva)
              oApp:cDestinoMail := " "
              RETURN nil 
           ENDIF   
           PRINT oPrn NAME "Presupuesto "+cNumcomp PREVIEW MODAL
           oPrn:SetPortrait()
           oPrn:SetPage(9)
           FOR x := 1 TO IF(oQryP:copiafb =1,2,oQryP:copiafb)
             PAGE 
             nHoja := 1   
             lImprimio_promo := .f.
             nBultos := 0
             ...
             *                   oPrn:nHorzRes(), oPrn:nVertRes() ,   nil, .t. )     
             ...
             oPrn:CmSay( 2.5, 11, "Punto de venta:"+SUBSTR(cNumcomp,2,4) + " Comprobante Nro:"+RIGHT(cNumcomp,8),oFont)
             oPrn:CmSay( 3.0, 11, "Fecha de emision:"+DTOC(oQryVen1:fecha)+" "+ALLTRIM(oQryVen1:hora),oFont)
             IF oQryP:imprimeDat
                 oPrn:CmSay( 3.5, 11, "CUIT:"+oApp:cuit_emp,oFont)
                 oPrn:CmSay( 4.0, 11, "Ingresos brutos:"+oApp:ingb_emp,oFont) 
                 oPrn:CmSay( 4.5, 11, "Inicio de Actividades:"+DTOC(oApp:inac_emp),oFont)    
             ...
             FOR i = 1 TO oQryDet1:nRecCount  
                 ...
                 IF oApp:impcodpro
                    ...
                    @ y-0.07, 01 PRINT TO oPrn TEXT STR(oQryDet1:codart,13) ;
                      SIZE 2.5,.5 CM FONT oFont5 ALIGN "TL"
                 ENDIF     
                 @ y-.07, 03.5 PRINT TO oPrn TEXT ALLTRIM(oQryDet1:detart)+" "+ALlTRIM(oQryDet1:condicioncompra) ;
                      SIZE 6.5,2.5 CM FONT oFont5 LASTROW nRow ALIGN "TL"
                 @ y-0.07, 10 PRINT TO oPrn TEXT STR(oQryDet1:cantidad,10,2) ;
                      SIZE 2,.5 CM FONT oFont5 ALIGN "TR"
                 IF oApp:usabultosventa
                    ...
                    nBultos := nBultos + oQryDet1:bultos                          
                 ENDIF  
                 @ y-0.07, 12 PRINT TO oPrn TEXT TRANSFORM(IF(LEFT(cNumComp,1)<>"A",oQryDet1:punit,oQryDet1:neton/oQryDet1:cantidad),"@E 999,999,999.99") ;
                      SIZE 2,.5 CM FONT oFont5 ALIGN "TR"
                 @ y-0.07, 14.2 PRINT TO oPrn TEXT IF(oQryDet1:descupor > 0,;
                           ALLTRIM(IF(oQryDet1:descupor = INT(oQryDet1:descupor),;
                                      STR(INT(oQryDet1:descupor),3),;
                                      STR(oQryDet1:descupor,6,2)))+"% "+;
                           IF(oQryPar:imprimedescuimp,ALLTRIM(TRANSFORM(oQryDet1:descu,"@E 999,999,999.99")),"")," ") ;
                      SIZE 1.8,.5 CM FONT oFont5 ALIGN "TR" 
                 @ y-0.07, 15.5 PRINT TO oPrn TEXT TRANSFORM(oQryDet1:subtot,"@E 999,999,999.99") ;
                      SIZE 2,.5 CM FONT oFont5 ALIGN "TR"
              ...
              IF nDesFac > 0
                 ...
                 @ nRow, 1 PRINT TO oPrn TEXT "Descuento Gral:"+TRANSFORM(nDesFac,"@E 999,999,999.99") ;
                      SIZE 18,3 CM FONT oFont ALIGN "L"
                 ELSE
                 y := y + .5
                 @ y, 1 PRINT TO oPrn TEXT ALLTRIM(STRTRAN(oQryVen1:observa,"PUNTO DE VENTA",""));
              ...
              @ 23.5, 12 PRINT TO oPrn TEXT "Total $:" ;
                      SIZE 4,.5 CM FONT oFont3 ALIGN "R"
              @ 23.5, 17 PRINT TO oPrn TEXT TRANSFORM(oQryVen1:importe,"@E 999,999,999.99") ;
          ...
          OTHERWISE
           ...
           IF !oApp:tick80
               ...
               *********** Impresion de Ticket NO FISCAL 
               ...
               FOR x := 1 TO oQryP:copiafb + nCopiaCC
               PRINT oPrn TO ALLTRIM(oQryP:impresoraT)
                  //oPrn:oFont := oFont
                  PAGE                                  
                     nRow := 1
                     IF oQryP:imprimeDat
                         ...
                         @ nRow, .1 PRINT TO oPrn TEXT "No Valido como factura " ;
                                      SIZE 5,.5 CM FONT oFont LASTROW nRow ALIGN "C"                 
                         @ nRow, .1 PRINT TO oPrn TEXT "Fecha Emision:" + +DTOC(oQryVen1:fecha) + " "+oQryVen1:hora;
                                      SIZE 5,.5 CM FONT oFont LASTROW nRow ALIGN "C"
                         ELSE 
                     ...
                     FOR i = 1 TO oQryDet1:nRecCount           
                         nRow1 := nRow  
                         IF oQryDet1:punit < 0 .and. !lImprimio_promo 
                            @ nRow1+.4, 00 PRINT TO oPrn TEXT "Descuentos Promos" SIZE 5,1 CM FONT oFont1 ALIGN "C" LASTROW nRow1
                            lImprimio_promo := .t.
                            nRow1 := nRow1 + .1
                         ENDIF
                         @ nRow1, 00 PRINT TO oPrn TEXT ALLTRIM(oQryDet1:detart);
                              SIZE 2.9,.5 CM FONT oFont LASTROW nRow ALIGN "L"
                         @ nRow1, 03 PRINT TO oPrn TEXT STR(oQryDet1:cantidad,07,1);
                              SIZE 0.7,.5 CM FONT oFont  ALIGN "R"
                  ...
                  PAGE                                  
                     ...
                     IF oQryVen1:coniva = 5 .and. oQryVen1:codcli <= 1 .and. "CONSUMIDOR"$oQryVen1:nombre 
                        @ nRow, .1 PRINT TO oPrn TEXT "A Consumidor Final";
                                  SIZE 7.1,.5 CM FONT oFont LASTROW nRow ALIGN "C"
                        ELSE 
                        @ nRow, .1 PRINT TO oPrn TEXT "Cliente: "+ALLTRIM(oQryVen1:nombre)+" ("+ALLTRIM(str(oQryVen1:codcli))+")";
                                  SIZE 7.1,1 CM FONT oFont LASTROW nRow ALIGN "L"
                        @ nRow, .1 PRINT TO oPrn TEXT oQryVen1:cuit;
                                  SIZE 7.1,.5 CM FONT oFont LASTROW nRow ALIGN "L"
                     ...
                     @ nRow1, 00.00 PRINT TO oPrn TEXT "Descripcion";
                                  SIZE 3.5,.5 CM FONT oFont LASTROW nRow ALIGN "L"
                     @ nRow1, 03.40 PRINT TO oPrn TEXT "Cant";
                                  SIZE 1,.5 CM FONT oFont LASTROW nRow ALIGN "R"
                     @ nRow1, 04.45 PRINT TO oPrn TEXT "Unit";
                                  SIZE 1.2,.5 CM FONT oFont LASTROW nRow ALIGN "R"
                     @ nRow1, 05.70 PRINT TO oPrn TEXT "Total";
                                  SIZE 1.5,.5 CM FONT oFont LASTROW nRow ALIGN "R"
                     oQryDet1:GoTop()  
                     oPrn:CmLine( nRow,0, nRow,9 )               
                     nRow := nRow + .2  
                     nDesFac := 0          
                     ...
                     FOR i = 1 TO oQryDet1:nRecCount           
                         nRow1 := nRow
                         IF oQryDet1:punit < 0 .and. !lImprimio_promo 
                            @ nRow1+.4, 00 PRINT TO oPrn TEXT "Descuentos Promos" SIZE 7.1,1 CM FONT oFont1 ALIGN "C" LASTROW nRow1
                            lImprimio_promo := .t.
                            nRow1 := nRow1 + .1
                         ENDIF
                         @ nRow1, 00.00 PRINT TO oPrn TEXT ALLTRIM(oQryDet1:detart);
                              SIZE 3.5,.8 CM FONT oFont LASTROW nRow ALIGN "L"
                         @ nRow1, 03.40 PRINT TO oPrn TEXT STR(oQryDet1:cantidad,07,2);
                              SIZE 1,.5 CM FONT oFont  ALIGN "R"
                         @ nRow1, 04.45 PRINT TO oPrn TEXT STR(oQryDet1:importe/oQryDet1:cantidad,9,2);
                              SIZE 1.2,.5   CM FONT oFont ALIGN "R"
                         @ nRow1, 05.70 PRINT TO oPrn TEXT STR(oQryDet1:importe,12,2);
...
STATIC FUNCTION VerHistorial(nCodcli,oQry,oBrw, nTipo)
LOCAL oBot := ARRAY(2), oForm, lRta := .f., aCor, oFont, oFont1, oError,;
      oQryVen, oQryDet, oBrwVen, oBrwDet, cIn
IF nTipo = 2
   cIn := "('FC','FR','NC')"
   ELSE
   cIn := "('FC','FR')"
...
oQryVen := oApp:oServer:Query("SELECT CONCAT(v.ticomp,v.letra, v.numcomp) AS compro, v.fecha as fecha, "+;                          
                          ...
                          " WHERE v.codcli = " + ClipValue2Sql(nCodCli) + " AND v.ticomp IN "+cIn+"  "+;
...
TRY
    ...
    oApp:oServer:Execute("INSERT INTO ventas_det_H (codart,detart,cantidad,punit,ptotal,stotal,iva,codiva,neto,descuento,pcosto) "+;
                     ...
                     "WHERE v.nrofac = "+ClipValue2Sql(oQryVen:compro)+" AND v.codart > 0 AND v.punit >0)")
...
DEFINE FONT oFont NAME "TAHOMA" SIZE 0,-11.5  
    ...
    oApp:oServer:Execute("TRUNCATE ventas_det_H")
    oApp:oServer:Execute("INSERT INTO ventas_det_H (codart,detart,cantidad,punit,neto,descuento,descup,stotal,iva,codiva,ptotal,pcosto,impint) "+;
                         "(SELECT p.codart,p.detart,p.cantidad,p.punit,p.neton,p.descu,p.descup,p.neto,p.iva,p.codiva,p.importe,p.pcosto,a.impint*p.cantidad "+;
                         " FROM ge_"+oApp:cId+"presupuestos_det p "+;
                         " LEFT JOIN ge_"+oApp:cId+"articu a ON a.codigo = p.codart "+;
                         "WHERE p.numven = "+ClipValue2Sql(oQryPresu:id)+")")
...
LOCAL aiva := { "IVA Responsable Inscripto","IVA Responsable no Inscripto",;
      ...
      oGet := ARRAY(4), aCor, nDescuP, lPreciosFinales := .f.
   DEFINE DIALOG oForm TITLE "Emision de presupuesto";
   ...
   IF oApp:presudiscriva
      ...
      @ 35, 50 CHECKBOX oGet[3] VAR lPreciosFinales OF oForm PIXEL SIZE 200,12 PROMPT "Precios Finales"
   ENDIF      
   acor := AcepCanc(oForm)
   @ acor[1],acor[2] BUTTON oBot[1] PROMPT "&Imprimir" OF oForm SIZE 30,10 ;
           ACTION ((lRta := .t.), oForm:End() ) PIXEL
   ...
   ** PRESUPUESTOS
   ...
   ENDIF   
   IF oQryPun:dosenuno
      P2x1ReimprimirPresu(oQryPresu, oQryDet, oQryCli, cNumcomp, dFecVen, lDolar,;
                          lImpSinIva, lPreciosFinales, config, aiva)
      oApp:cDestinoMail := " "
      RETURN nil
   ENDIF
   PRINT oPrn NAME "Presupuesto "+cNumcomp PREVIEW MODAL
   ...
   FOR x := 1 TO 2
     ...
     *                   oPrn:nHorzRes(), oPrn:nVertRes() ,   nil, .t. )
     ...
     oPrn:CmBox( 1.5, 9.4, 2.7, 10.7 )   // Box cuadrito comprobante
     oPrn:CmSay( 1.5, 9.8, "X",oFont2)
     oPrn:CmSay( 2.0, 16, "Nº: "+cNumcomp,oFont)         
     ...
     FOR i = 1 TO oQryDet:nRecCount                      
         ...
         IF !oApp:presudiscriva
             ...
             IF !lPreciosFinales
                ...
                ELSE
                @ y-0.07, 14 PRINT TO oPrn TEXT STR(oQryDet:importe/oQryDet:cantidad/IF(lDolar,oApp:dolar,1),12,2) ;
                     SIZE 2,.5 CM FONT oFont ALIGN "TR"
                @ y-0.07, 17 PRINT TO oPrn TEXT STR(oQryDet:importe/IF(lDolar,oApp:dolar,1),12,2);
                     SIZE 3,.5 CM FONT oFont ALIGN "TR"
             ENDIF
...
IF nCod <= 0 .OR. nTipoDoc = 3
   RETURN .t.
ENDIF   
nCanti := oApp:oServer:Query("SELECT SUM(cantidad) as canti FROM ventas_det_H WHERE codart ="+str(nCod)):canti
nCanti := nCanti + nVal
...
LOCAL cText, oQryTem, nNeto, nIva, nAux, i, nCurCodart, nTierPrio, nCodForma, nTot, oQryFp, oQryIva
STATIC cPrioChk := ""
...
*****************************************************************************
*** Impresion 2x1 (A4 apaisado, original y duplicado lado a lado).
*** Activado por la presencia del archivo 2x1.txt en la carpeta del sistema.
*** Cada hoja fisica contiene un par de copias (ORIGINAL+DUPLICADO,
*** TRIPLICADO+CUADRUPLICADO, etc.). Si el detalle no entra en una mitad,
*** se abre nueva hoja replicando el encabezado en ambos lados.
*****************************************************************************

STATIC FUNCTION P2x1FacturaElectronica(cTipoDoc, cNumComp, oQryVen1, oQryDet1, oQryIvaDet,;
                                       oQryP, oQryPar1, config, cCondVen, cFormaPag,;
                                       nIndiceDolar, cSigno, cDomicilioFiscal, aiva)
LOCAL i, x, y, oPrn, nRow, oFont, oFont1, oFont2, oFont3, oFont4, oFont5
LOCAL nBultos, lImprimio_promo, nDesFac
LOCAL nFx := 0.7, nL, nOffX, nHorzMid
LOCAL aOriginal := {"ORIGINAL","DUPLICADO","TRIPLICADO","CUADRUPLICADO","QUINTUPLICADO","SEXTUPLICADO"}
LOCAL nTotalCopias, nPar, nPares, nIdxL, nIdxR, nLadoTope, nIdxCopia
LOCAL nHoja, nConfigFon

   nConfigFon := config:fon
   IF nConfigFon < 14
      nConfigFon := nConfigFon * 2.8
   ENDIF
   DEFINE FONT oFont   NAME "ARIAL"   SIZE nConfigFon*nFx,      nConfigFon*2.5*nFx
   DEFINE FONT oFont1  NAME "CALIBRI" SIZE nConfigFon*1.5*nFx,  nConfigFon*4*nFx   BOLD
   DEFINE FONT oFont2  NAME "CALIBRI" SIZE nConfigFon*4*nFx,    nConfigFon*7*nFx   BOLD
   DEFINE FONT oFont3  NAME "ARIAL"   SIZE nConfigFon*nFx,      nConfigFon*2.5*nFx BOLD
   DEFINE FONT oFont5  NAME "ARIAL"   SIZE nConfigFon*.8*nFx,   nConfigFon*1.9*nFx BOLD
   AddFontResource("i2of5txt.ttf")
   DEFINE FONT oFont4  NAME "Interleaved 2of5 Text" SIZE nConfigFon*1.5*nFx, nConfigFon*5*nFx

   nTotalCopias := IF(oQryP:copiafa=1, 2, oQryP:copiafa)
   nPares := INT((nTotalCopias+1)/2)

   PRINT oPrn NAME "Factura "+cTipoDoc+cNumComp PREVIEW MODAL
   oPrn:SetLandscape()
   oPrn:SetPage(9)
   nHorzMid := oPrn:nHorzRes()/2

   FOR nPar := 1 TO nPares
       nIdxL := (nPar-1)*2 + 1
       nIdxR := IF(nIdxL+1 <= nTotalCopias, nIdxL+1, 0)
       nLadoTope := IF(nIdxR>0, 2, 1)
       oQryDet1:GoTop()
       nDesFac := 0
       nBultos := 0
       lImprimio_promo := .f.
       ...
       PAGE
          @ 0,0 PRINT TO oPrn IMAGE "fondofac.jpg" SIZE nHorzMid, oPrn:nVertRes() PIXEL STRETCH GRAY
          IF nIdxR > 0
             @ 0, nHorzMid PRINT TO oPrn IMAGE "fondofac.jpg" SIZE nHorzMid, oPrn:nVertRes() PIXEL STRETCH GRAY
          ENDIF
          FOR nL := 1 TO nLadoTope
              nOffX := IF(nL=1, 0, 14.85)
              nIdxCopia := IF(nL=1, nIdxL, nIdxR)
              @ 25*nFx, 1*nFx + nOffX PRINT TO oPrn IMAGE "https://app.bcnsoft.com.ar/img/arca.jpg" SIZE 2.5*nFx, 2.2*nFx CM
              oPrn:CmBox( .5*nFx,  .5*nFx + nOffX, 1.5*nFx, 20.5*nFx + nOffX )
              oPrn:CmBox( 1.5*nFx, .5*nFx + nOffX, 5*nFx,   20.5*nFx + nOffX )
              oPrn:CmBox( 5.3*nFx, .5*nFx + nOffX, 7.5*nFx, 20.5*nFx + nOffX )
              oPrn:CmBox( 8*nFx,   .5*nFx + nOffX, 9*nFx,   20.5*nFx + nOffX )
              oPrn:CmBox( 22*nFx,  .5*nFx + nOffX, 25*nFx,  20.5*nFx + nOffX )
              @ .8*nFx, 1.15*nFx + nOffX PRINT TO oPrn TEXT aOriginal[nIdxCopia] ;
                       SIZE 18*nFx,.9*nFx CM FONT oFont1 ALIGN "C"
              IF config:x1 = 2 .or. config:x1 = 3
                 @ 1.6*nFx, .6*nFx + nOffX PRINT TO oPrn IMAGE "logo.jpg" SIZE 8*nFx, 3*nFx CM
              ENDIF
              IF config:x1 = 1 .or. config:x1 = 3
                 @ 2*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oApp:nomb_emp) ;
                          SIZE 9*nFx,1*nFx CM FONT oFont1 ALIGN "C" LASTROW nRow
                 @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT "Domicilio Comercial:"+cDomicilioFiscal ;
                          SIZE 9*nFx,1*nFx CM FONT oFont LASTROW nRow ALIGN "C"
                 @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT aiva[oApp:tipo_iva] ;
                          SIZE 9*nFx,1*nFx CM FONT oFont LASTROW nRow ALIGN "C"
              ENDIF
              oPrn:CmSay( 1.7*nFx, 11*nFx + nOffX, IF(cTipoDoc="FC" .OR. cTipoDoc='FR',"FACTURA",;
                                                  IF(cTipoDoc="NC","NOTA DE CREDITO","NOTA DE DEBITO")), oFont1 )
              oPrn:CmBox( 1.5*nFx, 9.4*nFx + nOffX, 2.7*nFx, 10.7*nFx + nOffX )
              oPrn:CmSay( 1.5*nFx, 9.8*nFx + nOffX, LEFT(cNumComp,1), oFont2 )
              oPrn:CmSay( 2.3*nFx, 9.5*nFx + nOffX, "Cod:"+oQryVen1:tipfor, oFont )
              oPrn:CmSay( 2.5*nFx, 11*nFx + nOffX, "Punto de venta:"+SUBSTR(cNumcomp,2,4) + " Comprobante Nro:"+RIGHT(cNumcomp,8), oFont )
              oPrn:CmSay( 3.0*nFx, 11*nFx + nOffX, "Fecha de emision:"+DTOC(oQryVen1:fecmod)+" "+ALLTRIM(oQryVen1:hora), oFont )
              oPrn:CmSay( 3.5*nFx, 11*nFx + nOffX, "CUIT:"+oApp:cuit_emp, oFont )
              oPrn:CmSay( 4.0*nFx, 11*nFx + nOffX, "Ingresos brutos:"+oApp:ingb_emp, oFont )
              oPrn:CmSay( 4.5*nFx, 11*nFx + nOffX, "Inicio de Actividades:"+DTOC(oApp:inac_emp), oFont )
              @ 5.5*nFx, 1*nFx + nOffX   PRINT TO oPrn TEXT "C.U.I.T.:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 5.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT oQryVen1:cuit ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 5.5*nFx, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Razon Social:" ;
                       SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 5.5*nFx, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryVen1:nombre)+" ("+ALLTRIM(str(oQryVen1:codcli))+")" ;
                       SIZE 8*nFx,1*nFx CM FONT oFont LASTROW nRow
              @ 6*nFx, 1*nFx + nOffX  PRINT TO oPrn TEXT "Condicion IVA:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 6*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT aIva[oQryVen1:coniva] ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              nRow := IF(nRow<6*nFx, 6*nFx, nRow)
              @ nRow, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Direccion:" ;
                       SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ nRow, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryVen1:direccion) + " " + ;
                                                ALLTRIM(oQryVen1:localidad) + " "+ ALLTRIM(oQryVen1:codpos) ;
                       SIZE 8*nFx,1*nFx CM FONT oFont
              @ 6.5*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Condicion venta:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 6.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(cCondVen) ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 7*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Forma de pago:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 7*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(cFormaPag) ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 8.2*nFx, 0.7*nFx + nOffX PRINT TO oPrn TEXT "Codigo" ;
                       SIZE 2.5*nFx,.9*nFx CM FONT oFont5 ALIGN "L"
              @ 8.2*nFx, 3.5*nFx + nOffX PRINT TO oPrn TEXT "Descripcion del producto" ;
                       SIZE 6.5*nFx,.9*nFx CM FONT oFont5 ALIGN "L"
              @ 8.2*nFx, 10*nFx + nOffX PRINT TO oPrn TEXT "Cantidad" ;
                       SIZE 2*nFx,.9*nFx CM FONT oFont5 ALIGN "R"
              IF oApp:usabultosventa
                 @ 8.2*nFx, 12.05*nFx + nOffX PRINT TO oPrn TEXT "Btos" ;
                       SIZE .5*nFx,.9*nFx CM FONT oFont5 ALIGN "R"
              ENDIF
              @ 8.2*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT "Pr.Unitario" ;
                       SIZE 2*nFx,.9*nFx CM FONT oFont5 ALIGN "R"
              @ 8.2*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT "Desc." ;
                       SIZE 1.5*nFx,.9*nFx CM FONT oFont5 ALIGN "C"
              @ 8.2*nFx, 15.5*nFx + nOffX PRINT TO oPrn TEXT "Subtotal" ;
                       SIZE 2*nFx,.9*nFx CM FONT oFont5 ALIGN "R"
              @ 8.2*nFx, 17.5*nFx + nOffX PRINT TO oPrn TEXT "Alic. IVA" ;
                       SIZE 1*nFx,.9*nFx CM FONT oFont5 ALIGN "C"
              @ 8.2*nFx, 18.5*nFx + nOffX PRINT TO oPrn TEXT "Subtotal con IVA Incl." ;
                       SIZE 2*nFx,.9*nFx CM FONT oFont5 ALIGN "C"
          NEXT nL
          ...
          FOR i := 1 TO oQryDet1:nRecCount
              IF oQryDet1:punit < 0 .and. !lImprimio_promo
                 FOR nL := 1 TO nLadoTope
                     nOffX := IF(nL=1, 0, 14.85)
                     @ (y+.4)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Descuentos Promociones" ;
                                  SIZE 10*nFx,1*nFx CM FONT oFont ALIGN "TC" LASTROW nRow
                 NEXT nL
                 lImprimio_promo := .t.
                 y := (nRow / nFx) + .3
              ENDIF
              FOR nL := 1 TO nLadoTope
                  nOffX := IF(nL=1, 0, 14.85)
                  IF oApp:impcodpro
                     @ (y-0.07)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT IF(EMPTY(oQryDet1:codigopro),STR(oQryDet1:codart,13),LEFT(oQryDet1:codigopro,13)) ;
                              SIZE 2.5*nFx,.5*nFx CM FONT oFont5 ALIGN "TL"
                  ELSE
                     @ (y-0.07)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT STR(oQryDet1:codart,13) ;
                              SIZE 2.5*nFx,.5*nFx CM FONT oFont5 ALIGN "TL"
                  ENDIF
                  @ (y-0.07)*nFx, 3.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryDet1:detart)+" "+ALLTRIM(oQryDet1:condicioncompra) ;
                              SIZE 6.5*nFx,2.5*nFx CM FONT oFont5 LASTROW nRow ALIGN "TL"
                  @ (y-0.07)*nFx, 10*nFx + nOffX PRINT TO oPrn TEXT STR(oQryDet1:cantidad,10,2) ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
                  IF oApp:usabultosventa
                     @ (y-0.07)*nFx, 12.05*nFx + nOffX PRINT TO oPrn TEXT;
                          IF(INT(oQryDet1:bultos) <> oQryDet1:bultos,STR(oQryDet1:bultos,06,1),STR(oQryDet1:bultos,4)) ;
                              SIZE .5*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
                  ENDIF
                  @ (y-0.07)*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(IF(!LEFT(cNumComp,1)$"AM",oQryDet1:punit/nIndiceDolar ,oQryDet1:neton/oQryDet1:cantidad/nIndiceDolar),"@E 999,999,999.99") ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
                  @ (y-0.07)*nFx, 14.2*nFx + nOffX PRINT TO oPrn TEXT IF(oQryDet1:descupor > 0,;
                              ALLTRIM(IF(oQryDet1:descupor = INT(oQryDet1:descupor),;
                                         STR(INT(oQryDet1:descupor),3),;
                                         STR(oQryDet1:descupor,6,2)))+"% "+;
                              IF(oQryPar1:imprimedescuimp, ALLTRIM(TRANSFORM(oQryDet1:descu,"@E 999,999,999.99")),"")," ") ;
                              SIZE 1.8*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
                  @ (y-0.07)*nFx, 15.5*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet1:subtot/nIndiceDolar,"@E 999,999,999.99") ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
                  @ (y-0.07)*nFx, 17.7*nFx + nOffX PRINT TO oPrn TEXT oQryDet1:alicu ;
                              SIZE .7*nFx,.5*nFx CM FONT oFont5 ALIGN "TC"
                  @ (y-0.07)*nFx, 18.3*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet1:subtotalciva/nIndiceDolar,"@E 999,999,999.99") ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
              NEXT nL
              IF oApp:usabultosventa
                 nBultos := nBultos + oQryDet1:bultos
              ...
              nDesFac := nDesFac + oQryDet1:descu
              oQryDet1:Skip()
              y := (nRow / nFx) + .1 + config:salto/10
              IF y > 21 .and. !oQryDet1:Eof()
                 FOR nL := 1 TO nLadoTope
                     nOffX := IF(nL=1, 0, 14.85)
                     @ 23*nFx, 5*nFx + nOffX PRINT TO oPrn TEXT "CONTINUA EN OTRA HOJA" ;
                                  SIZE 9*nFx,2*nFx CM FONT oFont
                 NEXT nL
                 ENDPAGE
                 nHoja ++
                 PAGE
                    @ 0,0 PRINT TO oPrn IMAGE "fondofac.jpg" SIZE nHorzMid, oPrn:nVertRes() PIXEL GRAY
                    IF nIdxR > 0
                       @ 0, nHorzMid PRINT TO oPrn IMAGE "fondofac.jpg" SIZE nHorzMid, oPrn:nVertRes() PIXEL GRAY
                    ENDIF
                    FOR nL := 1 TO nLadoTope
                        nOffX := IF(nL=1, 0, 14.85)
                        nIdxCopia := IF(nL=1, nIdxL, nIdxR)
                        ...
                        oPrn:CmBox( 8*nFx,   .5*nFx + nOffX, 9*nFx,   20.5*nFx + nOffX )
                        oPrn:CmBox( 22*nFx,  .5*nFx + nOffX, 25*nFx,  20.5*nFx + nOffX )
                        @ .8*nFx, 1.15*nFx + nOffX PRINT TO oPrn TEXT aOriginal[nIdxCopia]+' Hoja:' + STR(nHoja,1) ;
                                 SIZE 18*nFx,.9*nFx CM FONT oFont1 ALIGN "C"
                        IF config:x1 = 2 .or. config:x1 = 3
                           @ 1.6*nFx, .6*nFx + nOffX PRINT TO oPrn IMAGE "logo.jpg" SIZE 8*nFx, 3*nFx CM
                        ENDIF
                        ...
                        @ 7*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(cFormaPag) ;
                                 SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
                        @ 8.2*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Codigo" ;
                                 SIZE 2.8*nFx,.5*nFx CM FONT oFont ALIGN "L"
                        @ 8.2*nFx, 4*nFx + nOffX PRINT TO oPrn TEXT "Descripcion del producto" ;
                                 SIZE 7*nFx,.5*nFx CM FONT oFont ALIGN "L"
                        @ 8.2*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Cantidad" ;
                                 SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "R"
                        @ 8.2*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT "Pr.Unitario" ;
                                 SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "R"
                        @ 8.2*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT "Subtotal" ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
                    NEXT nL
                    y := 9.2
              ENDIF
          NEXT i
          ...
          FOR nL := 1 TO nLadoTope
              nOffX := IF(nL=1, 0, 14.85)
              @ (y+.5)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryVen1:observa) + IF(nDesFac>0," Desc."+cSigno+" "+STR(nDesFac/nIndiceDolar,12,2)+ " (IVA Incl.) "," ") ;
                          SIZE 18*nFx,3*nFx CM FONT oFont ALIGN "L"
              IF oApp:usabultosventa
                 @ (y+.5)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Total Bultos:" + STR(nBultos,4) ;
                          SIZE 3*nFx,1*nFx CM FONT oFont ALIGN "L"
              ENDIF
              IF oApp:usareparto
                 @ 27*nFx, 13*nFx + nOffX PRINT TO oPrn TEXT CodigoBarra(STRTRAN(STR(oQryVen1:codbarra,11)," ","0")) ;
                          SIZE 6*nFx,1*nFx CM FONT oFont4 ALIGN "R"
              ENDIF
              @ 22.5*nFx, 1*nFx + nOffX  PRINT TO oPrn TEXT "Percepcion IIBB "+STR(oQryVen1:percep,6,2)+"% "+cSigno+":" ;
                          SIZE 5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 22.5*nFx, 6.1*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryVen1:IIBB/nIndiceDolar,"@E 999,999,999.99") ;
                          SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
              IF !LEFT(cNumComp,1)$"AM"
                 @ 22.5*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT "Subtotal "+cSigno+":" ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ 22.5*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM((oQryVen1:importe-oQryVen1:sobretasa)/nIndiceDolar,"@E 999,999,999.99") ;
                          SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
                 @ 23*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT "Otros Tributos "+cSigno+":" ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ 23*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryVen1:sobretasa/nIndiceDolar,"@E 999,999,999.99") ;
                          SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
                 @ 23.5*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT "Total "+cSigno+":" ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ 23.5*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryVen1:importe/nIndiceDolar,"@E 999,999,999.99") ;
                          SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
                 @ 23.7*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Regimen de transparencia fiscal Ley 27.743" ;
                          SIZE 7*nFx,.5*nFx CM FONT oFont5 ALIGN "R" LASTROW nRow
                 @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT "IVA Contenido: $"+TRANSFORM(oQryVen1:iva/nIndiceDolar,"@E 999,999,999.99") + " " ;
                          SIZE 7*nFx,.5*nFx CM FONT oFont5 ALIGN "R"
              ELSE
                 @ 22.5*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT "Subtotal "+cSigno+":" ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ 22.5*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryVen1:neto/nIndiceDolar,"@E 999,999,999.99") ;
                          SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
                 @ 23*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT "Otros Tributos "+cSigno+":" ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ 23*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryVen1:sobretasa/nIndiceDolar,"@E 999,999,999.99") ;
                 ...
                 oQryIvaDet:GoTop()
                 FOR i := 1 TO oQryIvaDet:nRecCount
                     @ y*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT "I.V.A. "+ALLTRIM(oQryIvaDet:nomiva) + " "+cSigno+":" ;
                              SIZE 4*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                     @ y*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryIvaDet:iva/nIndiceDolar,"@E 999,999,999.99") ;
                              SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
                     oQryIvaDet:Skip()
                     y := y + .5
                 NEXT i
                 @ y*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT "Total "+cSigno+":" ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ y*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryVen1:importe/nIndiceDolar,"@E 999,999,999.99") ;
                          SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
              ENDIF
              IF oQryPar1:imprimevendedor
                 @ 23.0*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT IF(!empty(oQryVen1:vendedor),"Vendedor: "+ALLTRIM(oQryVen1:vendedor),"") ;
                          SIZE 5*nFx,.5*nFx CM FONT oFont3
              ENDIF
              @ 25.5*nFx, 13*nFx + nOffX PRINT TO oPrn TEXT "CAE Nro:" ;
                       SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 25.5*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT oQryVen1:cae ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 26*nFx, 13*nFx + nOffX PRINT TO oPrn TEXT "Fecha Vto CAE:" ;
                       SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 26*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT DTOC(oQryVen1:fecVto) ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 25.5*nFx, 6*nFx + nOffX PRINT TO oPrn IMAGE "FactQR.bmp" SIZE 2.5*nFx, 2.5*nFx CM NOTRANSPARENT LASTROW nRow
              IF oQryVen1:endolares
                 @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT "Total expresado en PESOS ARGENTINOS, T/C "+STR(oQryVen1:Cotiza,12,2)+" = $ "+STR(oQryVen1:importe ,13,2) ;
                          SIZE 16*nFx,1*nFx CM FONT oFont5 ALIGN "L" LASTROW nRow
              ENDIF
              IF oApp:usar_puntos .and. oQryVen1:codcli > 1
                 @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT "(Puntos venta: "+STR(oQryVen1:puntos,8)+;
                              " - Acumulados: "+STR(oQryVen1:puntosacu,8)+;
                              " - Total: "+STR(oQryVen1:puntos+oQryVen1:puntosacu ,8) ;
                          SIZE 16*nFx,1*nFx CM FONT oFont5 ALIGN "L"
              ENDIF
          NEXT nL
       ENDPAGE
   NEXT nPar
...
STATIC FUNCTION P2x1Presupuesto(oQryDet, oGet, oBrw, lDolar, dFecVen, cNumcomp,;
                                lImpSinIva, lPreciosFinales, oQryP, config, aiva)
LOCAL i, x, y, oPrn, nRow, oFont, oFont1, oFont2, oFont3, cCodigopro, nIva
LOCAL nFx := 0.7, nL, nOffX, nHorzMid, cFondo
LOCAL nTotalCopias, nPar, nPares, nIdxL, nIdxR, nLadoTope, nIdxCopia
LOCAL aOriginal := {"ORIGINAL","DUPLICADO","TRIPLICADO","CUADRUPLICADO","QUINTUPLICADO","SEXTUPLICADO"}
LOCAL nHoja, nConfigFon
   ...
   cFondo := IF(FILE("fondopre.jpg"),"fondopre.jpg","fondofac.jpg")
   nTotalCopias := MAX(oQryP:copiafb, 1)
   IF nTotalCopias = 1
      nTotalCopias := 2
   ENDIF
   nPares := INT((nTotalCopias+1)/2)
   ...
   FOR nPar := 1 TO nPares
       ...
       PAGE
          @ 0,0 PRINT TO oPrn IMAGE cFondo SIZE nHorzMid, oPrn:nVertRes() PIXEL GRAY
          IF nIdxR > 0
             @ 0, nHorzMid PRINT TO oPrn IMAGE cFondo SIZE nHorzMid, oPrn:nVertRes() PIXEL GRAY
          ENDIF
          FOR nL := 1 TO nLadoTope
              nOffX := IF(nL=1, 0, 14.85)
              nIdxCopia := IF(nL=1, nIdxL, nIdxR)
              ...
              IF config:x1 = 1 .or. config:x1 = 3
                 @ 2*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oApp:nomb_emp) ;
                          SIZE 9*nFx,1*nFx CM FONT oFont1 ALIGN "C" LASTROW nRow
                 @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT "Domicilio Comercial:"+oApp:dire_emp ;
                          SIZE 9*nFx,1*nFx CM FONT oFont LASTROW nRow ALIGN "C"
                 @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT aiva[oApp:tipo_iva] ;
                          SIZE 9*nFx,1*nFx CM FONT oFont LASTROW nRow ALIGN "C"
              ENDIF
              oPrn:CmSay( 1.7*nFx, 11*nFx + nOffX, "PRESUPUESTO", oFont1 )
              oPrn:CmBox( 1.5*nFx, 9.4*nFx + nOffX, 2.7*nFx, 10.7*nFx + nOffX )
              oPrn:CmSay( 1.5*nFx, 9.8*nFx + nOffX, "X", oFont2 )
              oPrn:CmSay( 2.0*nFx, 16*nFx + nOffX, "N: "+SUBSTR(cNumcomp,2,16), oFont )
              oPrn:CmSay( 2.5*nFx, 11*nFx + nOffX, "Fecha de emision:"+DTOC(DATE()), oFont )
              IF dFecVen <> nil
                 oPrn:CmSay( 3.0*nFx, 11*nFx + nOffX, "Fecha de vencimiento:"+DTOC(dFecVen), oFont )
              ENDIF
              oPrn:CmSay( 3.5*nFx, 11*nFx + nOffX, "CUIT:"+oApp:cuit_emp, oFont )
              oPrn:CmSay( 4.0*nFx, 11*nFx + nOffX, "Ingresos brutos:"+oApp:ingb_emp, oFont )
              oPrn:CmSay( 4.5*nFx, 11*nFx + nOffX, "Inicio de Actividades:"+DTOC(oApp:inac_emp), oFont )
              @ 5.5*nFx, 1*nFx + nOffX   PRINT TO oPrn TEXT "C.U.I.T.:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 5.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT oGet[12]:cText ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 5.5*nFx, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Razon Social:" ;
                       SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 5.5*nFx, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oGet[9]:cText) + " ("+ALLTRIM(oGet[8]:cText)+")" ;
                       SIZE 8*nFx,1*nFx CM FONT oFont LASTROW nRow
              @ 6*nFx, 1*nFx + nOffX  PRINT TO oPrn TEXT "Condicion IVA:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 6*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT aIva[oGet[13]:nAt] ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              nRow := IF(nRow<6*nFx, 6*nFx, nRow)
              @ nRow, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Direccion:" ;
                       SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ nRow, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oGet[10]:cText) + " " + ALLTRIM(oGet[11]:cText) ;
                       SIZE 8*nFx,1*nFx CM FONT oFont
              @ 6.5*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Forma de Pago:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 6.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT aFormaNom[nFormaPago] ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 8.2*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Codigo" ;
                       SIZE 2.8*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 8.2*nFx, 4*nFx + nOffX PRINT TO oPrn TEXT "Descripcion del producto" ;
              ...
              @ 8.2*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Cantidad" ;
                       SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "R"
              @ 8.2*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT "Pr.Unit."+IF(oApp:presudiscriva .and. !lImpSinIva .and. !lPreciosFinales," S/IVA","") ;
                       SIZE 2.1*nFx,.5*nFx CM FONT oFont ALIGN "R"
              @ 8.2*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT "Subtotal"+IF(oApp:presudiscriva .and. !lImpSinIva .and. !lPreciosFinales," S/IVA","") ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
          NEXT nL
          ...
          FOR i := 1 TO oQryDet:nRecCount
              ...
              FOR nL := 1 TO nLadoTope
                  nOffX := IF(nL=1, 0, 14.85)
                  IF oApp:impcodpro
                     @ (y-0.07)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT IF(EMPTY(cCodigopro),STR(oQryDet:codart,13),ALLTRIM(cCodigopro)) ;
                              SIZE 2.9*nFx,.5*nFx CM FONT oFont ALIGN "TL"
                  ELSE
                     @ (y-0.07)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT STR(oQryDet:codart,13) ;
                              SIZE 2.9*nFx,.5*nFx CM FONT oFont ALIGN "TL"
                  ENDIF
                  @ (y-0.07)*nFx, 4*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryDet:detart) ;
                              SIZE 7*nFx,2.5*nFx CM FONT oFont LASTROW nRow ALIGN "TL"
                  @ (y-0.07)*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT STR(oQryDet:cantidad,08,2) ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                  IF !oApp:presudiscriva
                     @ (y-0.07)*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:punit/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                     @ (y-0.07)*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:ptotal/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                              SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                  ELSE
                     IF !lPreciosFinales
                        @ (y-0.07)*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:stotal/oQryDet:cantidad/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                                 SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                        @ (y-0.07)*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:stotal/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                     ELSE
                        @ (y-0.07)*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:ptotal/oQryDet:cantidad/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                                 SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                        @ (y-0.07)*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:ptotal/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                     ENDIF
                  ENDIF
              NEXT nL
              IF oApp:presudiscriva .and. !lPreciosFinales
                 nIva := nIva + oQryDet:ptotal - oQryDet:stotal
              ENDIF
              oQryDet:Skip()
              y := (nRow / nFx) + .1 + config:salto/10
              IF y > 21 .and. !oQryDet:Eof()
                 FOR nL := 1 TO nLadoTope
                     nOffX := IF(nL=1, 0, 14.85)
                     @ 23*nFx, 5*nFx + nOffX PRINT TO oPrn TEXT "CONTINUA EN OTRA HOJA" ;
                                  SIZE 9*nFx,2*nFx CM FONT oFont
                 ...
                 PAGE
                    ...
                    FOR nL := 1 TO nLadoTope
                        ...
                        nIdxCopia := IF(nL=1, nIdxL, nIdxR)
                        oPrn:CmBox( .5*nFx, .5*nFx + nOffX, 1.5*nFx, 20.5*nFx + nOffX )
                        @ .8*nFx, 1.15*nFx + nOffX PRINT TO oPrn TEXT aOriginal[nIdxCopia]+" (Hoja:"+STR(nHoja,2)+")" ;
                                 SIZE 18*nFx,.9*nFx CM FONT oFont1 ALIGN "C"
                        oPrn:CmBox( 1.5*nFx, .5*nFx + nOffX, 5*nFx,   20.5*nFx + nOffX )
                        oPrn:CmBox( 5.3*nFx, .5*nFx + nOffX, 7.5*nFx, 20.5*nFx + nOffX )
                        oPrn:CmBox( 8*nFx,   .5*nFx + nOffX, 9*nFx,   20.5*nFx + nOffX )
                        ...
                        IF config:x1 = 1 .or. config:x1 = 3
                           ...
                           @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT aiva[oApp:tipo_iva] ;
                                    SIZE 9*nFx,1*nFx CM FONT oFont LASTROW nRow ALIGN "C"
                        ENDIF
                        oPrn:CmSay( 2*nFx, 11*nFx + nOffX, "PRESUPUESTO", oFont1 )
                        oPrn:CmBox( 1.5*nFx, 9.4*nFx + nOffX, 2.7*nFx, 10.7*nFx + nOffX )
                        oPrn:CmSay( 1.5*nFx, 9.8*nFx + nOffX, "X", oFont2 )
                        oPrn:CmSay( 2.5*nFx, 11*nFx + nOffX, "Fecha de emision:"+DTOC(DATE()), oFont )
                        IF dFecVen <> nil
                        ...
                        @ nRow, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oGet[10]:cText) + " " + ALLTRIM(oGet[11]:cText) ;
                                 SIZE 8*nFx,1*nFx CM FONT oFont
                        @ 6.5*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Condicion:" ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                        @ 6.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT "Contado" ;
                                 SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
                        @ 8.2*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Codigo" ;
                                 SIZE 2.8*nFx,.5*nFx CM FONT oFont ALIGN "L"
                        @ 8.2*nFx, 4*nFx + nOffX PRINT TO oPrn TEXT "Descripcion del producto" ;
                        ...
                        @ 8.2*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Cantidad" ;
                                 SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "R"
                        @ 8.2*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT "Pr.Unit."+IF(oApp:presudiscriva .and. !lImpSinIva," S/IVA","") ;
                                 SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "R"
                        @ 8.2*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT "Subtotal"+IF(oApp:presudiscriva .and. !lImpSinIva," S/IVA","") ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R"
                    NEXT nL
                    y := 9.2
              ENDIF
          ...
          FOR nL := 1 TO nLadoTope
              nOffX := IF(nL=1, 0, 14.85)
              IF lDolar
                 oPrn:CmSay( y*nFx, 1*nFx + nOffX, "(Los Precios anteriores estan expresados en dolares americanos)", oFont )
              ENDIF
              IF lImpSinIva
                 oPrn:CmSay( (y+.5)*nFx, 1*nFx + nOffX, "(Los Precios anteriores NO INCLUYEN IVA)", oFont )
              ENDIF
              @ (y+1)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT oGet[23]:cText ;
                       SIZE 18*nFx,3*nFx CM FONT oFont ALIGN "L"
              IF oBrw:aCols[6]:nTotal > 0
                 @ 23.5*nFx, 1*nFx + nOffX  PRINT TO oPrn TEXT "Descuento General"+IF(lDolar,"u$s","$")+":" ;
                          SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ 23.5*nFx, 5*nFx + nOffX  PRINT TO oPrn TEXT TRANSFORM(oBrw:aCols[6]:nTotal/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont1 ALIGN "L"
              ENDIF
              IF !oApp:presudiscriva
                 @ 23.5*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Total presupuesto "+IF(lDolar,"u$s","$")+":" ;
                          SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ 23.5*nFx, 15*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oGet[21]:value/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont1 ALIGN "L"
              ELSE
                 IF !lImpSinIva
                    IF !lPreciosFinales
                       @ 23.5*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Tasa gral (IVA) "+IF(lDolar,"u$s","$")+":" ;
                                SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                       @ 23.5*nFx, 15*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(nIva/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                                SIZE 4*nFx,.5*nFx CM FONT oFont1 ALIGN "L"
                    ENDIF
                    @ 24.0*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Total presupuesto "+IF(lDolar,"u$s","$")+":" ;
                             SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                    @ 24.0*nFx, 15*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oGet[21]:value/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                             SIZE 4*nFx,.5*nFx CM FONT oFont1 ALIGN "L"
                 ELSE
                    @ 24.0*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Total presupuesto "+IF(lDolar,"u$s","$")+":" ;
                             SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                    @ 24.0*nFx, 15*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oGet[21]:value/IF(lDolar,oApp:dolar,1)-nIva/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                             SIZE 4*nFx,.5*nFx CM FONT oFont1 ALIGN "L"
                 ENDIF
              ENDIF
              @ 24.0*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Vendedor: "+ALLTRIM(oGet[36]:aItems[oGet[36]:nAt]) ;
                       SIZE 5*nFx,.5*nFx CM FONT oFont3
          NEXT nL
       ENDPAGE
   NEXT nPar
...
STATIC FUNCTION P2x1Remito(oQryDet, oGet, oBrw, cNumComp, nCodCli, cFactRemitada,;
                           oQryP, oQryCli, lRta, config, aiva, dFecha)
LOCAL i, x, y, oPrn, nRow, oFont, oFont1, oFont2, oFont3, cCodigopro, nBultos
LOCAL nFx := 0.7, nL, nOffX, nHorzMid, cFondo
LOCAL nTotalCopias, nPar, nPares, nIdxL, nIdxR, nLadoTope, nIdxCopia
LOCAL aOriginal := {"ORIGINAL","DUPLICADO","TRIPLICADO","CUADRUPLICADO","QUINTUPLICADO","SEXTUPLICADO"}
LOCAL nHoja, nConfigFon
   ...
   cFondo := IF(FILE("fondorem.jpg"),"fondorem.jpg","fondofac.jpg")
   nTotalCopias := MAX(oQryP:copiare, 1)
   IF nTotalCopias = 1
      nTotalCopias := 2
   ENDIF
   nPares := INT((nTotalCopias+1)/2)
       ...
       PAGE
              ...
              IF lRta
                 ...
                 IF config:x1 = 1 .or. config:x1 = 3
                    ...
                    @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT aiva[oApp:tipo_iva] ;
                             SIZE 9*nFx,1*nFx CM FONT oFont LASTROW nRow ALIGN "C"
                 ENDIF
                 oPrn:CmSay( 1.7*nFx, 11*nFx + nOffX, "REMITO", oFont1 )
                 oPrn:CmBox( 1.5*nFx, 9.4*nFx + nOffX, 2.7*nFx, 10.7*nFx + nOffX )
                 oPrn:CmSay( 1.5*nFx, 9.8*nFx + nOffX, "R", oFont2 )
                 oPrn:CmSay( 2.5*nFx, 11*nFx + nOffX, "Punto de venta:"+LEFT(cNumcomp,4) + " Remito Nro:"+RIGHT(cNumcomp,8), oFont )
                 oPrn:CmSay( 3.0*nFx, 11*nFx + nOffX, "Fecha de emision:"+DTOC(dFecha), oFont )
                 oPrn:CmSay( 3.5*nFx, 11*nFx + nOffX, "CUIT:"+oApp:cuit_emp, oFont )
                 oPrn:CmSay( 4.0*nFx, 11*nFx + nOffX, "Ingresos brutos:"+oApp:ingb_emp, oFont )
                 oPrn:CmSay( 4.5*nFx, 11*nFx + nOffX, "Inicio de Actividades:"+DTOC(oApp:inac_emp), oFont )
              ELSE
                 @ 3*nFx, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Fecha de emision:" SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "TR"
                 @ 3*nFx, 12.5*nFx + nOffX PRINT TO oPrn TEXT DTOC(DATE())  SIZE 8*nFx,1*nFx CM FONT oFont ALIGN "TL"
              ENDIF
              @ 5.5*nFx, 1*nFx + nOffX   PRINT TO oPrn TEXT "C.U.I.T.:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 5.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryCli:cuit) ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 5.5*nFx, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Razon Social:" ;
                       SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 5.5*nFx, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryCli:nombre) + " ("+ALLTRIM(STR(oQryCli:codigo))+")" ;
                       SIZE 8*nFx,1*nFx CM FONT oFont LASTROW nRow
              @ 6*nFx, 1*nFx + nOffX  PRINT TO oPrn TEXT "Condicion IVA:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 6*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT aIva[IF(oQryCli:coniva<>0,oQryCli:coniva,5)] ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              nRow := IF(nRow<6*nFx, 6*nFx, nRow)
              @ nRow, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Direccion:" ;
                       SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ nRow, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryCli:direccion) + " " + ALLTRIM(oQryCli:localidad) ;
                       SIZE 8*nFx,1*nFx CM FONT oFont
              @ 8.2*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Codigo" ;
                       SIZE 2.8*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 8.2*nFx, 4*nFx + nOffX PRINT TO oPrn TEXT "Descripcion del producto" ;
                       SIZE 7*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 8.2*nFx, 15*nFx + nOffX PRINT TO oPrn TEXT "Cantidad" ;
                       SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "R"
              IF oApp:usabultosventa
                 @ 8.2*nFx, 18*nFx + nOffX PRINT TO oPrn TEXT "Btos" ;
                          SIZE 1*nFx,.5*nFx CM FONT oFont ALIGN "R"
              ENDIF
          NEXT nL
          ...
          FOR i := 1 TO oQryDet:nRecCount
              ...
              FOR nL := 1 TO nLadoTope
                  ...
                  @ (y-0.07)*nFx, 4*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryDet:detart) ;
                              SIZE 7*nFx,2.5*nFx CM FONT oFont LASTROW nRow ALIGN "TL"
                  @ (y-0.07)*nFx, 15*nFx + nOffX PRINT TO oPrn TEXT STR(oQryDet:cantidad,08,2) ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                  IF oApp:usabultosventa
                     @ (y-0.07)*nFx, 18*nFx + nOffX PRINT TO oPrn TEXT;
                          IF(INT(oQryDet:bultos) <> oQryDet:bultos,STR(oQryDet:bultos,06,1),STR(oQryDet:bultos,4)) ;
                              SIZE 1*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                  ENDIF
              NEXT nL
              IF oApp:usabultosventa
                 nBultos := nBultos + oQryDet:bultos
          ...
          FOR nL := 1 TO nLadoTope
              nOffX := IF(nL=1, 0, 14.85)
              IF oGet <> nil
                 @ y*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT oGet[23]:cText ;
                          SIZE 18*nFx,3*nFx CM FONT oFont ALIGN "L" LASTROW nRow
                 y := nRow / nFx
              ENDIF
              IF LEN(cFactRemitada) > 0
                 @ y*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Factura remitada: "+ALLTRIM(cFactRemitada) ;
                          SIZE 18*nFx,3*nFx CM FONT oFont ALIGN "L" LASTROW nRow
                 y := nRow / nFx
              ENDIF
              IF oApp:usabultosventa
                 @ y*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Total Bultos:"+STR(nBultos,4) ;
                          SIZE 3*nFx,1*nFx CM FONT oFont ALIGN "L"
              ENDIF
          NEXT nL
       ENDPAGE
...
STATIC FUNCTION P2x1FacturaNoFiscal(cTipoDoc, cNumComp, oQryVen1, oQryDet1, oQryIvaDet,;
                                    oQryP, oQryPar, config, cCondVen, cFormaPag, aiva)
LOCAL i, x, y, oPrn, nRow, oFont, oFont1, oFont2, oFont3, oFont4, oFont5
LOCAL nBultos, lImprimio_promo, nDesFac
LOCAL nFx := 0.7, nL, nOffX, nHorzMid, cFondo
...
LOCAL nHoja, nConfigFon
   ...
   cFondo := IF(oQryP:imprimeDat,"fondofac.jpg","fondonof.jpg")
   nTotalCopias := IF(oQryP:copiafb=1, 2, oQryP:copiafb)
   nPares := INT((nTotalCopias+1)/2)
       ...
       PAGE
          ...
          FOR nL := 1 TO nLadoTope
              ...
              IF oQryP:imprimeDat
                 ...
                 ENDIF
              ENDIF
              oPrn:CmSay( 1.7*nFx, 11*nFx + nOffX, ALLTRIM(oQryPar:textonofiscal)+IF(cTipoDoc="NC"," NC",""), oFont1 )
              oPrn:CmBox( 1.5*nFx, 9.4*nFx + nOffX, 2.7*nFx, 10.7*nFx + nOffX )
              oPrn:CmSay( 1.5*nFx, 9.8*nFx + nOffX, oQryPar:letranofiscal, oFont2 )
              oPrn:CmSay( 2.5*nFx, 11*nFx + nOffX, "Punto de venta:"+SUBSTR(cNumcomp,2,4) + " Comprobante Nro:"+RIGHT(cNumcomp,8), oFont )
              oPrn:CmSay( 3.0*nFx, 11*nFx + nOffX, "Fecha de emision:"+DTOC(oQryVen1:fecha)+" "+ALLTRIM(oQryVen1:hora), oFont )
              IF oQryP:imprimeDat
                 oPrn:CmSay( 3.5*nFx, 11*nFx + nOffX, "CUIT:"+oApp:cuit_emp, oFont )
                 oPrn:CmSay( 4.0*nFx, 11*nFx + nOffX, "Ingresos brutos:"+oApp:ingb_emp, oFont )
                 oPrn:CmSay( 4.5*nFx, 11*nFx + nOffX, "Inicio de Actividades:"+DTOC(oApp:inac_emp), oFont )
              ...
              @ 6*nFx, 1*nFx + nOffX  PRINT TO oPrn TEXT "Condicion IVA:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 6*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT aIva[oQryVen1:coniva] ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont
              nRow := IF(nRow<6*nFx, 6*nFx, nRow)
              @ nRow, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Direccion:" ;
                       SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ nRow, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryVen1:direccion) + " " + ALLTRIM(oQryVen1:localidad) + " " + ALLTRIM(oQryVen1:codpos) ;
                       SIZE 8*nFx,1*nFx CM FONT oFont
              @ 6.5*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Condicion venta:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 6.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(cCondVen) ;
                  ...
                  IF oApp:usabultosventa
                     @ (y-0.07)*nFx, 12.05*nFx + nOffX PRINT TO oPrn TEXT;
                          IF(INT(oQryDet1:bultos) <> oQryDet1:bultos,STR(oQryDet1:bultos,06,1),STR(oQryDet1:bultos,4)) ;
                              SIZE .5*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
                  ENDIF
                  @ (y-0.07)*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(IF(LEFT(cNumComp,1)<>"A",oQryDet1:punit,oQryDet1:neton/oQryDet1:cantidad),"@E 999,999,999.99") ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
                  @ (y-0.07)*nFx, 14.2*nFx + nOffX PRINT TO oPrn TEXT IF(oQryDet1:descupor > 0,;
                              ALLTRIM(IF(oQryDet1:descupor = INT(oQryDet1:descupor),;
                                         STR(INT(oQryDet1:descupor),3),;
                              ...
                              IF(oQryPar:imprimedescuimp,ALLTRIM(TRANSFORM(oQryDet1:descu,"@E 999,999,999.99")),"")," ") ;
                              SIZE 1.8*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
                  @ (y-0.07)*nFx, 15.5*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet1:subtot,"@E 999,999,999.99") ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont5 ALIGN "TR"
              NEXT nL
              IF oApp:usabultosventa
                 nBultos := nBultos + oQryDet1:bultos
              ...
              IF y > 21 .and. !oQryDet1:Eof()
                 ...
                 PAGE
                    ...
                    FOR nL := 1 TO nLadoTope
                        ...
                        @ nRow, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Direccion:" ;
                                 SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                        @ nRow, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryVen1:direccion) + " " + ALLTRIM(oQryVen1:localidad) ;
                                 SIZE 8*nFx,1*nFx CM FONT oFont
                        @ 6.5*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Condicion venta:" ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                        @ 6.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(cCondVen) ;
          ...
          FOR nL := 1 TO nLadoTope
              ...
              ENDIF
              IF nDesFac > 0
                 @ (y+1)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(STRTRAN(oQryVen1:observa,"PUNTO DE VENTA","")) ;
                          SIZE 18*nFx,3*nFx CM FONT oFont LASTROW nRow ALIGN "L"
                 @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT "Descuento Gral:"+TRANSFORM(nDesFac,"@E 999,999,999.99") ;
                          SIZE 18*nFx,3*nFx CM FONT oFont ALIGN "L"
              ELSE
                 @ (y+1)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(STRTRAN(oQryVen1:observa,"PUNTO DE VENTA","")) ;
                          SIZE 18*nFx,3*nFx CM FONT oFont LASTROW nRow ALIGN "L"
              ...
              @ 23.0*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT IF(!empty(oQryVen1:vendedor),"Vendedor: "+ALLTRIM(oQryVen1:vendedor),"") ;
                       SIZE 5*nFx,.5*nFx CM FONT oFont3
              @ 23.5*nFx, 12*nFx + nOffX PRINT TO oPrn TEXT "Total $:" ;
                       SIZE 4*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 23.5*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryVen1:importe,"@E 999,999,999.99") ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "R" LASTROW nRow
              IF oApp:usar_puntos .and. oQryVen1:codcli > 1
                 @ nRow, 1*nFx + nOffX PRINT TO oPrn TEXT "(Puntos venta: "+STR(oQryVen1:puntos,8)+;
                              " - Acumulados: "+STR(oQryVen1:puntosacu,8)+;
                              " - Total: "+STR(oQryVen1:puntos+oQryVen1:puntosacu ,8) ;
...
STATIC FUNCTION P2x1ReimprimirPresu(oQryPresu, oQryDet, oQryCli, cNumcomp, dFecVen, lDolar,;
                                    lImpSinIva, lPreciosFinales, config, aiva)
LOCAL i, x, y, oPrn, nRow, oFont, oFont1, oFont2, oFont3, cCodigopro, nIva, nDescuP
LOCAL nFx := 0.7, nL, nOffX, nHorzMid, cFondo
LOCAL nTotalCopias := 2, nPar, nPares, nIdxL, nIdxR, nLadoTope, nIdxCopia
LOCAL aOriginal := {"ORIGINAL","DUPLICADO","TRIPLICADO","CUADRUPLICADO","QUINTUPLICADO","SEXTUPLICADO"}
LOCAL nHoja, nConfigFon
   ...
   FOR nPar := 1 TO nPares
       ...
       PAGE
          ...
          FOR nL := 1 TO nLadoTope
              ...
              oPrn:CmBox( 1.5*nFx, 9.4*nFx + nOffX, 2.7*nFx, 10.7*nFx + nOffX )
              oPrn:CmSay( 1.5*nFx, 9.8*nFx + nOffX, "X", oFont2 )
              oPrn:CmSay( 2.0*nFx, 16*nFx + nOffX, "N: "+cNumcomp, oFont )
              oPrn:CmSay( 2.5*nFx, 11*nFx + nOffX, "Fecha de emision:"+DTOC(oQryPresu:fecha), oFont )
              IF dFecVen <> nil
                 oPrn:CmSay( 3.0*nFx, 11*nFx + nOffX, "Fecha de vencimiento:"+DTOC(dFecVen), oFont )
              ENDIF
              oPrn:CmSay( 3.5*nFx, 11*nFx + nOffX, "CUIT:"+oApp:cuit_emp, oFont )
              ...
              @ 5.5*nFx, 1*nFx + nOffX   PRINT TO oPrn TEXT "C.U.I.T.:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 5.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT oQryPresu:cuit ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 5.5*nFx, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Razon Social:" ;
                       SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 5.5*nFx, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryPresu:nombre) ;
                       SIZE 8*nFx,1*nFx CM FONT oFont LASTROW nRow
              @ 6*nFx, 1*nFx + nOffX  PRINT TO oPrn TEXT "Condicion IVA:" ;
                       SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ 6*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT aIva[oQryPresu:coniva] ;
                       SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
              nRow := IF(nRow<6*nFx, 6*nFx, nRow)
              @ nRow, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Direccion:" ;
                       SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
              @ nRow, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryPresu:direccion) + " " + ALLTRIM(oQryPresu:localidad) ;
                       SIZE 8*nFx,1*nFx CM FONT oFont
              @ 8.2*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Codigo" ;
                       SIZE 2.8*nFx,.5*nFx CM FONT oFont ALIGN "L"
              @ 8.2*nFx, 4*nFx + nOffX PRINT TO oPrn TEXT "Descripcion del producto" ;
          ...
          FOR i := 1 TO oQryDet:nRecCount
              ...
              FOR nL := 1 TO nLadoTope
                  ...
                  IF !oApp:presudiscriva
                     @ (y-0.07)*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:punit/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                              SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                     @ (y-0.07)*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:importe/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                              SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                  ELSE
                     IF !lPreciosFinales
                        @ (y-0.07)*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:neto/oQryDet:cantidad/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                                 SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                        @ (y-0.07)*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:neto/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                     ELSE
                        @ (y-0.07)*nFx, 14*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:importe/oQryDet:cantidad/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                                 SIZE 2*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                        @ (y-0.07)*nFx, 17*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryDet:importe/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont ALIGN "TR"
                     ENDIF
              ...
              IF y > 21 .and. !oQryDet:Eof()
                 ...
                 PAGE
                    ...
                    FOR nL := 1 TO nLadoTope
                        ...
                        @ 5.5*nFx, 1*nFx + nOffX   PRINT TO oPrn TEXT "C.U.I.T.:" ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                        @ 5.5*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT oQryCli:cuit ;
                                 SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
                        @ 5.5*nFx, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Razon Social:" ;
                                 SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                        @ 5.5*nFx, 12.5*nFx + nOffX PRINT TO oPrn TEXT ALLTRIM(oQryCli:nombre) + " ("+ALLTRIM(STR(oQryCli:codigo))+")" ;
                        ...
                        @ 6*nFx, 1*nFx + nOffX  PRINT TO oPrn TEXT "Condicion IVA:" ;
                                 SIZE 3*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                        @ 6*nFx, 4.1*nFx + nOffX PRINT TO oPrn TEXT aIva[oQryCli:coniva] ;
                                 SIZE 6*nFx,.5*nFx CM FONT oFont ALIGN "L"
                        nRow := IF(nRow<6*nFx, 6*nFx, nRow)
                        @ nRow, 9.5*nFx + nOffX PRINT TO oPrn TEXT "Direccion:" ;
                                 SIZE 2.5*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
          ...
          FOR nL := 1 TO nLadoTope
              ...
              IF lImpSinIva
                 oPrn:CmSay( (y+.5)*nFx, 1*nFx + nOffX, "(Los Precios anteriores NO INCLUYEN IVA)", oFont )
              ENDIF
              @ (y+1)*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT oQryPresu:observa ;
                       SIZE 18*nFx,3*nFx CM FONT oFont ALIGN "L"
              IF nDescuP > 0
                 @ 23.5*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Descuento General"+IF(lDolar,"u$s","$")+":" ;
                          SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ 23.5*nFx, 5*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(nDescuP/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont1 ALIGN "L"
              ENDIF
              IF !oApp:presudiscriva .OR. lPreciosFinales
                 @ 23.5*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Total presupuesto "+IF(lDolar,"u$s","$")+":" ;
                          SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                 @ 23.5*nFx, 15*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryPresu:importe/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                          SIZE 4*nFx,.5*nFx CM FONT oFont1 ALIGN "L"
              ELSE
                 IF !lImpSinIva
                    @ 23.5*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Tasa gral (IVA) "+IF(lDolar,"u$s","$")+":" ;
                    ...
                    @ 24.0*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Total presupuesto "+IF(lDolar,"u$s","$")+":" ;
                             SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                    @ 24.0*nFx, 15*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryPresu:importe/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                             SIZE 4*nFx,.5*nFx CM FONT oFont1 ALIGN "L"
                 ELSE
                    @ 24.0*nFx, 11*nFx + nOffX PRINT TO oPrn TEXT "Total presupuesto "+IF(lDolar,"u$s","$")+":" ;
                             SIZE 3.9*nFx,.5*nFx CM FONT oFont3 ALIGN "R"
                    @ 24.0*nFx, 15*nFx + nOffX PRINT TO oPrn TEXT TRANSFORM(oQryPresu:importe/IF(lDolar,oApp:dolar,1)-nIva/IF(lDolar,oApp:dolar,1),"@E 999,999,999.99") ;
                             SIZE 4*nFx,.5*nFx CM FONT oFont1 ALIGN "L"
                 ENDIF
              ENDIF
              @ 24.0*nFx, 1*nFx + nOffX PRINT TO oPrn TEXT "Vendedor: "+ALLTRIM(oQryPresu:vendedor) ;
                       SIZE 5*nFx,.5*nFx CM FONT oFont3
          NEXT nL
       ENDPAGE
   NEXT nPar
