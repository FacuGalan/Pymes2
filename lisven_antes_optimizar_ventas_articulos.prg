...
LOCAL oRep, oFont1, oFont2, oFont3, oQry, oDlg1, oFont,;
      ...
      cTodos := "Todos los vendedores            ", mnomven := SPACE(30), ;
      mdesde := DATE(), mhasta := DATE(),mdesdeh := "00:00:00", mhastah := "23:59:59",;
      mvendedor := 0, aTipoForm, oQryTip, nTipo, lIva := .F., oQry1

aTipoForm := {"FC","NC","ND","  ","RE"}
nTipo := 4
DEFINE FONT oFont NAME "TAHOMA" SIZE 0,-11.5
DEFINE DIALOG oDlg1 TITLE "Listado de Ventas por fecha" FROM 03,15 TO 16,70 Of oApp:oWnd
   acor := AcepCanc(oDlg1)    
   @ 07, 01 SAY "Desde Fecha:"        OF oDlg1 PIXEL SIZE 60,10 RIGHT 
   @ 22, 01 SAY "Hasta Fecha:"        OF oDlg1 PIXEL SIZE 60,10 RIGHT 
   @ 37, 01 SAY "Desde Hora:"        OF oDlg1 PIXEL SIZE 60,10 RIGHT 
   @ 52, 01 SAY "Hasta Hora:"        OF oDlg1 PIXEL SIZE 60,10 RIGHT 
   @ 67, 01 SAY "Tipo Formulario:"    OF oDlg1 PIXEL SIZE 60,10 RIGHT 
   @ 05, 65 GET oGet[1] VAR mdesde    OF oDlg1 PIXEL
   @ 20, 65 GET oGet[2] VAR mhasta    OF oDlg1 PIXEL VALID(mhasta >= mdesde)
   @ 35, 65 GET oGet[4] VAR mdesdeh    OF oDlg1 PIXEL PICTURE "99:99:99" WHEN(nTipo <> 5)
   @ 50, 65 GET oGet[5] VAR mhastah    OF oDlg1 PIXEL PICTURE "99:99:99" VALID(mhastah >= mdesdeh) WHEN(nTipo <> 5)
   @ 65, 65 COMBOBOX oGet[5] VAR nTipo ITEMS aTipoForm OF oDlg1 SIZE 40,10 PIXEL WHEN(!lIva)
...
IF nTipo <> "RE"
    ...
    oQry := oApp:oServer:Query("SELECT CONCAT(v.ticomp,v.letra, v.numcomp) AS compro, v.fecha as fecha, v.hora as hora, "+;
                              " v.codcli as codcli, c.nombre AS cliente,c.cuit, v.neto*IF(v.ticomp='NC',-1,1) AS neto,"+;
                              " v.iva*IF(v.ticomp='NC',-1,1) AS iva, v.cae AS cae,"+;
                              "  v.importe*IF(v.ticomp='NC',-1,1) AS importe, v.vendedor AS vendedor, v.usuario as usuario, LEFT(v.observa,35) as observa "+;
                              ...
                              " v.hora >= " + ClipValue2Sql(mdesdeh) + " AND "+;
                              " v.hora <= "+ ClipValue2Sql(mhastah) + " AND "+;
    ...
    oQry1 := oApp:oServer:Query("SELECT SUM(v.importe*IF(v.ticomp='NC',-1,1)) AS importe, SUM(IF(v.ticomp='NC',-1,1)) AS casos"+;
    ...
    REPORT oRep TITLE IF(lIva,"Reporte I.V.A. Ventas","Reporte de Ventas " )  + ;
                      ...
                      " desde " + mdesdeh + " a " + mhastah + " hs";
           FONT  oFont1,oFont2,oFont3 ;
           HEADER OemToAnsi(oApp:nomb_emp) , ;
           "Ventas por fecha",;
           "Ticket Promedio :$" + ALLTRIM(STR(oQry1:importe / oQry1:casos,14,2)),; 
           "Cantidad tickets:" + ALLTRIM(STR(oQry1:casos,5)) CENTER ;
    ...
    COLUMN TITLE "Hora"      DATA oQry:hora      SIZE 07 FONT 1
    COLUMN TITLE "Nro"       DATA oQry:codcli    SIZE 05 FONT 1
    COLUMN TITLE "Cliente"   DATA oQry:cliente   SIZE 27 FONT 1
    IF lIva
       COLUMN TITLE "C.U.I.T."   DATA oQry:cuit   SIZE 13 FONT 1
       COLUMN TITLE "Neto "      DATA oQry:neto PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
       COLUMN TITLE "I.V.A."     DATA oQry:iva  PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
    ...
    COLUMN TITLE "Facturó"   DATA oQry:usuario    SIZE 08 FONT 1
    COLUMN TITLE "Vendedor"  DATA oQry:vendedor   SIZE 10 FONT 1
    COLUMN TITLE "Observaciones"  DATA oQry:observa   SIZE 22 FONT 1
    ...
    oQry := oApp:oServer:Query("SELECT DISTINCT(v.nrorem) AS compro, v.fecha as fecha,  "+;
                              " v.codcli as codcli, c.nombre AS cliente,c.cuit, IF(v.facturado,'Si','No') AS facturado,"+;
                              " v.factura AS factura, v.vendedor AS vendedor,  v.usuario as usuario "+;
                              " FROM ge_"+oApp:cId+"remitos v LEFT JOIN ge_"+oApp:cId+"clientes c ON v.codcli = c.codigo "+;
                              " WHERE v.fecha >= " + ClipValue2Sql(mdesde) + " AND "+;
                              " v.fecha <= "+ ClipValue2Sql(mhasta) +;
                              " ORDER BY v.fecha,v.nrorem" )

    REPORT oRep TITLE "Reporte de Remitos de Ventas del " + DTOC(mdesde) + " al " + DTOC(mhasta) ;
           FONT  oFont1,oFont2,oFont3 ;
           HEADER OemToAnsi(oApp:nomb_emp) , ;
           "Remitos por fecha" CENTER ;
           FOOTER "Hoja:" + STR(oRep:npage,3) ,"Fecha:"+DTOC(DATE()) CENTER;
           PREVIEW CAPTION  "Remitos por fecha"

    COLUMN TITLE "Nro.Remito" DATA oQry:compro    SIZE 15 FONT 1
    COLUMN TITLE "Fecha"      DATA oQry:fecha     PICTURE "@D" SIZE 08 FONT 1
    COLUMN TITLE "Nro"        DATA oQry:codcli    SIZE 05 FONT 1
    COLUMN TITLE "Cliente"    DATA oQry:cliente   SIZE 30 FONT 1
    COLUMN TITLE "Confeccion" DATA oQry:usuario    SIZE 10 FONT 1
    COLUMN TITLE "Vendedor"   DATA oQry:vendedor   SIZE 10 FONT 1
    COLUMN TITLE "Facturado?" DATA oQry:facturado   SIZE 10 FONT 1
...
COLUMN TITLE "Saldo"     DATA oQry:saldo PICTURE "9,999,999,999.99" ;
                         SIZE 10 FONT 2 TOTAL
...
PROCEDURE RepVen2()
LOCAL oRep, oFont1, oFont2, oFont3, oQry, oDlg1, oFont,;
      acor:= ARRAY(4), mrta:=.F., oGet:= ARRAY(15), oBot1 := ARRAY(5), oBot2,;
      ...
      cTodos := "TODOS                  ", oQryVen, cDesdeHora := "00:00:00", cHastaHora := "23:59:59", ;
      ...
      oQryPun, nCaja := 1, aCaja := {"0 TODAS LAS CAJAS"}, aCajaNr := {0}
...
oQryPun:= oApp:oServer:Query( "SELECT caja,nombre FROM ge_"+oApp:cId+"punto")  
DO WHILE !oQryPun:Eof()
   AADD(aCaja,STR(oQryPun:caja,4)+" "+oQryPun:Nombre)
   AADD(aCajaNr,oQryPun:caja)
   oQryPun:Skip()
ENDDO
...
DEFINE DIALOG oDlg1 TITLE "Ventas por articulo" FROM 03,15 TO 30,153
   ...
   @ 140,332 COMBOBOX oGet[12] VAR nCaja ITEMS aCaja OF oDlg1 SIZE 90,30 PIXEL WHEN(!lRanking .and. !lProv .and. !lPorFactura) 
   @ 142,425 SAY "Desde hora:" OF oDlg1 PIXEL SIZE 40,12 RIGHT
   @ 157,425 SAY "Hasta hora:" OF oDlg1 PIXEL SIZE 40,12 RIGHT
   @ 140,470 GET oGet[14] VAR cDesdeHora PICTURE "99:99:99" PIXEL OF oDlg1
   @ 155,470 GET oGet[15] VAR cHastaHora PICTURE "99:99:99" PIXEL OF oDlg1
...
IF !lProv .and. !lPorFactura    
    ...
    cSql := "SELECT v.codart AS codart, v.detart AS detart, a.codigopro as codigopro, d.nombre AS marca, "+;
            ...
            "v.nrofac AS numfac,v.cantidad * v.pcosto * IF(LEFT(v.nrofac,2)='NC',-1,1) AS costo, ve.hora as hora   FROM ventas_det_report v "
       ELSE
    cSql := "SELECT v.codart AS codart, v.detart AS detart, a.codigopro as codigopro, d.nombre AS marca,"+;
            ...
            "SUM(v.cantidad * v.pcosto * IF(LEFT(v.nrofac,2)='NC',-1,1)) AS costo, ve.hora as hora "+;
    ...
    IF(nCaja <> 0)
            cSql := cSql + ;        
            " LEFT JOIN (SELECT ticomp,letra,numcomp, caja FROM ge_"+oApp:cId+"concfact GROUP BY ticomp,letra,numcomp) cf "+;
            " ON cf.ticomp = LEFT(v.nrofac,2) AND cf.letra = SUBSTR(v.nrofac,3,1) AND RIGHT(v.nrofac,13) = cf.numcomp "
    ENDIF        
    cSql := cSql + ;        
            "WHERE v.fecha >= " + ClipValue2Sql(mdesde) +;
            "  AND v.fecha <= " + ClipValue2Sql(mhasta) +;
            "  AND ve.hora >= " + ClipValue2Sql(cDesdeHora)+;
            "  AND ve.hora <= " + ClipValue2Sql(cHastaHora)
    IF(cVendedor <> "TODOS")
       IF !oApp:usua_es_vendedor       
          cSql := cSql +" AND "+ IF(cVendedor = "TODOS","TRUE"," ve.vendedor = " + ClipValue2Sql(cVendedor)) 
          ELSE 
          cSql := cSql +" AND "+ IF(cVendedor = "TODOS","TRUE"," ve.usuario = " + ClipValue2Sql(cVendedor)) 
       ENDIF   
    ENDIF        
    ...
    IF nCaja <> 0 
       cSql := cSql + " AND cf.caja = " + STR(nCaja)
    ENDIF    
    IF lResu
       IF lRanking
    ...
    REPORT oRep TITLE "Articulos vendidos por fecha del " + ;
                      ...
                      " desde " + cDesdeHora + " a " + cHastaHora + " hs",;
                      IF(lRanking,"Ranking","")+" Vendedor:"+ALLTRIM(cVendedor) ;
    ...
    IF !lResu
       ...
       COLUMN TITLE "Fecha"     DATA oQry:fecha    PICTURE "@D"   SIZE 8 FONT 1
       COLUMN TITLE "Hora"      DATA oQry:hora     SIZE 7 FONT 1   
       ELSE
    ...
    ENDIF   
    COLUMN TITLE "Cantidad"  DATA oQry:cantidad PICTURE "9,999,999,999.99" ;
                                SIZE 10 FONT 2 TOTAL
    COLUMN TITLE "Importe"   DATA oQry:precio ;
                             PICTURE "9,999,999,999.99" SIZE 12 FONT 2 TOTAL
...
ELSE
    ...
    cSql := "SELECT v.codart AS codart, v.detart AS detart, a.codigopro as codigopro, c.nombre as nomcli, v.nrofac as nrofac,"+;
              ...
              "cf.observa as formapag, ve.hora "+;
              " FROM ventas_det_report v "
    cSql := cSql + ;              
              "LEFT JOIN ge_"+oApp:cId+"clientes c ON c.codigo = v.codcli "+;
              "LEFT JOIN ge_"+oApp:cId+"ventas_encab ve ON v.nrofac = CONCAT(ve.ticomp,ve.letra,ve.numcomp) "+; 
              ...
              "LEFT JOIN (SELECT CONCAT(ticomp,letra, numcomp) AS compro, "+;
                               ...
                               "FROM ge_"+oApp:cId+"concfact WHERE fecha>="+ClipValue2Sql(mdesde)+" AND "+;
                               " fecha <= "+ClipValue2Sql(mhasta)+;
                               " GROUP BY CONCAT(ticomp,letra, numcomp) ) cf "+;
                          " ON cf.compro = v.nrofac "+;  
              "WHERE v.fecha >= " + ClipValue2Sql(mdesde) +;
              "  AND v.fecha <= " + ClipValue2Sql(mhasta)+;
              " AND a.marca IN " + cMarca + " AND a.rubro IN " + cRubro+;
              " AND a.prov IN " + cProvee + " AND a.depto IN "+cDepto+ " AND a.empresa IN "+cEmpresa+;
    ...
    REPORT oRep TITLE "Articulos vendidos por fecha del " + ;
             ...
             FOOTER "Hoja:" + STR(oRep:npage,3) ,"Fecha:"+DTOC(DATE()) CENTER;
             PREVIEW CAPTION  "Articulos por fecha"
      GROUP oGru ON oQry:nrofac HEADER oQry:nrofac+" "+DTOC(oQry:fecha) + " "+oQry:nomcli + " "+oQry:formapag FOOTER "Totales Factura"  FONT 3       

      COLUMN TITLE "Articulo" DATA oQry:detart   SIZE 30 FONT 1
      COLUMN TITLE "Codigo"   DATA oQry:codart   SIZE 12 FONT 1
      COLUMN TITLE "Codigo Pro" DATA oQry:codigopro   SIZE 11 FONT 1
      COLUMN TITLE "Cantidad"  DATA oQry:cantidad PICTURE "999,999.99" ;
                                  SIZE 6 FONT 1 TOTAL
      COLUMN TITLE "Total"     DATA oQry:precio PICTURE "9,999,999,999.99" SIZE 12 FONT 2 TOTAL
      COLUMN TITLE "Neto"      DATA oQry:neto   PICTURE "9,999,999,999.99" SIZE 12 FONT 2 TOTAL
      COLUMN TITLE "IVA"       DATA oQry:iva    PICTURE "9,999,999,999.99" SIZE 10 FONT 2 TOTAL
      COLUMN TITLE "Imp.Int"   DATA oQry:impint PICTURE "9,999,999,999.99" SIZE 08 FONT 2 TOTAL
      COLUMN TITLE "Desc.%"    DATA oQry:descup PICTURE "999.99" SIZE 6 FONT 1
      COLUMN TITLE "Desc.$"    DATA oQry:descu  PICTURE "9,999,999,999.99" SIZE 10 FONT 2 TOTAL
      COLUMN TITLE "Utilidad"  DATA oQry:precio - oQry:costo PICTURE "9,999,999,999.99" SIZE 10 FONT 2 TOTAL
      COLUMN TITLE "Costo Unit" DATA oQry:costo / oQry:cantidad PICTURE "9,999,999,999.99" SIZE 10 FONT 1
      COLUMN TITLE "Hora"       DATA LEFT(oQry:hora,5) SIZE 05 FONT 1
      // Digo que el titulo lo escriba con al letra 2
      oRep:oTitle:aFont[1] := {|| 3 }
      oRep:bInit := {|| oQry:GoTop() }
      oRep:bSkip := {|| oQry:Skip() }
...
IF nTipo = 1     
   oQry := oApp:oServer:Query("SELECT SUM(IF(LEFT(v.nrofac,2)='NC',-v.cantidad,v.cantidad)) AS cantidad,  "+;
                         ...
                         " SUM(IF(LEFT(v.nrofac,2)='NC',-v.pcosto,v.pcosto)*v.cantidad) AS costo, "+;
                         " r.nombre AS rubro "+;                         
...
COLUMN TITLE "Importe"   DATA oQry:importe   PICTURE "9,999,999,999.99" ;
                         SIZE 10 FONT 2 TOTAL
COLUMN TITLE "Utilidad"  DATA oQry:importe - oQry:costo   PICTURE "9,999,999,999.99" ;
                         SIZE 10 FONT 2 TOTAL
...
IF nTipo1 = 1
    ...
    COLUMN TITLE "Pr.X.Op."  DATA oQry:importe/oQry:casos    PICTURE "9,999,999,999.99" SIZE 09 FONT 1
...
ELSE 
    ...
    GROUP oGru ON oQry:id HEADER ALLTRIM(oQry:nombre)+ " Ven:"+ALLTRIM(oQry:vendedor) + ;
            " Emisión:"+DTOC(oQry:fecha)+" Vto:"+DTOC(oQry:fecvto)  FOOTER "Total Presupuesto"  FONT 3
    ...
    COLUMN TITLE "P.Unitar"  DATA oQry:punit    PICTURE "9,999,999,999.99" SIZE 10 FONT 2 TOTAL
    COLUMN TITLE "Desc.%"    DATA oQry:descup   PICTURE "999.99" SIZE 08 FONT 1
    COLUMN TITLE "Desc.$"    DATA oQry:descu    PICTURE "9,999,999,999.99" SIZE 08 FONT 1 TOTAL 
    COLUMN TITLE "Subtotal"  DATA oQry:importe  PICTURE "9,999,999,999.99" SIZE 10 FONT 2 TOTAL
    COLUMN TITLE "Estado"     DATA IF(oQry:estado=1,"Presupuestado","Facturado")      SIZE 11 FONT 1
...
COLUMN TITLE "Fecha"     DATA oQry:fecha     PICTURE "@D" SIZE 08 FONT 1
COLUMN TITLE "Ingreso"   DATA oQry:debe PICTURE "9,999,999,999.99" ;
                         SIZE 10 FONT 2 TOTAL
COLUMN TITLE "Egreso"     DATA oQry:haber PICTURE "9,999,999,999.99" ;
                         SIZE 10 FONT 2 TOTAL
...
COLUMN TITLE "Neto"      DATA oQry:neto      PICTURE "9,999,999,999.99" SIZE 12 FONT 2 TOTAL
COLUMN TITLE "A Expreso" DATA oQry:comiexp   PICTURE "9,999,999,999.99" SIZE 12 FONT 2 TOTAL
COLUMN TITLE "A Cl.Esp." DATA oQry:comicli   PICTURE "9,999,999,999.99" SIZE 12 FONT 2 TOTAL
COLUMN TITLE "Por Flia." DATA oQry:comifam   PICTURE "9,999,999,999.99" SIZE 12 FONT 2 TOTAL
...
COLUMN TITLE "Cliente"       DATA oQry:cliente  SIZE 25 FONT 1
COLUMN TITLE "Neto"          DATA oQry:neto     PICTURE "9,999,999,999.99" SIZE 12 FONT 1 TOTAL
COLUMN TITLE "Total Comp."   DATA oQry:importe  PICTURE "9,999,999,999.99" SIZE 12 FONT 1 TOTAL
...
ventas := oApp:oServer:Query("SELECT v.*, c.* FROM ge_"+oApp:cId+"ventas_encab v LEFT JOIN "+;
                             ...
                             "WHERE letra <> 'X' AND "+; 
                             "v.fecmod >= " + ClipValue2Sql(mdesde) + " AND "+;
                             "       v.fecmod <= "+ ClipValue2Sql(mhasta) + " ORDER BY v.fecmod ")
mtexto := ""
mtexto1:= ""
DO WHILE !ventas:EOF()
// Anterior
    ...
    mtexto := mtexto +;
              DTOS(ventas:fecmod) + cDoc + "0" + SUBSTR(ventas:numcomp,1,4)  + REPLICATE("0",12)+;
...
"SELECT "+;
"    CONCAT(ve.ticomp,ve.letra,ve.numcomp) AS compro, "+;
"    ve.fecmod as fecha, "+; 
"    ve.nombre, "+; 
"    ve.cuit, "+; 
"    LEFT(ti.nombre,3) AS tip, "+;
"    diva.exento * IF(ve.ticomp='NC',-1,1) AS exento, "+;
"    diva.neto * IF(ve.ticomp='NC',-1,1) AS neto, "+;
"    diva.neto1 * IF(ve.ticomp='NC',-1,1) AS neto1, "+;
"    ve.sobretasa * IF(ve.ticomp='NC',-1,1) AS impint, "+;
"    t.tasa AS tasa, "+;
"    t1.tasa AS tasa2, "+;
"    diva.iva1 * IF(ve.ticomp='NC',-1,1) AS iva1, "+;
"    diva.iva2 * IF(ve.ticomp='NC',-1,1) AS iva2, "+;
"    ve.percep, "+;
"    ve.iibb * IF(ve.ticomp='NC',-1,1) AS iibb "+;
"FROM ge_"+oApp:cId+"ventas_encab ve "+;
"JOIN ( "+;
"    SELECT "+; 
"        tipocomp, "+; 
"        letra, "+; 
"        numfac, "+;
"        MAX(CASE WHEN codiva = 3 THEN neto END) AS exento, "+;
"        MAX(CASE WHEN codiva = 5 THEN neto END) AS neto, "+;
"        MAX(CASE WHEN codiva = 4 THEN neto END) AS neto1, "+;
"        MAX(CASE WHEN codiva = 5 THEN iva END) AS iva1, "+;
"        MAX(CASE WHEN codiva = 4 THEN iva END) AS iva2 "+;
"    FROM ge_"+oApp:cId+"ventivadet "+;
"    GROUP BY tipocomp, letra, numfac "+;
") diva ON diva.tipocomp = ve.ticomp "+; 
"         AND diva.letra = ve.letra "+; 
"         AND diva.numfac = ve.numcomp "+;
"LEFT JOIN ge_"+oApp:cId+"ivas t ON t.codigo = 5 "+;
"LEFT JOIN ge_"+oApp:cId+"ivas t1 ON t1.codigo = 4 "+;
...
"AND ve.fecmod BETWEEN " + ClipValue2Sql(mdesde) + " AND "+ ClipValue2Sql(mhasta)+" "+;
"ORDER BY ve.fecmod, ve.ticomp, ve.letra, ve.numcomp")
/*"SELECT "+;
"    CONCAT(ve.ticomp,ve.letra,ve.numcomp) AS compro, "+;
"    ve.fecha, "+; 
"    ve.nombre, "+; 
...
"WHERE ve.letra <> 'X' "+;
IF(nPunto = 0," "," AND LEFT(numcomp,4) = " + ClipValue2Sql(STRTRAN(STR(nPunto,4)," ","0")))+;
"AND ve.fecha BETWEEN " + ClipValue2Sql(mdesde) + " AND "+ ClipValue2Sql(mhasta)+" "+;
"ORDER BY ve.fecha")*/
...
COLUMN TITLE "Exento "     DATA oQry:exento   PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "Neto "       DATA oQry:neto     PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "Neto2"       DATA oQry:neto1    PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "I.V.A. "     DATA oQry:iva1     PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "I.V.A. 2"    DATA oQry:iva2     PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
...
COLUMN TITLE "IIBB"        DATA oQry:iibb     PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "Imp.Int"     DATA oQry:impint   PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
...
" LEFT JOIN ge_"+oApp:cId+"tipoiva ti ON ti.codigo = ve.coniva  "+;
" WHERE vd.letra <> 'X' "+;
IF(nPunto = 0," "," AND LEFT(ve.numcomp,4) = " + ClipValue2Sql(STRTRAN(STR(nPunto,4)," ","0")))+;
" AND ve.fecmod >= " + ClipValue2Sql(mdesde) + " AND "+;
"     ve.fecmod <= "+ ClipValue2Sql(mhasta) +;
...
COLUMN TITLE "Situación I.V.A."  DATA oQry:tip1     SIZE  20 FONT 1
COLUMN TITLE "Exento "     DATA IF(oQry:codiva=3,oQry:neto,0)   PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "Neto "       DATA IF(oQry:codiva=3,0,oQry:neto)     PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "Tasa "       DATA oQry:tasa     PICTURE "99.99"        SIZE 05 FONT 1 
COLUMN TITLE "I.V.A. "     DATA oQry:iva      PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "IIBB"        DATA oQry:iibb     PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "Imp.Int"     DATA oQry:impint   PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
COLUMN TITLE "Total"       DATA oQry:importe  PICTURE "9,999,999,999.99" SIZE 10 FONT 1 TOTAL
...
ENDIF
CursorWait()
oQry := oApp:oServer:Query("SELECT v.cuit,v.fecmod as fecha,IF(v.ticomp='NC','C',IF(v.ticomp='FC','F','D')) as tipo,v.letra,"+;
                           " CONCAT(LEFT(v.numcomp,4),RIGHT(v.numcomp,8)) as compro,"+;
                           "v.neto*IF(v.ticomp='NC',-1,1) as neto,v.iibb*IF(v.ticomp='NC',-1,1) as iibb"+;
                           " FROM ge_"+oApp:cId+"ventas_encab v "+;
                           " WHERE v.fecmod >= " + ClipValue2Sql(mdesde) +;
                           " AND   v.fecmod <= " + ClipValue2Sql(mhasta) +;
  ...
  COLUMN TITLE "Neto"       DATA oQry:neto        PICTURE "9,999,999,999.99" SIZE 10 TOTAL
  COLUMN TITLE "Percepc."   DATA oQry:iibb        PICTURE "9,999,999,999.99" SIZE 10 TOTAL
