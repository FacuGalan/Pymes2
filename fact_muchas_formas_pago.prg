...
ENDIF   
   ...
   */
   ...
   REDEFINE BTNBMP oBot[31] ID 302 OF oDlg1 2007 CENTER; //DESCUENTOS
                   PROMPT "&Descuentos";
                   ACTION(CargaDescu(),oGet[02]:SetFocus()) WHEN("B"$cPermisos)
   REDEFINE BTNBMP oBot[32] ID 303 OF oDlg1 2007 CENTER; //BORRA ITEM
                   PROMPT "&Borrar item [F6]";
                   ACTION(oQryDet:Delete(),oBrwDet:Refresh(),oApp:oServer:Execute("TRUNCATE formapag_temp"),nPagado := 0,oGet[04]:Refresh(),oGet[02]:SetFocus());
                   WHEN(oQryDet:RecCount()>0 .and. "B"$cPermisos)
...
LOCAL nNumero,cNumComp,cLetra:=IF(oApp:tipo_iva<>6,IF(nCondicion = 1 .or. nCondicion = 2 .or. nCondicion = 6 ,"A","B"),"C"),nPuntoVta,nCtaCte,nEfecti,;
      ...
      nCodCue := 0, nNumOpe := 0, cObserva1:=SPACE(255), nNumDep, nPunLocal
...
IF nCodArt >= 0
  ...
  IF oQryPun:pidestock = 2 .and. oQryAux:stockact-nCantidad < 0 .and. !oQryAux:stockotro
     ...
     RETURN .F.
  ENDIF
  IF oQryPun:pidestock < 3 .and. oQryArt:stockmin > 0 .and. oQryArt:stockact-nCantidad  < oQryArt:stockmin .and. !oQryAux:stockotro
...
DEFINE DIALOG oDlgA TITLE "DEPARTAMENTOS" FROM 05,10 TO 22,110 OF oWnd FONT oFont
   ...
   @ 40, 90 GET oGet3 VAR nImporte PICTURE "9999999.99"  OF oDlgA PIXEL RIGHT SIZE 95,22 FONT oFont
...
oApp:oServer:Execute("";
    ...
    +"`IMPINT`  DECIMAL(10,3) DEFAULT '0.00', ";
    +"PRIMARY KEY (`id`)) ENGINE=INNODB DEFAULT CHARSET=utf8")
